...
This guide explains how to connect to the Forge WebSocket API to receive real-time events and send actions to the agent.

## Overview

Forge uses [Socket.IO](https://socket.io/) for WebSocket communication between the client and server. The WebSocket connection allows you to:
...
- Verify that the Forge server is running and accessible
