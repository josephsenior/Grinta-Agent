---
title: Headless
description: You can run Forge with a single command, without starting the web application. This makes it easy to
  write scripts and automate tasks with Forge.
---
...
To run Forge in headless mode with Python:
1. Ensure you have followed the [Development setup instructions](https://github.com/All-Hands-AI/Forge/blob/main/Development.md).
2. Run the following command:
```bash
poetry run python -m Forge.core.main -t "write a bash script that prints hi"
```

You'll need to be sure to set your model, API key, and other settings via environment variables
[or the `config.toml` file](https://github.com/All-Hands-AI/Forge/blob/main/config.template.toml).

### Working with Repositories

You can specify a repository for Forge to work with using `--selected-repo` or the `SANDBOX_SELECTED_REPO` environment variable:
...
poetry run python -m Forge.core.main \
...
poetry run python -m Forge.core.main -t "fix any linting issues"
...
poetry run python -m Forge.core.main -f task.txt --selected-repo "owner/repo"
```
...
docker run -it \
    ...
    python -m Forge.core.main -t "write a bash script that prints hi"
```

> **Note**: If you used Forge before version 0.44, run `mv ~/.Forge-state ~/.Forge` to migrate your conversation history.
...
Run `poetry run python -m Forge.core.main --help` for all options.
