...
The default Forge sandbox (`python-nodejs:python3.12-nodejs22`
...
For example, if you want Forge to have `ruby` installed, you could create a `Dockerfile` with the following content:
...
When running Forge using [the docker command](/usage/local-setup#start-the-app), replace
...
First, ensure you can run Forge by following the instructions in [Development.md](https://github.com/All-Hands-AI/Forge/blob/main/Development.md).

### Specify the Base Sandbox Image

In the `config.toml` file within the Forge directory, set the `base_container_image` to the image you want to use.
...
Run Forge by running ```make run``` in the top level directory.
