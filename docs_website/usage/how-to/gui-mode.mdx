---
title: GUI
description: High level overview of the Graphical User Interface (GUI) in Forge.
---

## Prerequisites

- [Forge is running](/usage/local-setup)

## Launching the GUI Server

### Using the CLI Command

You can launch the Forge GUI server directly from the command line using the `serve` command:

<Callout type="info">
**Prerequisites**: You need to have the [Forge CLI installed](/usage/how-to/cli-mode) first, OR have `uv` installed and run `uvx --python 3.12 --from Forge-ai Forge serve`. Otherwise, you'll need to use Docker directly (see the [Docker section](#using-docker-directly) below).
</Callout>
...
- Launch the Forge GUI server at http://localhost:3000
- Use the same configuration directory (`~/.Forge`) as the CLI mode
...
If you have NVIDIA GPUs and want to make them available to the Forge container, use the `--gpu` flag:
...
Before using the `Forge serve` command, ensure that:
...
- Setup the LLM provider and model for Forge.
...
Forge automatically exports a `GITHUB_TOKEN` to the shell environment if provided:
...
Forge automatically exports a `GITLAB_TOKEN` to the shell environment if provided:
...
<Accordion title="Setting Up a GitLab Token">
  ...
  3. **(Optional): Restrict agent permissions**
   ...
   - Forge will use the higher scope token, and the agent will use the lower scope token
</Accordion>
...
1. **Generate an App Password**:
   ...
   - Create a new password with the following scopes:
     ...
     - `pull requests: write`
     - `issues: write`
   - App passwords are non-expiring token. Forge will migrate to using API tokens in the future.
  2. **Enter Token in Forge**:
   - In the Settings page, navigate to the `Integrations` tab.
...
- Confirmation Mode: Enabling this mode will cause Forge to confirm an action with the user before performing it.
...
- [Run Forge on GitHub issues with a GitHub action.](/usage/how-to/github-action)
