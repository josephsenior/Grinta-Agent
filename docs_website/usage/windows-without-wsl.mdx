...
description: Running Forge GUI on Windows without using WSL or Docker
---

# Running Forge GUI on Windows Without WSL

This guide provides step-by-step instructions for running Forge on a Windows machine without using WSL or Docker.
...
3. **Install .NET Core Runtime**
   ...
   - Verify installation by opening PowerShell and running:
     ...
     dotnet --info
     ```
   - This step is required for the PowerShell integration via pythonnet. Without it, Forge will fall back to a more limited PowerShell implementation.
...
2. **Start the Backend**
   ...
   pwsh
   $env:RUNTIME="local"; poetry run uvicorn Forge.server.listen:app --host 0.0.0.0 --port 3000 --reload --reload-exclude "./workspace"
   ```

   This will start the Forge app using the local runtime with PowerShell integration, available at `localhost:3000`.
...
To install and run the Forge CLI on Windows without WSL, follow these steps:
...
The Forge CLI **requires** the .NET Core runtime for PowerShell integration. Without it, the CLI will fail to start with a `coreclr` error. Install the .NET SDK which includes the runtime:
...
### 3. Install and Run Forge

After installing the prerequisites, you can install and run Forge with:
...
If you encounter an error like `Failed to load CoreCLR` or `pythonnet.load('coreclr')` when running Forge CLI, this indicates that the .NET Core runtime is missing or not properly configured. To fix this:
...
When running Forge on Windows without WSL or Docker, be aware of the following limitations:
...
4. **Run Forge using PowerShell 7** instead of Windows PowerShell:
   ```powershell
   pwsh
   cd path\to\Forge
   $env:RUNTIME="local"; poetry run uvicorn Forge.server.listen:app --host 0.0.0.0 --port 3000 --reload --reload-exclude "./workspace"
