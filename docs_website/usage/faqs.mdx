...
### I'm new to Forge. Where should I start?

1. **Quick start**: Use [Forge Cloud](/usage/cloud/Forge-cloud) to get started quickly with
...
### Can I use Forge for production workloads?

Forge is meant to be run by a single user on their local workstation. It is not appropriate for multi-tenant
deployments where multiple users share the same instance. There is no built-in authentication, isolation, or scalability.

If you're interested in running Forge in a multi-tenant environment, check out the source-available,
commercially-licensed [Forge Cloud Helm Chart](https://github.com/all-Hands-AI/Forge-cloud).

<Info>
Using Forge for work? We'd love to chat! Fill out
...
**Generally yes, but with important considerations.** Forge runs all code in a secure, isolated Docker container
...
Your files are stored in different locations depending on how you've configured Forge:
...
Forge comes with a basic runtime environment that includes Python and Node.js.
...
1. **Search existing issues**: Check our [GitHub issues](https://github.com/All-Hands-AI/Forge/issues) to see if
  others have encountered the same problem.
2. **Join our community**: Get help from other users and developers:
   - [Slack community](https://join.slack.com/t/Forge-ai/shared_invite/zt-3847of6xi-xuYJIPa6YIPg4ElbDWbtSA)
...
4. **Report bugs**: If you've found a bug, please [create an issue](https://github.com/All-Hands-AI/Forge/issues/new)
