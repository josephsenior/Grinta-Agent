...
<Tip>
Forge only supports Windows via WSL. Please be sure to run all commands inside your WSL terminal.
</Tip>
...
When running Forge, the following error is seen:
```
...
* Check if the `~/.Forge` is owned by `root`. If so, you can:
  * Change the directory's ownership: `sudo chown <user>:<user> ~/.Forge`.
  * or update permissions on the directory: `sudo chmod 777 ~/.Forge`
  * or delete it if you don’t need previous data. Forge will recreate it. You'll need to re-enter LLM settings.
...
When accessing Forge through a non-localhost URL (such as a LAN IP address), the VS Code tab shows a "Forbidden"
...
1. Set a specific port for VS Code using the `SANDBOX_VSCODE_PORT` environment variable:
   ...
   docker run -it --rm \
       ...
       --name Forge-app \
       docker.all-hands.dev/all-hands-ai/Forge:latest
   ```
