---
title: Local LLMs
description: When using a Local LLM, Forge may have limited functionality. It is highly recommended that you use GPUs to serve local models for optimal experience.
---
...
- 2025/03/31: We released an open model Forge LM 32B v0.1 that achieves 37.1% on SWE-Bench Verified
([blog](https://www.all-hands.dev/blog/introducing-Forge-lm-32b----a-strong-open-coding-agent-model), [model](https://huggingface.co/all-hands/Forge-lm-32b-v0.1)).

## Quickstart: Running Forge with a Local LLM using LM Studio

This guide explains how to serve a local Devstral LLM using [LM Studio](https://lmstudio.ai/) and have Forge connect to it.
...
- **Devstral Small 2505** as the LLM for software development, trained on real GitHub issues and optimized for agent-style workflows like Forge.
...
1. Check [the installation guide](/usage/local-setup) and ensure all prerequisites are met before running Forge, then run:
...
Status: Image is up to date for docker.all-hands.dev/all-hands-ai/Forge:0.55
Starting Forge...
Running Forge as root
14:22:13 - Forge:INFO: server_config.py:50 - Using config class None
...
### 6. Configure Forge to use the LLM server

Once you open Forge in your browser, you'll need to configure it to use the local LLM server you just started.

When started for the first time, Forge will prompt you to set up the LLM provider.

1. Click "see advanced settings" to open the LLM Settings page.

![image](./screenshots/07_Forge_open_advanced_settings.png)
...
![image](./screenshots/08_Forge_configure_local_llm_parameters.png)

That's it! You can now start using Forge with the local LLM server.

If you encounter any issues, let us know on [Slack](https://join.slack.com/t/Forge-ai/shared_invite/zt-3847of6xi-xuYJIPa6YIPg4ElbDWbtSA) or [Discord](https://discord.gg/ESHStjSjD4).

## Advanced: Alternative LLM Backends

This section describes how to run local LLMs with Forge using alternative backends like Ollama, SGLang, or vLLM — without relying on LM Studio.
...
```bash
# ⚠️ WARNING: Forge requires a large context size to work properly.
...
### Run Forge (Alternative Backends)

#### Using Docker

Run Forge using [the official docker run command](../installation#start-the-app).

#### Using Development Mode

Use the instructions in [Development.md](https://github.com/All-Hands-AI/Forge/blob/main/Development.md) to build Forge.

Start Forge using `make run`.

### Configure Forge (Alternative Backends)

Once Forge is running, open the Settings page in the UI and go to the `LLM` tab.
