...
description: Overview of Forge Cloud integrations with project management platforms including Jira Cloud, Jira Data Center, and Linear. Learn about setup requirements, usage methods, and troubleshooting.
---
...
Forge Cloud integrates with project management platforms (Jira Cloud, Jira Data Center, and Linear) to enable AI-powered task delegation. Users can invoke the Forge agent by:
- Adding `@Forge` in ticket comments
- Adding the `Forge` label to tickets
...
2. **Workspace Integration** - Self-service configuration through the Forge Cloud UI to link your Forge account to the target workspace
...
Once both the platform configuration and workspace integration are completed, users can trigger the Forge agent within their project management platforms using two methods:

### Method 1: Comment Mention
Add a comment to any issue with `@Forge` followed by your task description:
```
@Forge Please implement the user authentication feature described in this ticket
```

### Method 2: Label-based Delegation
Add the label `Forge` to any issue. The Forge agent will automatically process the issue based on its description and requirements.

### Git Repository Detection

The Forge agent needs to identify which Git repository to work with when processing your issues. Here's how to ensure proper repository detection:
...
**Jira Integrations:** Always include the repository information in your issue description or `@Forge` comment to ensure proper repository detection.
...
- **Authentication errors**: Verify Git provider permissions and Forge Cloud access
- **Agent fails to identify git repo**: Ensure you're signing in with the same Git provider account that contains the repositories you want Forge to work on
- **Partial functionality**: Ensure both platform configuration and workspace integration are properly completed

### Getting Help
For additional support, contact Forge Cloud support with:
