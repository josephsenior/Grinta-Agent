...
description: This page outlines all available configuration options for Forge, allowing you to customize its
...
When running Forge in CLI, headless, or development mode, you can use a project-specific `config.toml` file for configuration, which must be
...
Please note that the configuration options may be subject to change in future versions of Forge. It's recommended to refer to the official documentation for the most up-to-date information.
