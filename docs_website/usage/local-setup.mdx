...
description: Getting started with running Forge on your own.
---

## Recommended Methods for Running Forge on Your Local System
...
A system with a modern processor and a minimum of **4GB RAM** is recommended to run Forge.
...
<Accordion title="Windows">
  ...
  **Alternative: Windows without WSL**
  ...
  If you prefer to run Forge on Windows without WSL or Docker, see our [Windows Without WSL Guide](/usage/windows-without-wsl).
...
We recommend using [uv](https://docs.astral.sh/uv/) for the best Forge experience. uv provides better isolation from your current project's virtual environment and is required for Forge' default MCP servers (like the [fetch MCP server](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch)).
...
uvx --python 3.12 --from Forge-ai Forge serve

# Or with GPU support (requires nvidia-docker)
uvx --python 3.12 --from Forge-ai Forge serve --gpu

# Or with current directory mounted
uvx --python 3.12 --from Forge-ai Forge serve --mount-cwd
```
...
docker run -it --rm --pull=always \
    ...
    --name Forge-app \
    docker.all-hands.dev/all-hands-ai/Forge:0.55
```

</Accordion>

> **Note**: If you used Forge before version 0.44, you may want to run `mv ~/.Forge-state ~/.Forge` to migrate your conversation history to the new location.

You'll find Forge running at http://localhost:3000!

### Setup

After launching Forge, you **must** select an `LLM Provider` and `LLM Model` and enter a corresponding `API Key`.
...
Forge requires an API key to access most language models. Here's how to get an API key from the recommended providers:

<AccordionGroup>

<Accordion title="Forge (Recommended)">

1. [Log in to Forge Cloud](https://app.all-hands.dev).
2. Go to the Settings page and navigate to the `API Keys` tab.
3. Copy your `LLM API Key`.

Forge provides access to state-of-the-art agentic coding models with competitive pricing. [Learn more about Forge LLM provider](/usage/llms/Forge-llms).
...
To run Forge with a locally hosted language model instead of a cloud provider, see the [Local LLMs guide](/usage/llms/local-llms) for setup instructions.

#### Setting Up Search Engine

Forge can be configured to use a search engine to allow the agent to search the web for information when needed.

To enable search functionality in Forge:
...
The [docker command above](/usage/local-setup#start-the-app) pulls the most recent stable release of Forge. You have other options as well:
- For a specific release, replace `$VERSION` in `Forge:$VERSION` and `runtime:$VERSION`, with the version number.
For example, `0.9` will automatically point to the latest `0.9.x` release, and `0` will point to the latest `0.x.x` release.
- For the most up-to-date development version, replace `$VERSION` in `Forge:$VERSION` and `runtime:$VERSION`, with `main`.
This version is unstable and is recommended for testing or development purposes only.

## Next Steps

- [Connect Forge to your local filesystem.](/usage/runtimes/docker#connecting-to-your-filesystem) to use Forge with your GitHub repositories
- [Run Forge in a scriptable headless mode.](/usage/how-to/headless-mode)
- [Run Forge with a friendly CLI.](/usage/how-to/cli-mode)
- [Run Forge on tagged issues with a GitHub action.](/usage/how-to/github-action)
