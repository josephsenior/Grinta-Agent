...
description: This page outlines how to configure and use the Model Context Protocol (MCP) in Forge, allowing you
  to extend the agent's capabilities with custom tools.
---

## Overview

Model Context Protocol (MCP) is a mechanism that allows Forge to communicate with external tool servers. These
...
<Note>
MCP is currently not available on Forge Cloud. This feature is only available when running Forge locally.
</Note>

### How MCP Works

When Forge starts, it:
...
1. Forge routes the call to the appropriate MCP server.
2. The server processes the request and returns a response.
3. Forge converts the response to an observation and presents it to the agent.

## Configuration

MCP configuration can be defined in:
* The Forge UI through the Settings under the `MCP` tab.
...
Then configure Forge to use the HTTP endpoint:
...
Forge supports three different MCP transport protocols:
