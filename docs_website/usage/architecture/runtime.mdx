...
The Forge Docker Runtime is the core component that enables secure and flexible execution of AI agent's action.
It creates a sandboxed environment using Docker, where arbitrary code can be run safely without risking the host system.

## Why do we need a sandboxed runtime?

Forge needs to execute arbitrary code in a secure, isolated environment for several reasons:
...
The Forge Runtime system uses a client-server architecture implemented with Docker containers. Here's an overview of how it works:

```mermaid
graph TD
    A[User-provided Custom Docker Image] --> B[Forge Backend]
...
2. Image Building: Forge builds a new Docker image (the "OH runtime image") based on the user-provided image. This new image includes Forge-specific code, primarily the "runtime client"
3. Container Launch: When Forge starts, it launches a Docker container using the OH runtime image
...
5. Communication: The Forge backend (client: `Forge/runtime/impl/action_execution/action_execution_client.py`; runtimes: `Forge/runtime/impl/docker/docker_runtime.py`, `Forge/runtime/impl/local/local_runtime.py`) communicates with the action execution server over RESTful API, sending actions and receiving observations
6. Action Execution: The runtime client receives actions from the backend, executes them in the sandboxed environment, and sends back observations
7. Observation Return: The action execution server sends execution results back to the Forge backend as observations

The role of the client:

- It acts as an intermediary between the Forge backend and the sandboxed environment
...
## How Forge builds and maintains OH Runtime images

Forge' approach to building and managing runtime images ensures efficiency, consistency, and flexibility in creating and maintaining Docker images for both production and development environments.

Check out the [relevant code](https://github.com/All-Hands-AI/Forge/blob/main/Forge/runtime/utils/runtime_build.py) if you are interested in more details.

### Image Tagging System

Forge uses a three-tag system for its runtime images to balance reproducibility with flexibility.
The tags are:

- **Versioned Tag**: `oh_v{Forge_version}_{base_image}` (e.g.: `oh_v0.9.9_nikolaik_s_python-nodejs_t_python3.12-nodejs22`)
- **Lock Tag**: `oh_v{Forge_version}_{16_digit_lock_hash}` (e.g.: `oh_v0.9.9_1234567890abcdef`)
- **Source Tag**: `oh_v{Forge_version}_{16_digit_lock_hash}_{16_digit_source_hash}`
...
This effectively gives a hash for the dependencies of Forge independent of the source code.

#### Versioned Tag - Most Generic

This tag is a concatenation of Forge version and the base image name (transformed to fit in tag standard).

#### Build Process

When generating an image...

- **No re-build**: Forge first checks whether an image with the same **most specific source tag** exists. If there is such an image,
  no build is performed - the existing image is used.
- **Fastest re-build**: Forge next checks whether an image with the **generic lock tag** exists. If there is such an image,
  Forge builds a new image based upon it, bypassing all installation steps (like `poetry install` and
...
This tagging approach allows Forge to efficiently manage both development and production environments.

1. Identical source code and Dockerfile always produce the same image (via hash-based tags)
2. The system can quickly rebuild images when minor changes occur (by leveraging recent compatible images)
3. The **lock** tag (e.g., `runtime:oh_v0.9.3_1234567890abcdef`) always points to the latest build for a particular base image, dependency, and Forge version combination

## Volume mounts: named volumes and overlay

Forge supports both bind mounts and Docker named volumes in SandboxConfig.volumes:
...
- Forge/runtime/impl/docker/docker_runtime.py (named volumes in _build_docker_run_args; overlay mounts in _process_overlay_mounts)
- Forge/core/config/sandbox_config.py (volumes field)


## Runtime Plugin System

The Forge Runtime supports a plugin system that allows for extending functionality and customizing the runtime environment. Plugins are initialized when the action execution server starts up inside the runtime.
...
References:
- Forge/runtime/impl/docker/docker_runtime.py (port ranges, locking, DOCKER_HOST_ADDR, vscode_url)
- Forge/runtime/impl/local/local_runtime.py (vscode_url factory)
- Forge/runtime/impl/remote/remote_runtime.py (vscode_url mapping)
- Forge/runtime/action_execution_server.py (/vscode/connection_token)


Examples:
- Jupyter: Forge/runtime/plugins/jupyter/__init__.py (JupyterPlugin, Kernel Gateway)
- VS Code: Forge/runtime/plugins/vscode/* (VSCodePlugin, exposes tokenized URL)
- Agent Skills: Forge/runtime/plugins/agent_skills/*

Key aspects of the plugin system:

1. Plugin Definition: Plugins are defined as Python classes that inherit from a base `Plugin` class
2. Plugin Registration: Available plugins are registered in `Forge/runtime/plugins/__init__.py` via `ALL_PLUGINS`
