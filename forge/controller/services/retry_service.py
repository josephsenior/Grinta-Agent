...
from forge.core.retry_queue import RetryQueue, RetryTask, get_retry_queue
...
class RetryService:
    ...
    def initialize(self) -> None:
        ...
        async def _worker_wrapper() -> None:
            try:
                ...
                logger.exception(
                "Retry worker crashed for controller %s: %s", self.controller.id, exc
                )
    ...
    async def schedule_retry_after_failure(self, exc: Exception) -> bool:
        ...
        controller.state.last_error = f"{type(exc).__name__}: retry scheduled"
    ...
    async def _handle_task_failure(self, task: RetryTask, exc: Exception) -> None:
        ...
        if queue is None:
            ...
            return
        try:
            rescheduled = await queue.mark_failure(
            ...
            logger.warning(
                "Retry queue backend error during cleanup: %s", backend_exc
            )
            self._retry_pending = False
    ...
    async def _resume_agent_after_retry(self, task: RetryTask) -> None:
        ...
        controller = self.controller
        message = (
            f"🔁 Retrying after {task.reason}. Attempt {task.attempts}/{task.max_attempts}."
        )
        controller.event_stream.add_event(
