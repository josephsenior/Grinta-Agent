...
class SocketIOConnectionManager:
    ...
    def register_connection(
        ...
        logger.info(
            f"Connection registered: sid={sid}, user_id={user_id}, conversation_id={conversation_id}"
        )
    ...
    def unregister_connection(self, sid: str) -> None:
        ...
        logger.info(f"Connection unregistered: sid={sid}")
    ...
    def queue_message(self, sid: str, event: str, data: Any) -> bool:
        ...
        if len(conn_info.message_queue) >= conn_info.max_queue_size:
            logger.warning(f"Message queue full for connection {sid}")
            return False
    ...
    async def deliver_queued_messages(self, sid: str, sio: "socketio.AsyncServer") -> int:
        ...
        while conn_info.message_queue:
            ...
            if now - queued_at > self._message_queue_ttl:
                ...
                logger.debug(f"Dropped expired message for {sid}")
                continue
            ...
            try:
                ...
                logger.error(f"Error delivering queued message to {sid}: {e}")
                break

        if delivered > 0:
            logger.info(f"Delivered {delivered} queued messages to {sid}")
...
_connection_manager: Optional[SocketIOConnectionManager] = None
