...
class RequestSizeLoggingMiddleware:
    ...
    def _content_length_from_headers(self, headers) -> Optional[int]:
        try:
    ...
    def _response_size(self, response: Response) -> Optional[int]:
        try:
    ...
    def _wrap_streaming_response(
        ...
        response: Response,
        request: Request,
        req_size: Optional[int],
        request_id: Optional[str],
    ) -> bool:
        body_iterator = getattr(response, "body_iterator", None)
        if (
            body_iterator is None
            or not callable(getattr(body_iterator, "__aiter__", None))
        ):
            return False
