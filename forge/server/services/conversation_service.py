...
def _process_git_provider_tokens(git_provider_tokens: PROVIDER_TOKEN_TYPE | None) -> PROVIDER_TOKEN_TYPE:
...
def _process_custom_secrets(custom_secrets: CUSTOM_SECRETS_TYPE_WITH_JSON_SCHEMA | None) -> MappingProxyType:
    ...
    if isinstance(custom_secrets, UserSecrets):
        return custom_secrets.custom_secrets
...
async def setup_init_conversation_settings(
    ...
    normalized_tokens = _process_git_provider_tokens(provider_tokens)
    if not normalized_tokens:
        normalized_tokens = settings.secrets_store.provider_tokens
    ...
    for provider in providers_set or []:
        if isinstance(provider, ProviderType):
            ...
            try:
                normalized_providers.append(ProviderType(provider))  # type: ignore[arg-type]
            except Exception:
                continue
    ...
    if normalized_providers:
        ...
        normalized_tokens = MappingProxyType(token_dict)
