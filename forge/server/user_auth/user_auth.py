"""Authentication abstractions and helpers for Forge server requests."""
...
class AuthType(str, Enum):
    """Enumerate supported authentication flows for the server."""
...
class UserAuth(ABC):
    ...
    This is an extension point in Forge that allows applications to provide their own
    ...
    def get_auth_type(self) -> AuthType | None:
        """Return configured authentication type based on environment variables."""
        auth_type = os.getenv("FORGE_AUTH_TYPE", "basic")
        return None
...
async def get_user_auth(request: Request) -> UserAuth:
    ...
    if user_auth is None:
        msg = "Failed to get user auth instance"
