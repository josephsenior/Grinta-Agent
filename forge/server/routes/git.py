...
async def get_user_repositories(
    ...
    logger.info(
        "Returning 401 Unauthorized - Git provider token required for user_id: %s",
        user_id,
    )
    msg = "Git provider token required. (such as GitHub)."
    raise AuthenticationError(msg)
...
async def get_suggested_tasks(
    ...
    logger.info(
        "Returning 401 Unauthorized - No providers set for user_id: %s",
        user_id,
    )
...
async def get_repository_microagents(
    ...
    try:
        ...
        return JSONResponse(
            content=f"Error scanning repository: {
                e!s}",
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
async def get_repository_microagent_content(
    repository_name: str,
    file_path: Annotated[str, Query(description="Path to the microagent file within the repository")],
    provider_tokens: Annotated[PROVIDER_TOKEN_TYPE | None, Depends(get_provider_tokens)] = None,
    access_token: Annotated[SecretStr | None, Depends(get_access_token)] = None,
    user_id: Annotated[str | None, Depends(get_user_id)] = None,
    ...
    try:
        ...
        return JSONResponse(
            content=f"Error fetching microagent content: {
                e!s}",
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
async def summarize_repository(
    ...
    return JSONResponse(
        ...
        content={"error": "Repository summarization is not implemented on this server."},
    )
