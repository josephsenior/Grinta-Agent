...
class MetricsCollector:
    ...
    def record_conversation_start(conversation_id: str, user_id: str | None = None) -> None:
        ...
        if user_id:
            ...
            _metrics_store["users"][user_id]["conversations"] = _metrics_store["users"][user_id].get("conversations", 0) + 1
    ...
    def record_llm_call(
        ...
        _metrics_store["llm_calls"][key] = _metrics_store["llm_calls"].get(key, {
            "total": 0,
            "successful": 0,
            "failed": 0,
            "total_cost": 0.0,
    ...
    def record_api_request(
        ...
        _metrics_store["api_requests"][key] = _metrics_store["api_requests"].get(key, {
            "total": 0,
    ...
    def get_metrics_summary() -> Dict[str, Any]:
        ...
        return {
            "conversations": {
                ...
                "p50_duration_seconds": round(percentile(durations, 0.5), 2),
                "p95_duration_seconds": round(percentile(durations, 0.95), 2),
                "p99_duration_seconds": round(percentile(durations, 0.99), 2),
            },
            "llm": {
            ...
            "api": {
                ...
                "p50_latency_ms": round(percentile(api_latencies, 0.5), 2),
                "p95_latency_ms": round(percentile(api_latencies, 0.95), 2),
                "p99_latency_ms": round(percentile(api_latencies, 0.99), 2),
                "by_endpoint": {
                    key: {
                        "total": stats.get("total", 0),
                        "by_status": dict(stats.get("by_status", {})),
                        "avg_latency_ms": round(
                            sum(stats.get("latencies", [])) / len(stats.get("latencies", [1])), 2
                        ),
                    }
