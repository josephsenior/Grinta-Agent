"""Conversation manager that runs agent loops inside nested Docker runtimes."""
...
from forge.server.shared import event_service_adapter
...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def __aenter__(self):
        """Prepare runtime class inside the nested container before usage."""
        runtime_cls = get_runtime_cls(self.config.runtime)
        runtime_cls.setup(self.config)

    async def __aexit__(self, exc_type, exc_value, traceback):
        """Teardown nested runtime resources when the manager exits."""
        runtime_cls = get_runtime_cls(self.config.runtime)
    ...
    async def attach_to_conversation(
        ...
        """Nested runtimes cannot attach to existing conversations (unsupported)."""
        msg = "unsupported_operation"
        raise ValueError(msg)

    async def detach_from_conversation(
        self, conversation: ServerConversation
    ) -> NoReturn:
        """Nested runtimes cannot detach conversations (unsupported)."""
        msg = "unsupported_operation"
        raise ValueError(msg)
    ...
    async def join_conversation(
        ...
        """Joining conversations is not supported when nested in Docker."""
        msg = "unsupported_operation"
        raise ValueError(msg)
    ...
    async def get_running_agent_loops(
        ...
        conversation_ids = {
            name[len("Forge-runtime-") :]
            for name in names
            if name.startswith("Forge-runtime-")
        }
    ...
    async def get_connections(
        ...
        """Nested runtimes do not maintain direct socket connections."""
    ...
    async def maybe_start_agent_loop(
        ...
        """Start the nested runtime if needed and return its connection info."""
    ...
    async def _start_agent_loop(
        ...
        async with self._conversation_locks[sid]:
            ...
            if sid in self._starting_conversation_ids:
                logger.info(
                    f"Conversation {sid} already starting, skipping duplicate initialization"
                )
                return
            ...
            try:
                ...
                if await self._start_existing_container(runtime):
                    ...
                    if sid not in self._health_monitors:
                        self._health_monitors[sid] = asyncio.create_task(
                            self._monitor_runtime_health(sid)
                        )
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                await self._initialize_conversation(
                    client, api_url, sid, settings, initial_user_msg, replay_json
                )
        finally:
            self._starting_conversation_ids.discard(sid)
    ...
    async def _send_settings(
        ...
        """
        settings_json = model_dump_with_options(
            settings, context={"expose_secrets": True}
        )
    ...
    async def _send_provider_tokens(
        self, client: httpx.AsyncClient, api_url: str, settings: Settings
    ) -> None:
    ...
    async def _send_custom_secrets(
        self, client: httpx.AsyncClient, api_url: str, settings: Settings
    ) -> None:
    ...
    async def _initialize_conversation(
        ...
        init_conversation: dict[str, Any] = {
            "initial_user_msg": initial_user_msg.content
    ...
    async def send_to_event_stream(self, connection_id: str, data: dict) -> NoReturn:
        """Nested runtimes do not expose direct event streams (unsupported)."""
        msg = "unsupported_operation"
        raise ValueError(msg)
    ...
    async def request_llm_completion(
        ...
        """Nested runtimes cannot proxy LLM completions directly."""
        msg = "unsupported_operation"
        raise ValueError(msg)

    async def send_event_to_conversation(self, sid, data) -> None:
        """Forward events to the nested runtime's HTTP API."""
        async with httpx.AsyncClient(
            headers={
                "X-Session-API-Key": self._get_session_api_key_for_conversation(sid)
            },
    ...
    async def disconnect_from_session(self, connection_id: str) -> NoReturn:
        """Disconnect is unsupported because nested runtimes manage connections."""
        msg = "unsupported_operation"
        raise ValueError(msg)

    async def close_session(self, sid: str) -> None:
        """Shutdown the nested runtime container for a conversation."""
        try:
            container = self.docker_client.containers.get(f"Forge-runtime-{sid}")
        ...
        try:
            async with httpx.AsyncClient(
                ...
                if response.status_code == 200:
                    ...
                    logger.debug(
                        "Failed to get conversation info for %s: %s",
                        conversation_id,
    ...
    async def get_agent_loop_info(
        ...
        """Return metadata for all nested runtime conversations."""
        results = []
        containers: list[Container] = self.docker_client.containers.list()
        for container in containers:
            if not container.name or not container.name.startswith("Forge-runtime-"):
                continue
            conversation_id = container.name[len("Forge-runtime-") :]
            ...
            agent_loop_info = AgentLoopInfo(
                ...
                event_store=NestedEventStore(
                    base_url=nested_url, sid=conversation_id, user_id=user_id
                ),
                status=(
                    ConversationStatus.STARTING
    ...
    def get_instance(
        ...
        """Get or create DockerNestedConversationManager instance.
        ...
        Args:
            ...
            monitoring_listener: Monitoring listener
    ...
    def _get_nested_url(self, sid: str) -> str:
        """Get nested URL for a conversation session.

        Args:
            sid: Session ID

        Returns:
            Nested URL for accessing the container
    ...
    def get_nested_url_for_container(self, container: Container) -> str:
        """Get nested URL for a Docker container.

        Args:
            container: Docker container object

        Returns:
            Nested URL string for accessing the container

        """
        env = container.attrs["Config"]["Env"]
        container_port = int(next(e[5:] for e in env if e.startswith("port=")))
        container_name = container.name or ""
        conversation_id = container_name[len("Forge-runtime-") :]
    ...
    def _get_session_api_key_for_conversation(self, conversation_id: str) -> str:
        jwt_secret = (
            self.config.jwt_secret.get_secret_value() if self.config.jwt_secret else ""
        )
    ...
    async def ensure_num_conversations_below_limit(
        ...
        """Enforce configured maximum concurrent conversations per user."""
    ...
    def _get_provider_handler(self, settings: Settings) -> ProviderHandler:
        ...
        # Normalize to MappingProxyType for ProviderHandler
        normalized_tokens: MappingProxyType[ProviderType, ProviderToken]
        if provider_tokens is None:
            normalized_tokens = MappingProxyType({})
        elif isinstance(provider_tokens, MappingProxyType):
            normalized_tokens = provider_tokens
        else:
            normalized_tokens = MappingProxyType(dict(provider_tokens))
        return ProviderHandler(provider_tokens=cast(PROVIDER_TOKEN_TYPE, normalized_tokens))  # type: ignore[arg-type]

    async def _create_runtime(
        self, sid: str, user_id: str | None, settings: Settings
    ) -> DockerRuntime:
        config: ForgeConfig = ExperimentManagerImpl.run_config_variant_test(
        ...
        if config.file_store == "local":
            ...
            volumes.append(
                f"{file_store_path}/{conversation_dir}:/root/.Forge/{conversation_dir}:rw"
            )
        ...
        if event_service_adapter is None:  # type: ignore[unreachable]
            raise RuntimeError("Event service adapter is not initialized")
        event_service_adapter.start_session(
            session_id=sid,
            user_id=user_id,
            labels={"source": "docker_nested_conversation_manager"},
        )
        event_stream = event_service_adapter.get_event_stream(sid)
        runtime = DockerRuntime(
            config=config,
            ...
            main_module="forge.server",
            llm_registry=llm_registry,
        )
        runtime.setup_initial_env = lambda: None  # type: ignore[assignment]
        return runtime
    ...
    async def _monitor_runtime_health(self, sid: str) -> None:
        ...
        while True:
            try:
                ...
                if (
                    sid not in self._starting_conversation_ids
                    and not await self.is_agent_loop_running(sid)
                ):
                    logger.info(
                        f"Conversation {sid} no longer active, stopping health monitor"
                    )
                    break
                ...
                try:
                    ...
                    if status in ["exited", "dead"]:
                        logger.error(
                            f"🔴 Runtime container {container_name} crashed (status={status})!"
                        )
                    ...
                    logger.warning(
                        f"Runtime container {container_name} not found (attempt {consecutive_failures}/{max_consecutive_failures})"
                    )

                    if consecutive_failures >= max_consecutive_failures:
                        logger.error(
                            f"Runtime container {container_name} missing after {consecutive_failures} checks, stopping monitor"
                        )
                        break
    ...
    async def _start_existing_container(self, runtime: DockerRuntime) -> bool:
        try:
            if container := self.docker_client.containers.get(runtime.container_name):
                ...
                if status in ["exited", "dead"]:
                    logger.warning(
                        f"Runtime container {runtime.container_name} crashed (status={status}), removing for auto-restart..."
                    )
                ...
                logger.warning(
                    f"Container {runtime.container_name} in unknown status '{status}', removing..."
                )
                await call_sync_from_async(container.remove, force=True)
                return False
            return False
