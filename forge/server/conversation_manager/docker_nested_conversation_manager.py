...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def get_running_agent_loops(
        ...
        conversation_ids = {
            name[len("Forge-runtime-"):] for name in names if name.startswith("Forge-runtime-")
        }
    ...
    async def _create_runtime(self, sid: str, user_id: str | None, settings: Settings) -> DockerRuntime:
        config: ForgeConfig = ExperimentManagerImpl.run_config_variant_test(user_id, sid, self.config)
        ...
        if config.file_store == "local":
            ...
            volumes.append(f"{file_store_path}/{conversation_dir}:/root/.Forge/{conversation_dir}:rw")
