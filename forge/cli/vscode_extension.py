"""Helpers for detecting editors and installing the Forge VS Code extension."""
...
def _info(message: str) -> None:
    """Print helper to allow easy mocking in tests."""
    print(message)
...
def download_latest_vsix_from_github() -> str | None:
    ...
    """
    api_url = "https://api.github.com/repos/All-Hands-AI/Forge/releases"
    try:
...
def _handle_already_installed_extension(
    ...
    if _is_extension_installed(editor_command, extension_id):
        _info(f"INFO: Forge {editor_name} extension is already installed.")
...
def attempt_vscode_extension_install() -> None:
    """Check if running in a supported editor and install the Forge companion extension.
...
def _is_extension_installed(editor_command: str, extension_id: str) -> bool:
    """Check if the Forge extension is already installed.
...
def _attempt_bundled_install(editor_command: str, editor_name: str) -> bool:
    ...
    Uses the VSIX file packaged with the Forge installation.
