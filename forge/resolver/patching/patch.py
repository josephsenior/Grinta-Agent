"""Parsing utilities that convert SCM patches into structured diff objects."""
...
def parse_patch(text: str | list[str]) -> Iterable[diffobj]:
    """Yield parsed diff objects (header, changes, raw text) from patch input."""
...
def parse_header(text: str | list[str]) -> header | None:
    """Parse header information using SCM-specific or diff headers."""
...
def parse_scm_header(text: str | list[str]) -> header | None:
    """Parse version-control specific headers (git, cvs, svn) if present."""
    lines = text.splitlines() if isinstance(text, str) else text
    check = [
...
def parse_diff_header(text: str | list[str]) -> header | None:
    """Parse generic diff headers (unified, context, diffcmd) as fallback."""
    lines = text.splitlines() if isinstance(text, str) else text
    check = [
...
def parse_diff(text: str | list[str]) -> list[Change] | None:
    """Parse body hunks from diff text and return normalized changes."""
    lines = text.splitlines() if isinstance(text, str) else text
    check = [
...
def parse_diffcmd_header(text: str | list[str]) -> header | None:
    """Parse generic diff command header and return a header tuple."""
    lines = text.splitlines() if isinstance(text, str) else text
...
def parse_unified_header(text: str | list[str]) -> header | None:
    """Parse unified diff header section and return normalized header info."""
    lines = text.splitlines() if isinstance(text, str) else text
...
def parse_context_header(text: str | list[str]) -> header | None:
    """Parse context diff header section and return normalized header info."""
    lines = text.splitlines() if isinstance(text, str) else text
