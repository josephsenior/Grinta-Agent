 ...
 */
function createNewForgeTerminal(): vscode.Terminal {
 ...
 * @returns vscode.Terminal
 */
function findOrCreateForgeTerminal(): vscode.Terminal {
  const ForgeTerminals = vscode.window.terminals.filter((terminal) =>
    terminal.name.startsWith("Forge"),
  );
      ...
      if (event.execution === execution) {
        if (event.exitCode === 0) {
          outputChannel.appendLine(
            "DEBUG: Forge command completed successfully",
          );
          ...
          outputChannel.appendLine(
            `DEBUG: Forge command exited with code ${event.exitCode}`,
          );
