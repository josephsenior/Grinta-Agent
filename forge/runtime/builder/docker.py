"""Docker-based runtime builder implementation and supporting helpers."""
...
from forge.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    """RuntimeBuilder implementation that builds and caches Docker images."""

    def __init__(self, docker_client: docker.DockerClient) -> None:
        """Store docker client, validate server compatibility, and prepare logging."""
    ...
    def _parse_image_tags(self, tags: list[str]) -> tuple[str, str, str, str | None]:
    ...
    def _build_docker_command(
        ...
        buildx_cmd = [
            ...
            "build",
            "--progress=plain",
            f"--build-arg=FORGE_RUNTIME_VERSION={oh_version}",
            f"--build-arg=FORGE_RUNTIME_BUILD_TIME={datetime.datetime.now().isoformat()}",
