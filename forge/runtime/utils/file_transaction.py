...
class FileTransaction:
    ...
    async def __aexit__(self, exc_type, exc_value, traceback) -> None:
        ...
        if exc_type is not None:
            ...
            logger.error(
                f"File transaction failed: {exc_value}, rolling back {len(self.operations)} operations"
            )
            await self.rollback()
        elif not self.committed:
            # No exception, but commit wasn't called explicitly - auto-commit
            logger.info(
                f"Auto-committing file transaction with {len(self.operations)} operations"
            )
            self.committed = True
    ...
    async def commit(self) -> None:
        ...
        self.committed = True
        logger.info(
            f"Committed file transaction with {len(self.operations)} operations"
        )
    ...
    async def rollback(self) -> None:
        ...
        for operation in reversed(self.operations):
            try:
                ...
                logger.error(
                    f"Failed to rollback operation {operation.operation_type} on {operation.file_path}: {e}"
                )
