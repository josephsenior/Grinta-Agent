...
from forge.linter import DefaultLinter
from forge.utils.chunk_localizer import Chunk, get_top_k_chunk_matches
...
def get_new_file_contents(llm: LLM, old_contents: str, draft_changes: str, num_retries: int = 3) -> str | None:
    ...
    while num_retries > 0:
        messages = [
            {"role": "user", "content": USER_MSG.format(old_contents=old_contents, draft_changes=draft_changes)},
        ]
...
class FileEditRuntimeMixin:
    ...
    def __init__(self, enable_llm_editor: bool, llm_registry: LLMRegistry, *args: Any, **kwargs: Any) -> None:
            ...
            if len(updated_lint_error) > 0:
                ...
                error_message = (
                    f"\n[Linting failed for edited file {filepath}. {
                        len(updated_lint_error)} lint errors found.]\n[begin attempted changes]\n{
                        _obs.visualize_diff(
    ...
    def _handle_new_file_creation(self, action: FileEditAction) -> FileEditObservation:
    ...
    def _handle_append_edit(
        self, action: FileEditAction, original_content: str, old_lines: list[str], retry_num: int,
    ) -> Observation:
    ...
    def _build_range_error_message(
        self, action: FileEditAction, start_idx: int, end_idx: int, length_of_range: int, original_content: str,
    ) -> str:
        ...
        error_msg = f"[Edit error: The range of lines to edit is too long.]\n[The maximum number of lines allowed to edit at once is {
            self.MAX_LINES_TO_EDIT}. Got (L{
            start_idx + 1}-L{end_idx}) {length_of_range} lines.]\n"
        ...
        for i, chunk in enumerate(topk_chunks):
            ...
            error_msg += f"[begin relevant snippet {
                i +
                1}. Line range: L{
                chunk.line_range[0]}-L{
                chunk.line_range[1]}. Similarity: {
                chunk.normalized_lcs}]\n"
        ...
        error_msg += f'**IMPORTANT**: Please REDUCE the range of edits to less than {
            self.MAX_LINES_TO_EDIT} lines by setting `start` and `end` in the edit action (e.g. `<file_edit path="{
            action.path}" start=[PUT LINE NUMBER HERE] end=[PUT LINE NUMBER HERE] />`). '
        return error_msg
    ...
    def llm_based_edit(self, action: FileEditAction, retry_num: int = 0) -> Observation:
        ...
        if isinstance(obs, ErrorObservation) and "File not found".lower() in obs.content.lower():
            logger.debug(
                "Agent attempted to edit a file that does not exist. Creating the file. Error msg: %s", obs.content,
            )
        ...
        end_idx = action.end - 1 + 1 if action.end != -1 else len(old_file_lines)
        ...
        if length_of_range > self.MAX_LINES_TO_EDIT + 1:
            error_msg = self._build_range_error_message(
                action, start_idx, end_idx, length_of_range, original_file_content,
            )
    ...
    def correct_edit(self, file_content: str, error_obs: ErrorObservation, retry_num: int = 0) -> Observation:
        ...
        from forge.agenthub.codeact_agent.tools import LLMBasedFileEditTool
