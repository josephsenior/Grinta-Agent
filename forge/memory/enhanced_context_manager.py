...
class EnhancedContextManager:
    ...
    def search_decisions(self, query: str) -> List[Decision]:
        ...
        for decision in self.decisions.values():
            if (query_lower in decision.description.lower() or
                query_lower in decision.rationale.lower() or
                query_lower in decision.context.lower()):
    ...
    def _promote_to_long_term(self) -> None:
        ...
        self.working_memory = [
            ...
            if id(item) not in promoted_items
        ]
    ...
    def _cleanup_long_term(self) -> None:
        ...
        remove_candidates.sort(
            key=lambda x: (x.get("access_count", 0), x.get("timestamp", datetime.now())),
            reverse=False
        )
    ...
    def detect_contradiction(
        ...
        for decision in self.decisions.values():
            ...
            if self._contains_negation(new_lower, desc_lower):
                ...
                logger.warning(f"⚠️  Contradiction detected with decision: {decision.description}")
        ...
        for anchor in self.anchors.values():
            ...
            if self._contains_negation(new_lower, content_lower):
                self.stats["contradictions_detected"] += 1
                logger.warning(f"⚠️  Contradiction detected with anchor: {anchor.content[:50]}...")
    ...
    def _contains_negation(self, new_text: str, old_text: str) -> bool:
        ...
        negation_words = {"not", "no", "never", "won't", "don't", "shouldn't", "can't", "doesn't"}
    ...
    def get_relevant_context(
        ...
        if include_decisions:
            ...
            all_decisions = {d.decision_id: d for d in recent_decisions + searched_decisions}
    ...
    def load_from_file(self, file_path: str) -> None:
        ...
        try:
            ...
            # Load decisions
            self.decisions = {
                did: Decision.from_dict(d) for did, d in state.get("decisions", {}).items()
            }
            self.decision_history = state.get("decision_history", [])
            
            # Load anchors
            self.anchors = {
                aid: ContextAnchor.from_dict(a) for aid, a in state.get("anchors", {}).items()
            }
