...
class PromptRegistry:
    ...
    def __init__(self):
        ...
        self._prompt_variants: Dict[str, List[str]] = defaultdict(list)  # prompt_id -> [variant_ids]
        self._active_variants: Dict[str, str] = {}  # prompt_id -> active_variant_id
        self._testing_variants: Dict[str, List[str]] = defaultdict(list)  # prompt_id -> [testing_variant_ids]
    ...
    def update_variant_metrics(self, variant_id: str, success: bool, 
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        with self._lock:
            ...
            for variant in self._variants.values():
                category_counts[variant.category.value] += 1
            
            # Count active and testing variants
            active_count = len(self._active_variants)
            testing_count = sum(len(variants) for variants in self._testing_variants.values())
    ...
    def export_data(self) -> Dict[str, any]:
        ...
        with self._lock:
            return {
                'variants': {vid: variant.to_dict() for vid, variant in self._variants.items()},
                'prompt_variants': dict(self._prompt_variants),
                'active_variants': dict(self._active_variants),
                'testing_variants': {pid: list(vids) for pid, vids in self._testing_variants.items()}
            }
    ...
    def import_data(self, data: Dict[str, any]):
        ...
        with self._lock:
            ...
            self._prompt_variants.update(data.get('prompt_variants', {}))
            
            # Import active variants
            self._active_variants.update(data.get('active_variants', {}))
            
            # Import testing variants
            for pid, vids in data.get('testing_variants', {}).items():
