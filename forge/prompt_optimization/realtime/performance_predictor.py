...
class PerformancePredictor:
    ...
    def _extract_features(
        ...
        historical_success_rate = historical_metrics.success_rate if historical_metrics else 0.5
        historical_avg_time = historical_metrics.avg_execution_time if historical_metrics else 1.0
        historical_error_rate = historical_metrics.error_rate if historical_metrics else 0.1
    ...
    def _calculate_complexity(self, content: str) -> float:
        ...
        complexity_bonus = sum(1 for indicator in complex_indicators if indicator in content.lower())
    ...
    def _features_to_array(self, features: PredictionFeatures) -> np.ndarray:
        ...
        task_type_map = {'general': 0, 'reasoning': 1, 'generation': 2, 'debugging': 3, 'optimization': 4}
    ...
    def _prepare_training_data(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        """
        X = np.array([self._features_to_array(features).flatten() for features, _ in self.training_data])
    ...
    def _train_ensemble_models(
        ...
        best_model_name = max(
            ...
            key=lambda k: self.model_performance[k][-1]
        )
        if hasattr(self.models[best_model_name], 'feature_importances_'):
            self.feature_importance = dict(zip(
                ['prompt_length', 'prompt_complexity', 'variant_version',
                 'historical_success_rate', 'historical_avg_time', 'historical_error_rate',
                 'context_domain', 'context_task_type', 'context_urgency',
                 'time_of_day', 'day_of_week', 'recent_performance_trend',
                 'resource_availability', 'user_experience_level', 'system_load'],
    ...
    def train(self, training_data: List[Tuple[PredictionFeatures, float]]) -> None:
        ...
        X_train_scaled, X_test_scaled, y_train, y_test = self._split_and_scale_data(X, y)
    ...
    def _default_prediction(self, variant: PromptVariant, context: Dict[str, Any]) -> PerformancePrediction:
    ...
    def _generate_recommendations(
        ...
        if features.context_urgency == 'critical' and predicted_score < 0.8:
            recommendations.append("Critical task with low predicted performance - use proven variant")
        
        if features.resource_availability < 0.5:
            recommendations.append("Low resource availability - consider simpler approach")
    ...
    def add_training_data(self, features: PredictionFeatures, actual_score: float) -> None:
