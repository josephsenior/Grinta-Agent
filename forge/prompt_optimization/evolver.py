...
from .models import PromptCategory, PromptVariant, PromptPerformance, PromptMetrics
...
class PromptEvolver:  # pragma: no cover - integration-heavy component
    ...
    def evolve_prompt(
        self, prompt_id: str, max_variants: int = 3
    ) -> list[str]:  # pragma: no cover - exercised via integration tests
        ...
        for candidate in candidates[:max_variants]:
            ...
            for variant_content in improved_variants:
                variant_id = self.optimizer.add_variant(
                    ...
                    metadata={
                        "evolution_strategy": analysis.get(
    ...
    def _analyze_prompt_performance(
    ...
    ) -> dict[str, Any]:  # pragma: no cover - depends on tracker history
        ...
        recent_performances = self.tracker.get_performances_for_variant(
            variant.id, limit=10
        )
    ...
    def _categorize_errors(
        self, failed_performances: Sequence[PromptPerformance]
    ...
    def _analyze_error_patterns(
        self, performances: Sequence[PromptPerformance]
    ) -> dict[str, Any]:  # pragma: no cover - analytics helper
    ...
    def _recommend_strategy(
        self, metrics: PromptMetrics, error_patterns: dict[str, Any]
    ) -> str:  # pragma: no cover - heuristic scoring
    ...
    def _generate_improved_variants(
        self, variant: PromptVariant, analysis: dict[str, Any]
    ) -> list[str]:  # pragma: no cover - LLM interaction
        ...
        try:
            ...
            response = self.llm.generate(
                prompt=evolution_prompt, max_tokens=2000, temperature=0.7
            )
...
"""
    ...
    def _fallback_evolution(
        self, variant: PromptVariant, strategy: str
    ) -> list[str]:  # pragma: no cover - backup path
    ...
    def evolve_all_underperforming(
        self, min_score: float = 0.7
    ) -> dict[str, list[str]]:  # pragma: no cover - integration behaviour
    ...
    def get_evolution_history(
        self, prompt_id: str
    ) -> list[dict[str, Any]]:  # pragma: no cover - reporting helper
        ...
        evolved_variants = [
            v for v in variants if v.metadata.get("generated_at") == "evolver"
        ]
    ...
    def _count_strategies(
        self, evolved_variants: Sequence[PromptVariant]
    ) -> dict[str, int]:  # pragma: no cover - reporting helper
    ...
    def _calculate_strategy_success_rate(
        self, strategy: str, evolved_variants: Sequence[PromptVariant]
    ) -> float:  # pragma: no cover - reporting helper
    ...
    def _calculate_strategy_success_rates(
        self, strategy_counts: dict[str, int], evolved_variants: Sequence[PromptVariant]
    ) -> dict[str, float]:  # pragma: no cover - reporting helper
    ...
    def get_evolution_statistics(self) -> dict[str, Any]:
        ...
        evolved_variants = [
            v for v in all_variants if v.metadata.get("generated_at") == "evolver"
        ]

        strategy_counts = self._count_strategies(evolved_variants)
        strategy_success_rates = self._calculate_strategy_success_rates(
            strategy_counts, evolved_variants
        )
        ...
        return {
            ...
            "evolution_rate": len(evolved_variants) / len(all_variants)
            if all_variants
            else 0,
        }
