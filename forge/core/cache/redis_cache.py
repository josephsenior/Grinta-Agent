...
class DistributedCache:
    ...
    def __init__(
        ...
        if not REDIS_AVAILABLE:
            logger.warning(f"Redis not available, using local fallback cache for {prefix}")
            return
        
        # Get connection details from environment
        host = host or os.getenv("REDIS_HOST")
        if not host:
            logger.warning(f"REDIS_HOST not set, using local fallback cache for {prefix}")
            self.enabled = False
            return
        ...
        try:
            ...
            # Test connection
            self.client.ping()
            logger.info(f"✅ Redis distributed cache initialized for {prefix} (ttl={ttl_seconds}s)")
            logger.info(f"   Connected to {host}:{port}, max_connections={max_connections}")
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if self.enabled and self.client:
            try:
                ...
                stats["redis_used_memory_mb"] = memory_info.get("used_memory", 0) / (1024 * 1024)
                stats["redis_max_memory_mb"] = memory_info.get("maxmemory", 0) / (1024 * 1024)
            except Exception as e:
