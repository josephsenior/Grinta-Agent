...
class SandboxConfig(BaseModel):
    ...
    Attributes:
        ...
        trusted_dirs: List of directories that can be trusted to run the Forge CLI.
        vscode_port: The port to use for VSCode. If None, a random port will be chosen.
            This is useful when deploying Forge in a remote machine where you need to expose a specific port.
    ...
    logger.debug("SandboxConfig user_id default: %s", user_id)
    ...
    remote_runtime_class: str | None = Field(default=None)
    ...
    runtime_binding_address: str = Field(
        default="0.0.0.0",
    )  # nosec B104 - Safe: configurable binding address for runtime
    ...
    def set_default_base_image(self) -> SandboxConfig:
        """Set default base container image if not specified.
