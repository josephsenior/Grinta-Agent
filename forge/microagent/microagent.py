"""Utilities and models for loading and instantiating Forge microagents."""
...
from forge.microagent.types import InputMetadata, MicroagentMetadata, MicroagentType
...
def _finalize_loaded_microagent(metadata_dict, path):
    ...
    try:
        ...
        raise MicroagentValidationError(str(exc)) from exc
...
class BaseMicroagent(BaseModel):
    ...
    def load(
        ...
        if path.name == ".FORGE_instructions":
            return RepoMicroagent(
                name="repo_legacy",
                content=file_content,
...
class KnowledgeMicroagent(BaseMicroagent):
    ...
    def triggers(self) -> list[str]:
        """Return list of trigger strings associated with this microagent."""
...
class RepoMicroagent(BaseMicroagent):
    ...
    RepoMicroagents are loaded from `.Forge/microagents/repo.md` files within repositories
    ...
    def __init__(self, **data) -> None:
        """Ensure repository microagents are instantiated with repo knowledge type."""
        super().__init__(**data)
...
def load_microagents_from_dir(
    ...
    Args:
        microagent_dir: Path to the microagents directory (e.g. .Forge/microagents)
