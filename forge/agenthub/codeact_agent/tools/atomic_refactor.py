...
class AtomicRefactor:
    ...
    def add_file_edit(
        ...
        logger.debug(f"Added {operation} edit for {file_path} to transaction {transaction.transaction_id}")
    ...
    def add_rename(
        ...
        transaction.edits.append(edit)
        logger.debug(f"Added rename {old_path} → {new_path} to transaction {transaction.transaction_id}")
    
    def _check_transaction_state(self, transaction: RefactorTransaction) -> Optional[RefactorResult]:
        ...
        for edit in transaction.edits:
            if edit.original_content and transaction.backup_dir:
                backup_path = os.path.join(
                    ...
                    os.path.basename(edit.file_path)
                )
                try:
    ...
    def _apply_modify_or_create(self, edit: RefactorEdit, validate: bool, validator: Optional[Callable]) -> None:
    ...
    def _apply_all_edits(
        ...
        for i, edit in enumerate(transaction.edits):
            try:
                ...
                # Rollback on failure
                logger.warning(f"⚠️  Rolling back {len(applied_edits)} edits due to failure")
