...
class WhitespaceHandler:
    ...
    def _determine_indent_style(
        ...
        if space_count > 0:
            size = (
                WhitespaceHandler._find_indent_size(space_sizes) if space_sizes
                else WhitespaceHandler.DEFAULT_INDENT_SIZES.get(language or '', 4)
            )
        ...
        return IndentStyle.SPACES, WhitespaceHandler.DEFAULT_INDENT_SIZES.get(language or '', 4)
    ...
    def normalize_indent(
        ...
        lines = code.split('\n')
        normalized_lines = [
            WhitespaceHandler._normalize_line_indent(line, current_config, target_config)
            for line in lines
        ]
