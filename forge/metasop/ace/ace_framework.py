...
class ACEFramework:
    ...
    def __init__(
        self,
        llm: LLM,
        context_playbook: Optional[ContextPlaybook] = None,
    ...
    def _run_reflection_phase(
        self, generation_result, execution_result, ground_truth, task_type, role
    ):
    ...
    def _run_curation_phase(
        self, reflection_result, query, task_type, role, expected_outcome
    ):
    ...
    def _update_metrics(
        ...
        if self.context_playbook.bullets:
            self.performance_metrics.avg_helpfulness = sum(
                bullet.helpfulness_score
                for bullet in self.context_playbook.bullets.values()
    ...
    def process_task(
        ...
        try:
            ...
            if (
                curation_result
                and curation_result.success
                and curation_result.delta_updates
            ):
                self._apply_delta_updates(curation_result.delta_updates)
                self.performance_metrics.context_updates += len(
                    curation_result.delta_updates
                )
            ...
            logger.info(
                f"ACE processing completed in {processing_time:.2f}s - Success: {overall_success}"
            )
    ...
    def process_task_with_feedback(
        self,
        query: str,
        previous_result: ACEFrameworkResult,
        task_type: str = "general",
        role: Optional[str] = None,
    ) -> ACEFrameworkResult:
    ...
    def multi_epoch_training(
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for i, (query, role, ground_truth) in enumerate(
                zip(queries, roles, ground_truths)
            ):
                ...
                if (i + 1) % 10 == 0:
                    logger.info(
                        f"Epoch {epoch + 1}: Processed {i + 1}/{len(queries)} queries"
                    )
    ...
    def _simulate_execution(
        self, trajectory: ACETrajectory, query: str, task_type: str
    ) -> ACEExecutionResult:
        ...
        success = (
            len(trajectory.content) > 50 and "error" not in trajectory.content.lower()
        )
            ...
            try:
                if update.type == "ADD":
                    self.context_playbook.add_bullet(
                        content=update.content, section=update.section, tags=update.tags
                    )
