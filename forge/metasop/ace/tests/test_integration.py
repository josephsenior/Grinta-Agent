...
class TestACEIntegration:
    ...
    def ace_framework(self, mock_llm):
        ...
        def _reflect(generation_result, execution_result, ground_truth, task_type, role):
        ...
        framework._run_generation_phase = Mock(side_effect=_generate)
        framework._run_reflection_phase = Mock(side_effect=_reflect)
        framework._run_curation_phase = Mock(side_effect=_curate)
        framework.generator.get_metrics = Mock(return_value={"total_generations": 0})
        framework.reflector.get_metrics = Mock(return_value={"total_reflections": 0})
        framework.curator.get_metrics = Mock(return_value={"total_curations": 0})
