...
from forge.metasop.ace.context_playbook import BulletSection
...
class TestACEIntegration:
    ...
    def ace_framework(self, mock_llm):
        ...
        framework = ACEFramework(llm=mock_llm, context_playbook=playbook, config=config)

        def _generate(query: str, task_type: str = "general", role=None):
            if task_type == "invalid_type":
                raise ValueError("Unsupported task type")
            content = f"Generated plan for {task_type}: {query}." + " details" * 20
            trajectory = ACETrajectory(
                content=content,
                task_type=task_type,
                used_bullet_ids=[],
                playbook_content=f"Playbook content for {task_type}",
                generation_metadata={"role": role},
            )
        ...
        insight = ACEInsight(
            reasoning="Thorough reasoning",
            error_identification="None",
            root_cause_analysis="N/A",
            correct_approach="Follow standard procedure",
        ...
        def _curate(reflection_result, query, task_type, role, expected_outcome):
        ...
        setattr(framework, "_run_generation_phase", Mock(side_effect=_generate))
        setattr(framework, "_run_reflection_phase", Mock(side_effect=_reflect))
        setattr(framework, "_run_curation_phase", Mock(side_effect=_curate))
        setattr(
            framework.generator,
            "get_metrics",
            Mock(return_value={"total_generations": 0}),
        )
        setattr(
            framework.reflector,
            "get_metrics",
            Mock(return_value={"total_reflections": 0}),
        )
        setattr(
            framework.curator,
            "get_metrics",
            Mock(return_value={"total_curations": 0}),
        )
    ...
    def test_ace_with_codeact_simulation(self, ace_framework):
        ...
        # Process through ACE framework
        result = ace_framework.process_task(
            query=codeact_task, task_type="code_generation"
        )
    ...
    def test_playbook_growth_over_multiple_tasks(self, ace_framework):
        ...
        for task in tasks:
            result = ace_framework.process_task(
                query=task, task_type="metasop", role="engineer"
            )
    ...
    def test_ace_error_handling(self, ace_framework):
        ...
        result = ace_framework.process_task(query="Test task", task_type="invalid_type")
    ...
    def test_ace_configuration_validation(self):
        ...
        valid_config = ACEConfig(
            enable_ace=True, max_bullets=1000, num_epochs=5, reflector_max_iterations=3
        )
