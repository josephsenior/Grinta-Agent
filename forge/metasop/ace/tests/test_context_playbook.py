...
from forge.metasop.ace.context_playbook import (
    ContextPlaybook,
    BulletPoint,
    BulletSection,
)
...
class TestContextPlaybook:
    ...
    def test_update_bullet(self):
        ...
        bullet_id = playbook.add_bullet(
            content="Test strategy", section=BulletSection.STRATEGIES_AND_HARD_RULES
        )
        assert bullet_id is not None
    ...
    def test_get_relevant_bullets(self):
        ...
        bullet2 = playbook.add_bullet(
            content="JavaScript debugging tips", section=BulletSection.DEBUGGING_TIPS
        )
    ...
    def test_get_playbook_content(self):
        ...
        # Add some bullets
        playbook.add_bullet(
            content="Test strategy 1", section=BulletSection.STRATEGIES_AND_HARD_RULES
        )
        playbook.add_bullet(
            content="Test strategy 2", section=BulletSection.STRATEGIES_AND_HARD_RULES
        )
    ...
    def test_cleanup_old_bullets(self):
        ...
        # Add a bullet
        bullet_id = playbook.add_bullet(
            content="Old strategy", section=BulletSection.STRATEGIES_AND_HARD_RULES
        )
        assert bullet_id is not None
    ...
    def test_grow_and_refine(self):
        ...
        for i in range(3):
            playbook.add_bullet(
                content=f"Strategy {i}", section=BulletSection.STRATEGIES_AND_HARD_RULES
            )

        # Add one more (should trigger grow-and-refine)
        playbook.add_bullet(
            content="Strategy 3", section=BulletSection.STRATEGIES_AND_HARD_RULES
        )
    ...
    def _setup_test_playbook(self) -> tuple[ContextPlaybook, str, str]:
        """Setup test playbook with sample bullets.

        Returns:
            Tuple of (playbook, bullet1_id, bullet2_id)
        ...
        assert bullet1 is not None
        assert bullet2 is not None
        bullet1_id: str = bullet1
        bullet2_id: str = bullet2

        playbook.update_bullet(bullet1_id, helpful=True)
        playbook.update_bullet(bullet2_id, harmful=True)

        return playbook, bullet1_id, bullet2_id

    def _verify_export_structure(
        self, exported: dict, bullet1: str, bullet2: str
    ) -> None:
        """Verify exported playbook structure.

        Args:
            exported: Exported playbook data
            bullet1: First bullet ID
            bullet2: Second bullet ID
    ...
    def _verify_import_result(
        self, new_playbook: ContextPlaybook, bullet1: str, bullet2: str
    ) -> None:
        """Verify imported playbook data.

        Args:
            new_playbook: Imported playbook
            bullet1: First bullet ID
            bullet2: Second bullet ID
    ...
    def test_export_import_playbook(self):
        ...
        playbook, bullet1, bullet2 = self._setup_test_playbook()

        exported = playbook.export_playbook()
        self._verify_export_structure(exported, bullet1, bullet2)

        new_playbook = ContextPlaybook()
        new_playbook.import_playbook(exported)
        self._verify_import_result(new_playbook, bullet1, bullet2)
    ...
    def test_redundancy_detection(self):
        ...
        assert bullet2 is None or isinstance(bullet2, str)
    ...
    def test_statistics(self):
        ...
        # Add some bullets
        bullet1 = playbook.add_bullet(
            content="Strategy 1", section=BulletSection.STRATEGIES_AND_HARD_RULES
        )
        assert bullet1 is not None
        bullet2 = playbook.add_bullet(
            content="Strategy 2", section=BulletSection.DEBUGGING_TIPS
        )
        assert bullet2 is not None
