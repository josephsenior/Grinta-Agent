...
def _print_helpfulness_stats(helpfulness_scores: list) -> None:
    """Print helpfulness score statistics."""
...
def _print_quality_distribution(helpfulness_scores: list) -> None:
    """Print quality distribution of bullets."""
...
def _print_usage_stats(playbook: ContextPlaybook, usage_counts: list) -> None:
...
def analyze_bullet_quality(playbook: ContextPlaybook):
    ...
    helpfulness_scores = [
        bullet.helpfulness_score for bullet in playbook.bullets.values()
    ]

    if helpfulness_scores:
        _print_helpfulness_stats(helpfulness_scores)
        _print_quality_distribution(helpfulness_scores)

    usage_counts = [bullet.usage_count for bullet in playbook.bullets.values()]
    if usage_counts:
        _print_usage_stats(playbook, usage_counts)


def show_top_strategies(
    playbook: ContextPlaybook, section: Optional[BulletSection] = None, limit: int = 10
):
    ...
    for i, bullet in enumerate(sorted_bullets[:limit], 1):
        ...
        print(
            f"    Usage: {bullet.usage_count}, Helpful: {bullet.helpful_count}, Harmful: {bullet.harmful_count}"
        )
...
def show_recent_insights(playbook: ContextPlaybook, days: int = 7, limit: int = 10):
    ...
    for i, bullet in enumerate(recent_bullets[:limit], 1):
        print(
            f"{i:2d}. [{bullet.id}] Created: {bullet.created_at.strftime('%Y-%m-%d %H:%M')}"
        )
        print(f"    Section: {bullet.section.value}")
        print(f"    Content: {bullet.content[:150]}...")
        print()
...
def main():
    ...
    if playbook is None:
        ...
        sample_playbook.update_bullet(
            list(sample_playbook.bullets.keys())[0], helpful=True
        )
        sample_playbook.update_bullet(
            list(sample_playbook.bullets.keys())[1], helpful=True
        )
