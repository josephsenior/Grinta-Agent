...
class ACEInsight(BaseModel):
    ...
    bullet_tags: List[Dict[str, str]] = Field(
        default_factory=list, description="Tags for bullets used (id, tag)"
    )
...
class ACEDeltaUpdate(BaseModel):
    ...
    section: Optional[BulletSection] = Field(
        default=None, description="Section for the update"
    )
    ...
    helpful: Optional[bool] = Field(
        default=None, description="Whether the bullet was helpful"
    )
    harmful: Optional[bool] = Field(
        default=None, description="Whether the bullet was harmful"
    )
...
class ACETrajectory(BaseModel):
    ...
    task_type: str = Field(
        description="Type of task (appworld, code_generation, general)"
    )
...
class ACEPerformanceMetrics(BaseModel):
    ...
    adaptation_latency: float = Field(
        default=0.0, description="Average adaptation latency"
    )
...
class ACECurationResult(BaseModel):
    ...
    redundancy_removed: int = Field(
        default=0, description="Number of redundant items removed"
    )
...
class ACEConfig(BaseModel):
    ...
    reflector_max_iterations: int = Field(
        default=5, description="Max reflection iterations"
    )
    enable_online_adaptation: bool = Field(
        default=True, description="Enable test-time learning"
    )
    playbook_persistence_path: Optional[str] = Field(
        default=None, description="Path for playbook persistence"
    )
    min_helpfulness_threshold: float = Field(
        default=0.0, description="Minimum helpfulness for retrieval"
    )
    max_playbook_content_length: int = Field(
        default=50, description="Max bullets in playbook content"
    )
    enable_grow_and_refine: bool = Field(
        default=True, description="Enable grow-and-refine mechanism"
    )
    cleanup_interval_days: int = Field(
        default=30, description="Days between cleanup cycles"
    )
    redundancy_threshold: float = Field(
        default=0.8, description="Similarity threshold for redundancy"
    )
