...
from typing import TYPE_CHECKING, Awaitable, Callable, Dict, List, Optional, Union, Any
...
class MetaSOPOrchestrator:
    ...
    def __init__(self, sop_name: str, config: ForgeConfig | None = None) -> None:
        ...
        self.llm_registry: Optional["LLMRegistry"] = None  # Will be set by router if available
        self.step_event_callback: Optional[Callable[[str, str, str, int], None]] = None  # Callback for real-time step events
        self.ace_framework: Optional["ACEFramework"] = None  # ACE framework for self-improving agents
        self.prompt_optimizer: Optional[dict[str, Any]] = None  # Dynamic prompt optimization system
        self.causal_engine: Optional["CausalReasoningEngine"] = None  # Causal reasoning engine for conflict prediction
        self.parallel_engine: Optional["ParallelExecutionEngine"] = None  # Parallel execution engine for intelligent step scheduling
        self.predictive_planner: Optional["PredictiveExecutionPlanner"] = None  # Predictive execution planner for optimization
        self.collaborative_streaming: Optional["ContextAwareStreamingEngine"] = None  # Context-aware collaborative streaming engine
        
        # Learning and feedback tracking
        self.active_steps: Dict[str, SopStep] = {}  # Track currently executing steps
        self.learning_storage: Optional["LearningStorage"] = None  # Persistent learning storage
    ...
    def _validate_micro_iteration_settings(self) -> None:
        ...
        """
        try:
            candidate_count = getattr(self.settings, "micro_iteration_candidate_count", None)
    ...
    def _initialize_basic_attributes(self, sop_name: str) -> None:
        """Initialize basic orchestrator attributes and state.

        Sets up core orchestrator instance variables including event tracking,
        trace history, execution context, and SOP template/profile loading.
        This creates the foundational state for step orchestration.

        Args:
            sop_name: Name of the SOP to load template and profiles for

        Side Effects:
            - Creates EventEmitter instance (self._emitter)
            - Initializes empty step_events list for tracking
            - Initializes empty traces list for execution history
            - Sets context and logger to None (populated during run())
            - Loads SOP template and role profiles
            - Initializes provenance tracking (_previous_step_hash)

        Example:
            >>> orch = MetaSOPOrchestrator("code_generation")
            >>> orch._ctx is None  # Set during run()
            True
            >>> len(orch.step_events)
            0
        ...
        self._logger: Logger | LoggerAdapter = logging.getLogger("forge")

        # Template and profiles
        self._load_template_and_profiles(sop_name)

        # Provenance tracking
        self._previous_step_hash: Optional[str] = None

    def _load_template_and_profiles(self, sop_name: str) -> None:
        """Load SOP template and role profiles with error handling.

        Attempts to load the SOP template for the given SOP name and the
        global role profiles. Failures in loading are gracefully handled:
        template failures result in None (fallback to runtime SOP), while
        profile failures still continue (profiles are optional).

        Args:
            sop_name: Name of the SOP to load (e.g., "code_generation", "planning")

        Side Effects:
            - Sets self.template to loaded SopTemplate or None
            - Sets self.profiles to loaded role profiles dict

        Raises:
            None - All exceptions are caught and logged

        Example:
            >>> orch._load_template_and_profiles("basic_sop")
            >>> isinstance(orch.template, SopTemplate) or orch.template is None
            True
    ...
    def _initialize_execution_components(self) -> None:
        ...
        Creates the default step and QA executors, and optionally wraps them
        with timeout wrappers if configured. Also initializes the failure
        classifier for result analysis.

        Timeout Configuration:
            - If step_timeout_seconds is set, DefaultStepExecutor is wrapped
            - If qa_timeout_seconds is set, DefaultQAExecutor is wrapped
            - Uses stuck_callback and stuck_threshold for deadlock detection

        Side Effects:
            - Sets self.step_executor (DefaultStepExecutor or TimeoutStepExecutor)
            - Sets self.qa_executor (DefaultQAExecutor or TimeoutQAExecutor)
            - Sets self.failure_classifier (DefaultFailureClassifier)

        Example:
            >>> orch.step_executor.__class__.__name__
            "DefaultStepExecutor"  # Or "TimeoutStepExecutor" if configured
    ...
    def _initialize_step_cache(self) -> None:
        ...
        self.step_cache: StepCache | None = None
        try:
    ...
    def _initialize_metrics_server(self) -> None:
        ...
        try:
            port = getattr(self.settings, "metrics_prometheus_port", None)
            if port is not None and not self._running_under_pytest():
    ...
    def _initialize_ace_framework(self) -> None:
        ...
        try:
            ...
            if llm is None:
                logger.warning("ACE framework enabled but no default LLM available; skipping initialization")
                self.ace_framework = None
                return
            ...
            persistence_path = self.settings.ace_playbook_persistence_path
            if persistence_path:
                try:
                    ...
                    logger.warning(f"Failed to load existing playbook from %s: %s", persistence_path, exc)
    ...
    def _initialize_learning_storage(self) -> None:
        ...
        try:
            ...
            storage_path = getattr(self.settings, "learning_persistence_path", "~/.Forge/learning/")
    ...
    def _reflect_and_update_ace(self, step: SopStep, result: StepResult, 
        ...
        try:
            ...
            trajectory = ACETrajectory(
                content=self._stringify_content(result.artifact.content) if result.artifact else "",
                task_type="metasop",
            ...
            execution_result = ACEExecutionResult(
                success=result.ok,
                output=self._stringify_content(result.artifact.content) if result.artifact else "",
    ...
    def _save_ace_playbook(self) -> None:
        ...
        try:
            path = getattr(self.settings, "ace_playbook_persistence_path", None)
            if path:
                self.ace_framework.save_playbook(path)
        except Exception as e:
    ...
    def _call_step_event_callback(self, event: dict[str, Any]) -> None:
    ...
    def _collect_causal_feedback(
        ...
        """
        if not (self.causal_engine and getattr(self.settings, "enable_learning", False)):
            return
        ...
        self._save_causal_patterns()

    def _save_causal_patterns(self) -> None:
        """Save causal patterns to storage."""
        if not self.learning_storage or not self.causal_engine:
            return
    ...
    def _collect_execution_feedback(
        ...
        enable_learning = getattr(self.settings, "enable_learning", False)

        try:
            self._collect_causal_feedback(step, success, artifacts, active_steps_at_time)
            ...
            if self.predictive_planner and getattr(self.settings, "predictive_learn_from_execution", False):
                try:
                    actual_duration_ms = 1000.0  # Placeholder until real timing is wired
                    self.predictive_planner.learn_from_execution(
                        step_id=step.id,
                        actual_duration_ms=actual_duration_ms,
                        success=success,
                    )
                    logger.debug("Updated predictive models for step %s", step.id)
                except Exception as planner_exc:  # pragma: no cover - defensive
                    logger.warning("Failed to update predictive planner: %s", planner_exc)

            if learning_storage and enable_learning:
                parallel_engine = self.parallel_engine
                if parallel_engine:
                    with contextlib.suppress(Exception):
                        learning_storage.save_parallel_stats(parallel_engine.get_execution_stats())
    ...
    def _prepare_event_for_emission(self, event: dict[str, Any]) -> None:
    ...
    def _add_source_metadata(self, event: dict[str, Any]) -> None:
        """Add source metadata to event."""
        if "source" not in event:
            event["source"] = "orchestrator"

    def _add_trace_id_metadata(self, event: dict[str, Any]) -> None:
        """Add trace_id metadata to event if available."""
        ctx = self._ctx
        if ctx and isinstance(ctx.extra, dict):
            tid = ctx.extra.get("trace_id")
            if tid and "trace_id" not in event:
                event["trace_id"] = tid

    def _emit_prepared_event(self, event: dict[str, Any]) -> None:
    ...
    def _get_last_emitted_event(self) -> dict[str, Any] | None:
    ...
    def _append_to_step_events(self, event_dict: dict[str, Any]) -> None:
    ...
    def _refresh_step_events(self) -> None:
        ...
        with contextlib.suppress(AttributeError, TypeError):
            self.step_events = list(self._emitter.events)
    ...
    def _deps_satisfied(self, done: dict[str, Artifact], step: SopStep) -> bool:
        ...
        Args:
            done: Dictionary mapping step IDs to completed artifacts
            step: Step to check dependencies for

        Returns:
            bool: True if all steps in step.depends_on are present in done dict

        Side Effects:
            - Logs dependency check result at INFO level
            - Tracks which steps have completed vs which are pending

        Notes:
            - Used during orchestration planning to determine step executability
            - Empty depends_on list returns True (step has no dependencies)
            - Enables topological sorting of execution DAG

        Example:
            >>> done = {"step_1": artifact1, "step_2": artifact2}
            >>> step = SopStep(id="step_3", depends_on=["step_1", "step_2"])
            >>> orch._deps_satisfied(done, step)
            True
    ...
    def _setup_logging_and_tracing(self, ctx: OrchestrationContext) -> None:
        ...
        try:
            ...
            try:
                ...
                from forge.core.logger import set_trace_context
    ...
    async def _process_orchestration_steps_async(
        ...
        try:
            ...
            if parallel_engine and hasattr(parallel_engine, 'execute_parallel_groups_async'):
                # Use optimized steps from predictive planner or original steps
                parallel_groups = parallel_engine.identify_parallel_groups(
    ...
    def _check_causal_safety(self, step: SopStep, done: dict[str, Artifact]) -> bool:
        ...
        try:
            can_proceed, predictions = self._run_causal_analysis(step, done)
            
            if not can_proceed:
                self._handle_blocking_predictions(step, predictions)
                return False
            
            self._handle_warning_predictions(step, predictions)
        except Exception as e:
    ...
    def _get_skipped_steps(self) -> list[str]:
        ...
        return [
            step_id
            for step_id in (event.get("step_id") for event in skipped)
    ...
    def _get_vector_hits(self, store: VectorOrLexicalMemoryStore, query: str, stats: dict) -> list:
    ...
    def _get_lexical_hits(self, store: VectorOrLexicalMemoryStore, query: str, stats: dict) -> list:
    ...
    def _perform_selective_tests(self, step: SopStep, ctx: OrchestrationContext) -> tuple[list[str] | None, str | None]:
        ...
        try:
            ...
            changed_paths: list[str] = []  # Placeholder for future git diff integration
    ...
    def _execute_orchestration_steps(self, ctx, retry_policy, repo_root):
        ...
        for _i, step in enumerate(template.steps):
            step_success, step_artifacts = self._process_single_step(
                step,
                ctx,
                done,
    ...
    def _setup_logger(self, trace_id) -> None:
        ...
        try:
            from forge.core.logger import bind_context, FORGE_logger
