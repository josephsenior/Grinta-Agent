...
class EnhancedMetaSOPEventEmitter:
    ...
    def emit_step_complete(
        ...
        validation_passed = self._validate_artifact_if_enabled(
            artifact, role, step_id, metadata
        )

        if self._maybe_emit_strict_validation_failure(
            validation_passed, step_id, role, artifact, metadata
        ):
            return

        if validation_passed:
            self._log_validation_success(role, step_id, metadata)

        self._emit_step_complete_event(
            step_id, role, artifact, retries, metadata, validation_passed
        )

    def _maybe_emit_strict_validation_failure(
        self,
        ...
        if validation_passed or not self.strict_validation:
            return False
        ...
        self._emit_validation_failed(step_id, role, artifact, errors, warnings, suggestions)
        return True

    def _log_validation_success(
        self, role: str, step_id: str, metadata: Dict[str, Any]
    ) -> None:
    ...
    def emit_artifact_produced(
        ...
        if self.enable_validation:
            is_valid, errors, warnings, suggestions = (
                validate_artifact_with_suggestions(artifact, role)
            )
...
def example_usage():
    ...
    def my_emit_callback(event_data):
        ...
        if event_data.get("metadata", {}).get("validation"):
            ...
            if validation["is_valid"]:
                ...
                print(
                    f"  ❌ Artifact validation failed: {len(validation['errors'])} errors"
                )
    ...
    emitter.emit_step_complete(
        step_id="step_001", role="Product Manager", artifact=pm_artifact, retries=0
    )

    # Example of invalid artifact (missing required field)
    invalid_artifact: dict[str, list[Any]] = {
        "user_stories": []  # Empty array violates minItems: 1
    }

    emitter.emit_step_complete(
        step_id="step_002", role="Product Manager", artifact=invalid_artifact, retries=0
    )
