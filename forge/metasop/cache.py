...
class StepCacheEntry:
    ...
    def to_json(self) -> dict[str, Any]:
        """Convert cache entry to JSON-serializable dictionary."""
        return asdict(self)
...
class StepCache:
    """Persistent cache storing step execution outputs keyed by fingerprints."""
    ...
    def __init__(
        ...
        """Initialize the step cache with configuration and warm-load from disk.

        Args:
            max_entries: Maximum number of cache entries (LRU eviction applied). Defaults to 256.
            cache_dir: Directory path for cache persistence. If None, cache is in-memory only.
            ttl_seconds: Time-to-live for cache entries in seconds. Expired entries are removed on access.
            min_tokens_threshold: Minimum tokens required for entry eligibility (filters low-value caches).
            exclude_roles: List of role names to exclude from caching (case-insensitive).
        ...
        Side Effects:
            ...
            - Creates cache_dir if provided and cache persistence is enabled
            - Performs warm-load from disk cache if cache_dir exists with persisted index

        Example:
            >>> cache = StepCache(
            ...     max_entries=512,
            ...     cache_dir="/tmp/step_cache",
            ...     ttl_seconds=3600,
            ...     exclude_roles=["debug", "test"]
            ... )
            >>> cache.get("ctx_hash", "production")
            StepCacheEntry(...)  # or None if miss
    ...
    def _is_entry_eligible(self, entry: StepCacheEntry) -> bool:
        ...
        Validates whether a cache entry should be stored based on configuration:
        - Caching must be enabled (max_entries > 0)
        - Entry's role must not be in exclusion list
        - Entry must meet minimum token threshold if configured

        Args:
            entry: The cache entry to evaluate for eligibility

        Returns:
            bool: True if entry should be cached, False if excluded

        Example:
            >>> cache = StepCache(max_entries=100, exclude_roles=["debug"])
            >>> eligible_entry = StepCacheEntry(role="production", total_tokens=500)
            >>> cache._is_entry_eligible(eligible_entry)
            True
    ...
    def _store_entry(self, entry: StepCacheEntry) -> list[tuple[str, StepCacheEntry]]:
        ...
        Adds a new entry to the in-memory cache using LRU ordering. If cache size
        exceeds max_entries, automatically evicts oldest entries to maintain size limits.

        Args:
            entry: The cache entry to store

        Returns:
            list[tuple[str, StepCacheEntry]]: List of evicted entries (if any) in (key, entry) format

        Example:
            >>> new_entry = StepCacheEntry(context_hash="abc123", ...)
            >>> evicted = cache._store_entry(new_entry)
            >>> len(evicted)
            1  # One entry was evicted to maintain max_entries limit
    ...
    def _evict_excess_entries(self) -> list[tuple[str, StepCacheEntry]]:
        ...
        Removes entries using LRU (Least Recently Used) strategy when cache size
        exceeds max_entries. Pops from the beginning of the OrderedDict to evict
        oldest entries first.

        Returns:
            list[tuple[str, StepCacheEntry]]: List of evicted entries in (key, entry) format

        Example:
            >>> cache._store  # Already at max size
            >>> evicted = cache._evict_excess_entries()
            >>> len(evicted)
            1  # One entry removed
            >>> cache._evictions
            1  # Eviction counter incremented
    ...
    def _persist_cache_operations(
        ...
        Writes new cache entry and removes evicted entries from disk storage, then
        updates the cache index file. Errors in persistence are silently ignored to
        prevent cache operations from failing.

        Args:
            entry: The cache entry to persist
            evicted_entries: List of entries that were evicted and should be removed from disk

        Returns:
            None

        Raises:
            No exceptions raised; all errors are silently caught and ignored.

        Example:
            >>> entry = StepCacheEntry(...)
            >>> evicted = [(key, old_entry)]
            >>> cache._persist_cache_operations(entry, evicted)
            >>> # Disk now reflects new state
    ...
    def _persist_entry(self, entry: StepCacheEntry) -> None:
        """Persist a single cache entry to disk in JSON format.
        ...
        Side Effects:
            Writes entry to <cache_dir>/<context_hash>.json. If write fails, error is silently ignored
            to prevent cache operations from failing due to I/O issues.

        Notes:
            - Ensures deterministic JSON output with sorted keys for reproducibility
            - Uses UTF-8 encoding with minimal separators for compact storage
            - Non-fatal: exceptions are caught and suppressed
    ...
    def _persist_index(self) -> None:
        """Persist cache index file to maintain insertion order across restarts.
        ...
        Side Effects:
            Writes <cache_dir>/index.json with ordered list of context_hash keys. This allows
            warm-loading cache entries in LRU order on restart.

        Notes:
            - Stores only the key order, not entry data (entries are stored separately)
            - Index updates are best-effort; I/O failures are silently ignored
            - Maintains compact JSON format for fast deserialization
    ...
    def _load_cache_entries(self, order: list[str]) -> None:
        """Load cache entries from disk in specified order.

        Populates the in-memory cache with entries from disk, respecting the order
        list and stopping once max_entries is reached. Handles missing or corrupted
        entries gracefully.

        Args:
            order: List of cache keys to load in priority order

        Returns:
            None

        Example:
            >>> order = ["hash1", "hash2", "hash3"]
            >>> cache._load_cache_entries(order)
            >>> len(cache._store)
            3  # Entries loaded from disk
