...
class CausalReasoningEngine:
    ...
    def _should_skip_analysis(self, proposed_step: SopStep, active_steps: List[SopStep]) -> bool:
        ...
        return (
            proposed_step.lock is None and 
    ...
    def _run_heuristic_checks(
        ...
        if conflict := self._check_artifact_conflicts(proposed_step, active_steps, completed_artifacts):
            predictions.append(conflict)
    ...
    def analyze_step_safety(
        ...
        try:
            ...
            predictions.extend(
                self._run_heuristic_checks(proposed_step, active_steps, completed_artifacts)
            )
...
Only include conflicts with confidence >= 0.6. Be specific about WHY conflicts might occur."""
    ...
    def _learn_conflict_patterns(
        ...
        conflicting_roles = self._extract_conflicting_roles(conflicts_observed, active_steps_at_time)
    ...
    def learn_from_execution(
        ...
        if conflicts_observed:
            self._learn_conflict_patterns(step, conflicts_observed, active_steps_at_time)
        
        # Learn success patterns
        self._learn_success_patterns(step, success, conflicts_observed, active_steps_at_time)
    ...
    def _check_pm_modification_conflict(self, step: SopStep, active_steps: List[SopStep]) -> str | None:
    ...
    def _check_resource_lock_conflict(self, step: SopStep, active_steps: List[SopStep]) -> str | None:
    ...
    def _check_high_concurrency(self, step: SopStep, active_steps: List[SopStep]) -> str | None:
