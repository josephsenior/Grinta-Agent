...
class ParallelExecutionEngine:
    ...
    def _initialize_step_execution_info(self, steps: List[SopStep], completed_artifacts: Dict[str, Artifact]) -> None:
    ...
    def _collect_group_results(self, future_to_step: Dict[Future[StepResult], SopStep]) -> StepResult:
    ...
    def _update_step_state_after_execution(self, step: SopStep, group_success: bool) -> None:
    ...
    def _calculate_performance_stats(self, start_time: float, parallel_groups: List[List[SopStep]]) -> None:
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_parallel_workers) as executor:
                for group_idx, step_group in enumerate(parallel_groups):
                    ...
                    future_to_step = self._submit_group_tasks(executor, step_group, executor_func, executor_args)
                    group_success, group_artifacts = self._collect_group_results(future_to_step)
    ...
    def _calculate_async_performance_stats(self, start_time: float, parallel_groups: List[List[SopStep]]) -> None:
        ...
        try:
            for group_idx, step_group in enumerate(parallel_groups):
                ...
                try:
                    ...
                    group_success, group_artifacts = await self._execute_step_tasks(async_tasks)
