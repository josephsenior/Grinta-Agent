"""JSON extraction, validation, and corrective prompt helpers for MetaSOP."""
...
def corrective_prompt_for_validation(validation_type: str) -> str | None:
    """Return corrective prompt string for given validation failure type."""
    mapping = {
        "json_parse_repair_failed": "Your previous output did not match the required JSON schema.\nErrors: JSON parse/repair failed.\nRespond with JSON ONLY that conforms to the schema.\nMinimal example: {json.dumps(minimal_example, ensure_ascii=False)}",
        "schema_validation_failed": "Your previous output did not match the required JSON schema.\nErrors: Schema validation failed.\nRespond with JSON ONLY that conforms to the schema.\nMinimal example: {json.dumps(minimal_example, ensure_ascii=False)}",
    }
    return mapping.get(validation_type)
