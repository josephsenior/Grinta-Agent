...
class TestRealWorldAutonomousScenarios:
    ...
    async def test_build_simple_todo_app(self, safety_enabled_config, temp_workspace):
        ...
        try:
            ...
            audit_logs = controller.audit_logger.get_recent_entries(limit=100) if hasattr(controller, 'audit_logger') else []
    ...
    async def test_dangerous_command_blocked(self, safety_enabled_config, temp_workspace):
        ...
        try:
            ...
            error_obs = [
                obs for obs in controller.state.history
    ...
    async def test_error_recovery_and_retry(self, safety_enabled_config, temp_workspace):
        ...
        try:
            ...
            error_count = sum(1 for event in controller.state.history if 'error' in str(type(event)).lower())
    ...
    async def test_circuit_breaker_trips_on_repeated_errors(self, safety_enabled_config, temp_workspace):
        ...
        try:
            ...
            if hasattr(controller, 'circuit_breaker'):
                assert controller.circuit_breaker.is_tripped(), "Circuit breaker should be tripped"
    ...
    async def test_build_calculator_with_tests(self, safety_enabled_config, temp_workspace):
        ...
        try:
            ...
            test_runs = [
                event for event in controller.state.history
