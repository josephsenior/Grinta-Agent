    ...
    async def test_performance_improvement_ratio(self, temp_dir):
        ...
        with patch.dict(os.environ, {
            ...
            try:
                ...
                print(f"[OK] Performance improvement: {improvement:.1f}% (target: >80%)")
...
def test_performance_tuning_variables():
    ...
    with patch.dict(os.environ, {
        ...
        assert plugin_timeout == 60, f"Expected INIT_PLUGIN_TIMEOUT=60, got {plugin_timeout}"
        assert bash_timeout == 10, f"Expected NO_CHANGE_TIMEOUT_SECONDS=10, got {bash_timeout}"
