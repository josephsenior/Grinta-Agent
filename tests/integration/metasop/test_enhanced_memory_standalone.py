...
if sys.platform == "win32":
    if hasattr(sys.stdout, "reconfigure"):
        ...
        sys.stdout = io.TextIOWrapper(
            sys.stdout.buffer, encoding="utf-8", closefd=False
        )
        sys.stderr = io.TextIOWrapper(
            sys.stderr.buffer, encoding="utf-8", closefd=False
        )

# Add Forge to path
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "forge"))
...
def test_enhanced_integration():
    ...
    for i, (query, expected_id) in enumerate(test_queries, 1):
        ...
        if results:
            top = results[0]
            print(
                f"   Top result: {top.get('step_id')} (score: {top.get('score', 0):.3f})"
            )
    ...
    print(
        f"  Accuracy: {accuracy:.1f}% (target: {target_accuracy}%) "
        f"{'✅' if accuracy >= target_accuracy * 0.9 else '⚠️'}"
    )
    print(
        f"  First query: {avg_latency:.1f}ms (target: <{target_latency_first}ms) "
        f"{'✅' if avg_latency <= target_latency_first else '⚠️'}"
    )
    print(
        f"  Cached query: {cache_avg:.1f}ms (target: <{target_latency_avg}ms) "
        f"{'✅' if cache_avg <= target_latency_avg else '⚠️'}"
    )
    ...
    if stats.get("mode") == "enhanced":
        ...
        print(
            "  Install for better quality: pip install chromadb sentence-transformers"
        )
    ...
    if success:
        ...
        print(
            f"   Accuracy below target: {accuracy:.1f}% < {target_accuracy * 0.9:.1f}%"
        )
