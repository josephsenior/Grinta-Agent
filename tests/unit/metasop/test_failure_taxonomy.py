...
def test_json_parse_detection():
    ftype, _ = classify_failure(
        "s1", "qa", validation_err="JSON parse/repair failed: unexpected token"
    )
...
def test_schema_validation_detection():
    ftype, _ = classify_failure(
        "s1", "pm", validation_err="Schema validation failed: missing required property"
    )
...
def test_qa_test_fail_detection_from_stdout():
    ...
    ftype, _ = classify_failure("s1", "qa", stdout=out, stderr="")
    assert ftype == "qa_test_fail"


def test_build_error_detection():
    err = 'Traceback (most recent call last):\n  ModuleNotFoundError: No module named "foo"'
    ftype, _ = classify_failure("s1", "engineer", stderr=err)
...
def test_retries_exhausted_flag():
    ftype, _ = classify_failure("s1", "engineer", retries_exhausted=True)
