...
def test_user_microagents_loading(temp_user_microagents_dir):
    ...
    with patch("openhands.memory.memory.USER_MICROAGENTS_DIR", str(temp_user_microagents_dir)):
        with tempfile.TemporaryDirectory() as temp_dir:
...
def test_user_microagents_directory_creation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("openhands.memory.memory.USER_MICROAGENTS_DIR", str(non_existent_dir)):
