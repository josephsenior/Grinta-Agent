...
from forge.resolver.interfaces.gitlab import GitlabIssueHandler
...
@patch("forge.resolver.interfaces.gitlab.GitlabIssueHandler.reply_to_comment")
@patch("httpx.post")
...
def test_update_existing_pull_request(
    ...
    expected_args = [
        ...
        patch_dir,
        "push",
        f"https://{username}:{token}@gitlab.com/{issue.owner}/{issue.repo}.git",
        issue.head_branch,
    ]
...
def test_send_pull_request(
    ...
    if pr_type == "branch":
        assert (
            result
            == "https://gitlab.com/test-owner/test-repo/-/compare/main...Forge-fix-issue-42"
        )
        ...
        assert post_data["source_branch"] == "Forge-fix-issue-42"
...
def test_reply_to_comment(mock_get, mock_post, mock_issue):
    ...
    mock_post.assert_called_once_with(
        f"https://gitlab.com/api/v4/projects/{
            quote(f'{mock_issue.owner}/{mock_issue.repo}', safe='')
        }/merge_requests/{mock_issue.number}/discussions/{
            comment_id.split('/')[-1]
        }/notes",
        headers={"Authorization": f"Bearer {token}", "Accept": "application/json"},
        json=data,
...
def _create_gitlab_mock_args():
    ...
    mock_args.git_user_name = "forge"
    mock_args.git_user_email = "Forge@all-hands.dev"
    return mock_args
...
def _verify_gitlab_successful_execution(
    ...
    _verify_gitlab_process_single_issue_call(
        mock_process_single_issue, mock_args, llm_config
    )

    # Verify other function calls
    _verify_gitlab_other_function_calls(
        mock_parser, mock_getenv, mock_path_exists, mock_load_single_resolver_output
    )


def _verify_gitlab_process_single_issue_call(
    mock_process_single_issue, mock_args, llm_config
):
...
def _verify_gitlab_other_function_calls(
    mock_parser, mock_getenv, mock_path_exists, mock_load_single_resolver_output
):
