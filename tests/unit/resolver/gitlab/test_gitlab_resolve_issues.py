...
def mock_gitlab_token():
    ...
    """
    with patch("openhands.resolver.issue_resolver.identify_token", return_value=ProviderType.GITLAB) as patched:
        yield patched
...
def test_download_issues_from_gitlab():
    ...
    def get_mock_response(url, *args, **kwargs):
        return mock_comments_response if "/notes" in url else mock_issues_response
...
async def test_process_issue(
    ...
    with patch("openhands.resolver.issue_resolver.create_runtime", mock_create_runtime), patch(
        ...
        "openhands.resolver.issue_resolver.OpenHandsConfig", return_value=MagicMock()
    ):
...
def test_file_instruction():
    ...
    assert images_urls in (["https://sampleimage.com/sample.png"], [])
...
def test_download_issue_with_specific_comment():
    ...
    mock_comments_response = MagicMock()
    mock_comments_response.json.return_value = [
        {"id": specific_comment_id, "body": "Specific comment body"},
        {"id": 102, "body": "Another comment body"},
    ]
    mock_comments_response.raise_for_status = MagicMock()
