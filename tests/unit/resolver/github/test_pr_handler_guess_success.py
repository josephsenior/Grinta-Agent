...
def pr_handler():
    ...
    return ServiceContextPR(GithubPRHandler("test-owner", "test-repo", "test-token"), llm_config)
...
def test_guess_success_review_threads_litellm_call():
    ...
    issue = Issue(
        ...
        review_threads=[
            ...
            ReviewThread(
                comment="Add more tests\n---\nlatest feedback:\nAdd test cases", files=["/tests/test_file.py"]
            ),
        ],
        thread_ids=["1", "2"],
...
def test_check_feedback_with_llm():
    ...
    handler = ServiceContextPR(GithubPRHandler("test-owner", "test-repo", "test-token"), llm_config)
    test_cases = [
        {"response": "--- success\ntrue\n--- explanation\nChanges look good", "expected": (True, "Changes look good")},
        {
...
def test_check_thread_comments_with_git_patch():
    ...
    llm_config = LLMConfig(model="test", api_key="test")
    handler = ServiceContextPR(GithubPRHandler("test-owner", "test-repo", "test-token"), llm_config)
    thread_comments = ["Please improve error handling", "Add input validation", "latest feedback:\nHandle edge cases"]
    issues_context = json.dumps(["Issue 1 description", "Issue 2 description"], indent=4)
...
def test_check_review_comments_with_git_patch():
    ...
    llm_config = LLMConfig(model="test", api_key="test")
    handler = ServiceContextPR(GithubPRHandler("test-owner", "test-repo", "test-token"), llm_config)
    review_comments = ["Please fix the code style", "Add more test cases", "latest feedback:\nImprove documentation"]
    issues_context = json.dumps(["Issue 1 description", "Issue 2 description"], indent=4)
