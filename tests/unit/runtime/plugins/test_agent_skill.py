...
def _numbered_test_lines(start, end) -> str:
    """Generate numbered test lines for testing file operations.
    ...
    Returns:
        str: String containing numbered lines separated by newlines.
    """
    return "\n".join((f"{i}|" for i in range(start, end + 1))) + "\n"


def _generate_test_file_with_lines(temp_path, num_lines) -> str:
    """Generate a test Python file with the specified number of empty lines.

    Args:
        temp_path: Temporary directory path for the test file.
        num_lines: Number of empty lines to write to the file.
...
def _generate_ruby_test_file_with_lines(temp_path, num_lines) -> str:
    """Generate a test Ruby file with the specified number of empty lines.
...
def test_find_file(tmp_path):
    ...
    expected += f"{os.path.join(str(tmp_path), 'a.txt')}\n"
...
def test_parse_docx(tmp_path):
    ...
    expected_output = f"[Reading DOCX file from {test_docx_path}]\n@@ Page 1 @@\nHello, this is a test document.\n\n@@ Page 2 @@\nThis is the second paragraph.\n\n\n"
