...
def _build_state(iteration: int) -> State:
    flag = IterationControlFlag(
        limit_increase_amount=10, current_value=iteration, max_value=100
    )
    history = [
    ...
    state = State()
    state.iteration_flag = flag
    state.history = cast(list, history)
    return state
...
def test_progress_tracker_update_metrics(monkeypatch):
    ...
    monkeypatch.setattr(
        "forge.controller.progress_tracker.datetime",
        SimpleNamespace(now=lambda: fake_now),
    )
...
def test_progress_tracker_estimate_completion(monkeypatch):
    ...
    if metrics.completion_percentage < 1:
        assert (
            metrics.estimated_completion_time is None
            or metrics.estimated_completion_time >= datetime.now()
        )
