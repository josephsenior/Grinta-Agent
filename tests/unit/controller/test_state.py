...
def test_save_and_restore_round_trip(monkeypatch):
    ...
    conversation_stats = SimpleNamespace(save_metrics=MagicMock())

    state = State(session_id="sid", agent_state=AgentState.RUNNING, conversation_stats=conversation_stats)
    state.history.append(MessageAction(content="hello"))

    state.save_to_session("sid", file_store, user_id="user-1")
    ...
    restored = State.restore_from_session("sid", file_store, user_id="user-1")
...
def test_restore_from_session_with_user_fallback():
    ...
    state.save_to_session("sid", file_store, user_id=None)

    restored = State.restore_from_session("sid", file_store, user_id="user-2")
...
def test_restore_from_session_propagates_other_errors():
    ...
    with pytest.raises(ValueError):
        State.restore_from_session("sid", ErrorFileStore())
...
def test_getstate_and_setstate_strip_transient_fields():
    ...
    for key in ["_history_checksum", "_view", "iteration", "local_metrics", "traffic_control_state", "delegates"]:
...
def test_view_caching(monkeypatch):
    ...
    monkeypatch.setattr(state_module.View, "from_events", staticmethod(fake_from_events))
...
def test_save_to_session_restores_conversation_stats_on_failure():
    ...
    state = State(conversation_stats=SimpleNamespace())
    with pytest.raises(RuntimeError):
        state.save_to_session("sid", FailingFileStore(), user_id=None)
