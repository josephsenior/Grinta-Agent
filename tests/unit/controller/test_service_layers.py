...
from forge.controller.services.delegate_service import DelegateService
...
def test_delegate_service_end_delegate_emits_completion(monkeypatch):
    controller = make_controller()
    context = ControllerContext(controller)
    delegate_service = DelegateService(context)

    delegate = types.SimpleNamespace(
        state=types.SimpleNamespace(
            iteration_flag=types.SimpleNamespace(current_value=3),
            outputs={"result": "done"},
        ),
        agent=types.SimpleNamespace(name="child"),
        get_agent_state=lambda: AgentState.FINISHED,
        close=AsyncMock(),
    )
    controller.delegate = delegate
    controller.state.history = [AgentDelegateAction(agent="child", inputs={})]
    controller.event_stream = MagicMock()

    delegate_service.end_delegate()

    controller.event_stream.add_event.assert_called()
    assert controller.delegate is None


def test_delegate_service_acquires_delegate_runtime(monkeypatch):
    controller = make_controller()
    context = ControllerContext(controller)
    provider = MagicMock()
    ...
    delegate_stream = FakeStream()
    handle = types.SimpleNamespace(event_stream=delegate_stream, release=MagicMock())
    provider.acquire.return_value = handle
    controller.delegate_runtime_provider = provider
    delegate_service = DelegateService(context)

    agent_name = "TestDelegateAgent"
    ...
    class StubAgentController:
        def __init__(self, **kwargs):
            ...
            self.conversation_stats = kwargs["conversation_stats"]
            self.file_store = kwargs["file_store"]
            self.state = kwargs["initial_state"]
            self.is_delegate = kwargs["is_delegate"]
            self.set_agent_state_to = AsyncMock()

        def get_agent_state(self):
            return AgentState.RUNNING

        async def close(self, set_stop_state: bool = True):
            return None

    monkeypatch.setattr(
        DelegateService,
        "_resolve_agent_class",
        lambda self, _name: FakeAgent,
    )
    monkeypatch.setattr(
        "forge.controller.services.delegate_service.State",
        lambda **kwargs: types.SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr(
        "forge.controller.agent_controller.AgentController",
        StubAgentController,
    )

    action = AgentDelegateAction(agent=agent_name, inputs={"task": "delegate"})
    asyncio.run(delegate_service.handle_delegate_action(action))
...
@pytest.mark.asyncio
async def test_delegate_service_attaches_run_context(monkeypatch):
    controller = make_controller()
    controller.file_store = object()
    controller.conversation_stats = object()
    controller.event_stream.get_latest_event_id = lambda: 10
    context = ControllerContext(controller)
    delegate_service = DelegateService(context)
    ...
    action = AgentDelegateAction(agent=agent_name, inputs={"task": "do it"})
    await delegate_service.handle_delegate_action(action)

    assert controller.delegate is not None
    run_ctx = controller.delegate.delegate_run_context
    assert run_ctx is not None
    assert run_ctx.event_stream is controller.event_stream
    assert run_ctx.file_store is controller.file_store
    assert run_ctx.iteration_flag is controller.state.iteration_flag


def test_delegate_run_context_validation_detects_mismatch():
    parent_stream = object()
    ctx = DelegateRunContext(
        parent_id="parent",
        event_stream=parent_stream,
    ...
    delegate = types.SimpleNamespace(
        ...
        conversation_stats=object(),
        file_store=None,
        state=types.SimpleNamespace(iteration_flag="other"),
        is_delegate=True,
    )

    with pytest.raises(RuntimeError):
        ctx.validate(delegate)
