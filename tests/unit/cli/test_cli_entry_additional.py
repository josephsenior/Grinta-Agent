...
def test_execute_command_launches_gui(monkeypatch):
    ...
    called = {}
    monkeypatch.setattr(entry, "launch_gui_server", lambda **kwargs: called.setdefault("launch", kwargs))
    parser = SimpleNamespace(print_help=lambda: pytest.fail("print_help should not be called"))
...
def test_execute_command_runs_cli(monkeypatch):
    ...
    captured = {}
    monkeypatch.setattr(entry, "run_cli_command", lambda args: captured.setdefault("args", args))
    parser = SimpleNamespace(print_help=lambda: pytest.fail("print_help should not be called"))
...
def test_main_help_flow(monkeypatch):
    ...
    parser = SimpleNamespace(parse_args=lambda: SimpleNamespace(command="cli", version=False))
    monkeypatch.setattr(entry, "get_cli_parser", lambda: parser)

    called = {}
    monkeypatch.setattr(entry, "get_subparser", lambda parser, name: called.setdefault("subparser", (parser, name)))
    monkeypatch.setattr(entry.sys, "exit", lambda code=0: (_ for _ in ()).throw(SystemExit(code)))
...
def test_main_cli_flow(monkeypatch):
    ...
    executed = {}
    monkeypatch.setattr(entry, "run_cli_command", lambda args: executed.setdefault("args", args))
...
def test_main_serve_flow(monkeypatch):
    ...
    class ServeParser:
        def parse_args(self):
            return SimpleNamespace(command="serve", mount_cwd=False, gpu=True, version=False)
    ...
    launched = {}
    monkeypatch.setattr(entry, "launch_gui_server", lambda **kwargs: launched.setdefault("kwargs", kwargs))
