...
class TestHandleCommands:
    ...
    async def test_handle_init_command(self, mock_handle_init, mock_dependencies):
        ...
        mock_handle_init.assert_called_once_with(
            mock_dependencies["config"], mock_dependencies["event_stream"], mock_dependencies["current_dir"]
        )
        assert close_repl is True
    ...
    async def test_handle_unknown_command(self, mock_dependencies):
        ...
        close_repl, reload_microagents, new_session, _ = await handle_commands(user_message, **mock_dependencies)
...
class TestHandleResumeCommand:
    ...
    async def test_handle_resume_command_paused_state(self, mock_print):
        ...
        event_stream = MagicMock(spec=EventStream)
        close_repl, new_session_requested = await handle_resume_command(event_stream, AgentState.PAUSED)
        event_stream.add_event.assert_called_once()
        args, kwargs = event_stream.add_event.call_args
    ...
    async def test_handle_resume_command_invalid_states(self, mock_print, invalid_state):
        ...
        event_stream = MagicMock(spec=EventStream)
        close_repl, new_session_requested = await handle_resume_command(event_stream, invalid_state)
        event_stream.add_event.assert_not_called()
        ...
        assert "/resume command is only available when agent is paused" in str(error_call)
        assert close_repl is False
        assert new_session_requested is False
