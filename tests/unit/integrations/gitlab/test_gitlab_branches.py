...
async def test_get_paginated_branches_gitlab_headers_and_parsing():
    ...
    mock_response = [
        {"name": "main", "commit": {"id": "abc", "committed_date": "2024-01-01T00:00:00Z"}, "protected": True},
        {"name": "dev", "commit": {"id": "def", "committed_date": "2024-01-02T00:00:00Z"}, "protected": False},
    ]
    headers = {
        "X-Total": "42",
        "Link": '<https://gitlab.example.com/api/v4/projects/group%2Frepo/repository/branches?page=3&per_page=2>; rel="next"',
    }
    with patch.object(service, "_make_request", return_value=(mock_response, headers)):
        ...
        assert res.branches[0] == Branch(
            name="main", commit_sha="abc", protected=True, last_push_date="2024-01-01T00:00:00Z"
        )
        assert res.branches[1] == Branch(
            name="dev", commit_sha="def", protected=False, last_push_date="2024-01-02T00:00:00Z"
        )
...
async def test_get_paginated_branches_gitlab_no_next_or_total():
    service = GitLabService(token=SecretStr("t"))
    mock_response = [
        {"name": "fix", "commit": {"id": "zzz", "committed_date": "2024-01-03T00:00:00Z"}, "protected": False}
    ]
    headers = {}
    with patch.object(service, "_make_request", return_value=(mock_response, headers)):
...
async def test_search_branches_gitlab_uses_search_param():
    service = GitLabService(token=SecretStr("t"))
    mock_response = [
        {"name": "feat/new", "commit": {"id": "111", "committed_date": "2024-01-04T00:00:00Z"}, "protected": False},
        {"name": "feature/xyz", "commit": {"id": "222", "committed_date": "2024-01-05T00:00:00Z"}, "protected": True},
    ]
    with patch.object(service, "_make_request", return_value=(mock_response, {})) as m:
        branches = await service.search_branches("group/repo", query="feat", per_page=50)
        ...
        assert branches[0] == Branch(
            name="feat/new", commit_sha="111", protected=False, last_push_date="2024-01-04T00:00:00Z"
        )
        assert branches[1] == Branch(
            name="feature/xyz", commit_sha="222", protected=True, last_push_date="2024-01-05T00:00:00Z"
        )
