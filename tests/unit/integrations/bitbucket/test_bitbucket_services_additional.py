...
@pytest.mark.asyncio
async def test_bitbucket_make_request_handles_refresh_and_headers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    responses = [
        ...
        DummyHTTPResponse(
            status_code=200,
            headers={"Link": "next", "X-Total": "5", "Content-Type": "application/json"},
            json_data={"data": 1},
        ),
    ]
    ...
    monkeypatch.setattr("forge.integrations.bitbucket.service.base.httpx.AsyncClient", lambda: AsyncClientStub())
...
@pytest.mark.asyncio
async def test_bitbucket_make_request_returns_headers_dict(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    response = DummyHTTPResponse(status_code=200, headers={"Content-Type": "application/json"}, json_data={"ok": True})
...
@pytest.mark.asyncio
async def test_bitbucket_make_request_wraps_http_status_error(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
...
@pytest.mark.asyncio
async def test_bitbucket_make_request_wraps_http_error(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
...
@pytest.mark.asyncio
async def test_bitbucket_is_pr_open_state_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    monkeypatch.setattr(service, "get_pr_details", AsyncMock(return_value={"state": "OPEN"}))
    assert await service.is_pr_open("workspace/repo", 4) is True
    monkeypatch.setattr(service, "get_pr_details", AsyncMock(return_value={"state": "MERGED"}))
...
@pytest.mark.asyncio
async def test_bitbucket_search_public_repository_handles_exceptions(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    monkeypatch.setattr(service, "get_repository_details_from_repo_name", AsyncMock(side_effect=ValueError("fail")))
    result = await service._search_public_repository("https://bitbucket.org/workspace/repo")
    assert result == []


@pytest.mark.asyncio
async def test_bitbucket_workspace_search_skips_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    ...
    matching = await service._search_matching_workspaces("work", 10, "pushed", ["workspace", "work-two", "other"])
    assert matching == [repo_obj]
    monkeypatch.setattr(
        service,
        "get_paginated_repos",
    ...
    all_workspaces = await service._search_all_workspaces("repo", 10, "pushed", ["alpha", "beta"])
    assert all_workspaces == [repo_obj]


@pytest.mark.asyncio
async def test_bitbucket_get_paginated_repos_without_installation_returns_empty() -> None:
    service = BitBucketService(token=SecretStr("token"))
    result = await service.get_paginated_repos(page=1, per_page=10, sort="created", installation_id=None)
    assert result == []


@pytest.mark.asyncio
async def test_bitbucket_get_paginated_repos_sort_mappings(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    responses = [({"values": []}, {})] * 3
    mock_request = AsyncMock(side_effect=responses)
    monkeypatch.setattr(service, "_make_request", mock_request)

    await service.get_paginated_repos(page=2, per_page=5, sort="created", installation_id="workspace")
    await service.get_paginated_repos(page=3, per_page=5, sort="full_name", installation_id="workspace")
    await service.get_paginated_repos(page=4, per_page=5, sort="pushed", installation_id="workspace", query="repo")
...
@pytest.mark.asyncio
async def test_bitbucket_get_paginated_repos_formats_link_headers(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    response_with_page = ({"values": [_make_repo()], "next": "https://example.com?foo=1&page=7"}, {})
    response_without_page = ({"values": [_make_repo(repo_id="2")], "next": "https://example.com/next"}, {})
    monkeypatch.setattr(service, "_make_request", AsyncMock(side_effect=[response_with_page, response_without_page]))

    repos = await service.get_paginated_repos(page=1, per_page=5, sort="pushed", installation_id="workspace")
    assert repos[0].link_header.endswith('page=7>; rel="next"')

    repos_no_page = await service.get_paginated_repos(page=1, per_page=5, sort="pushed", installation_id="workspace")
    assert repos_no_page[0].link_header.endswith('next>; rel="next"')


@pytest.mark.asyncio
async def test_bitbucket_get_all_repositories_skips_missing_slug_and_limits(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    workspaces = [{"slug": "workspace"}, {"name": "no-slug"}]
    large_repo_list = [_make_repo(repo_id=str(i)) for i in range(1001)]
    monkeypatch.setattr(service, "_fetch_paginated_data", AsyncMock(side_effect=[workspaces, large_repo_list]))
    repos = await service.get_all_repositories("pushed", AppMode.SAAS)
...
@pytest.mark.asyncio
async def test_bitbucket_get_all_repositories_sort_variants(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    workspaces = [{"slug": "workspace"}]
...
@pytest.mark.asyncio
async def test_bitbucket_get_all_repositories_skips_empty_workspace(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    workspaces = [{"slug": None}, {"slug": "workspace"}]
    repos = [_make_repo()]
    monkeypatch.setattr(service, "_fetch_paginated_data", AsyncMock(side_effect=[workspaces, repos]))
...
@pytest.mark.asyncio
async def test_bitbucket_microagents_directory_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    service = BitBucketService(token=SecretStr("token"))
    repo = Repository(id="1", full_name="workspace/repo", git_provider=ProviderType.BITBUCKET, is_public=True, main_branch=None)
    monkeypatch.setattr(service, "get_repository_details_from_repo_name", AsyncMock(return_value=repo))
    with pytest.raises(ResourceNotFoundError):
        await service._get_microagents_directory_url("workspace/repo", ".Forge/microagents")
