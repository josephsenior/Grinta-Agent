...
async def test_get_paginated_branches_bitbucket_parsing_and_pagination():
    ...
    mock_response = {
        "values": [
            ...
            {"name": "feature/x", "target": {"hash": "def", "date": "2024-01-02T00:00:00Z"}},
        ],
    ...
    with patch.object(service, "_make_request", return_value=(mock_response, {})):
        ...
        assert res.branches == [
            Branch(name="main", commit_sha="abc", protected=False, last_push_date="2024-01-01T00:00:00Z"),
            Branch(name="feature/x", commit_sha="def", protected=False, last_push_date="2024-01-02T00:00:00Z"),
        ]
...
async def test_search_branches_bitbucket_filters_by_name_contains():
    service = BitBucketService(token=SecretStr("t"))
    mock_response = {"values": [{"name": "bugfix/issue-1", "target": {"hash": "hhh", "date": "2024-01-10T10:00:00Z"}}]}
