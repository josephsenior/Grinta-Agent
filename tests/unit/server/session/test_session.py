...
async def test_notify_on_llm_retry(
    ...
    with patch("time.sleep"):
        ...
        llm = llm_registry.get_llm(service_id="test_service", config=default_llm_config)
        llm.completion(messages=[{"role": "user", "content": "Hello!"}], stream=False)
    assert mock_litellm_completion.call_count == 2
    session.queue_status_message.assert_called_once_with("info", RuntimeStatus.LLM_RETRY, ANY)
    await session.close()
