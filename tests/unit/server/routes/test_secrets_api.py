...
async def test_invalidate_legacy_secrets_store_migrates_once(monkeypatch):
    class DummySettings:
        ...
        def model_copy(self, update):
            return DummySettings(self.user_id, update.get("secrets_store", self.secrets_store))

    settings_store = AsyncMock()
    secrets_store = AsyncMock()
    provider_tokens = {
        ProviderType.GITHUB: ProviderToken(token=SecretStr("gh-key"), host="github.com"),
    }
...
async def test_invalidate_legacy_secrets_store_no_tokens(monkeypatch):
    ...
    settings_store = AsyncMock()
    secrets_store = AsyncMock()
    dummy_settings = DummySettings("user-2", SimpleNamespace(provider_tokens={}))
...
async def test_check_provider_tokens_detects_host_mismatch(monkeypatch):
    ...
    existing = {
        ProviderType.GITHUB: ProviderToken(token=SecretStr("existing-token"), host="github.com"),
    }
...
def test_load_custom_secrets_names_none_returns_empty(test_client):
    ...
    test_client.app.dependency_overrides[secrets_routes.get_user_secrets] = override_user_secrets
    try:
        response = test_client.get("/api/secrets")
    finally:
...
def test_load_custom_secrets_names_error(test_client):
    ...
    with patch("forge.server.routes.secrets.get_user_secrets", AsyncMock(return_value=user_secrets)):
        with patch("forge.server.routes.secrets.GETCustomSecrets", side_effect=Exception("boom")):
            response = test_client.get("/api/secrets")
...
@pytest.mark.asyncio
async def test_create_custom_secret_duplicate_returns_400(test_client, file_secrets_store):
...
@pytest.mark.asyncio
async def test_create_custom_secret_store_failure_returns_500(test_client, file_secrets_store):
...
@pytest.mark.asyncio
async def test_update_custom_secret_not_found_returns_404(test_client, file_secrets_store):
...
@pytest.mark.asyncio
async def test_update_custom_secret_name_conflict_returns_400(test_client, file_secrets_store):
    custom_secrets = {
        "FIRST": CustomSecret(secret=SecretStr("v1")),
        "SECOND": CustomSecret(secret=SecretStr("v2")),
    }
    await file_secrets_store.store(UserSecrets(custom_secrets=custom_secrets, provider_tokens={}))
...
@pytest.mark.asyncio
async def test_update_custom_secret_store_failure_returns_500(test_client, file_secrets_store):
    custom_secrets = {
...
@pytest.mark.asyncio
async def test_delete_custom_secret_store_failure_returns_500(test_client, file_secrets_store):
    custom_secrets = {
        "API_KEY": CustomSecret(secret=SecretStr("api-key-value")),
    }
    await file_secrets_store.store(UserSecrets(custom_secrets=custom_secrets, provider_tokens={}))
...
async def test_add_git_providers_with_host(test_client, file_secrets_store):
    ...
    with patch("forge.server.routes.secrets.check_provider_tokens", AsyncMock(return_value=("", normalized_tokens))):
        add_provider_data = {
...
def test_store_provider_tokens_handles_exception(test_client):
    ...
    test_client.app.dependency_overrides[secrets_routes.get_secrets_store] = override_store
    test_client.app.dependency_overrides[secrets_routes.get_provider_tokens] = override_provider_tokens
    try:
        with patch("forge.server.routes.secrets.check_provider_tokens", AsyncMock(return_value=("", normalized_tokens))):
            response = test_client.post(
                "/api/add-git-providers",
                json={"provider_tokens": {"github": {"token": "token", "host": "github.com"}}},
            )
    finally:
        test_client.app.dependency_overrides.pop(secrets_routes.get_secrets_store, None)
        test_client.app.dependency_overrides.pop(secrets_routes.get_provider_tokens, None)
    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
...
def test_unset_provider_tokens_error_returns_500(test_client):
    class DummyStore:
        async def load(self):
            return UserSecrets(provider_tokens={ProviderType.GITHUB: ProviderToken(token=SecretStr("token"))})
...
def test_store_provider_tokens_error_message_returns_401(test_client):
    with patch("forge.server.routes.secrets.check_provider_tokens", AsyncMock(return_value=("Invalid token", {}))):
        response = test_client.post(
            "/api/add-git-providers",
...
def test_secrets_app_proxy_callable():
    ...
    with TestClient(secrets_routes.app) as client:
        client.app.dependency_overrides[secrets_routes.get_secrets_store] = override_store
        client.app.dependency_overrides[secrets_routes.get_provider_tokens] = override_provider_tokens
        client.app.dependency_overrides[secrets_routes.get_user_secrets] = override_user_secrets
        try:
