...
from forge.storage.data_models.slack_integration import SlackConversationLink, SlackOutgoingMessage, SlackUserLink, SlackWorkspace
...
class DummySlackClient:
    ...
    def post_ephemeral_message(self, channel: str, user: str, text: str, thread_ts: str | None = None):
    ...
    def format_code_block(self, content: str, language: str | None = None):
        return f"```{language or ''}\n{content}\n```"

    def remove_bot_mention(self, text: str, bot_user_id: str | None):
        return text.replace(f"<@{bot_user_id}>", "").strip()

    def extract_repo_from_text(self, text: str):
...
class DummySlackStore:
    ...
    def generate_oauth_state(self, user_id: str, redirect_url: str | None):
        state = f"state-{user_id}"
        self.oauth_states[state] = SimpleNamespace(user_id=user_id, redirect_url=redirect_url)
        return state
    ...
    def save_workspace(self, workspace: SlackWorkspace):
        self.workspaces[workspace.team_id] = workspace

    def get_workspace(self, team_id: str):
        return self.workspaces.get(team_id)

    def list_workspaces(self):
        return list(self.workspaces.values())

    def save_user_link(self, link: SlackUserLink):
        self.user_links[(link.slack_workspace_id, link.slack_user_id)] = link

    def get_user_link(self, team_id: str, user_id: str):
        return self.user_links.get((team_id, user_id))

    def save_conversation_link(self, link: SlackConversationLink):
        self.conversation_links[(link.slack_workspace_id, link.slack_channel_id, link.slack_thread_ts)] = link

    def get_conversation_link(self, team_id: str, channel_id: str, thread_ts: str):
        return self.conversation_links.get((team_id, channel_id, thread_ts))

    def delete_workspace(self, team_id: str):
...
def test_create_slack_event_callback_posts_messages(monkeypatch):
    ...
    callback = slack_routes.create_slack_event_callback(client, "channel", "thread", "cid")

    asyncio.run(callback(AgentThinkAction(thought="ponder")))
    asyncio.run(callback(CmdRunAction(command="ls")))
    asyncio.run(callback(CmdOutputObservation(content="result", command="ls", exit_code=0)))
    asyncio.run(callback(MessageAction(content="hello")))
    asyncio.run(callback(AgentStateChangedObservation(content="", agent_state=AgentState.FINISHED.value)))
    asyncio.run(callback(AgentStateChangedObservation(content="", agent_state=AgentState.ERROR.value)))
    asyncio.run(callback(AgentStateChangedObservation(content="", agent_state=AgentState.RUNNING.value)))
    asyncio.run(callback(AgentStateChangedObservation(content="", agent_state="OTHER")))
...
def test_create_slack_event_callback_truncates_output():
    ...
    callback = slack_routes.create_slack_event_callback(client, "channel", "thread", "cid")
    long_output = "x" * 600
    asyncio.run(callback(CmdOutputObservation(content=long_output, command="ls", exit_code=1)))
...
def test_create_slack_event_callback_handles_errors(monkeypatch):
    ...
    callback = slack_routes.create_slack_event_callback(client, "channel", "thread", "cid")
    asyncio.run(callback(MessageAction(content="oops")))
...
async def test_verify_slack_signature_success(monkeypatch):
    ...
    request = DummyRequest(headers={"X-Slack-Request-Timestamp": timestamp, "X-Slack-Signature": signature}, body=body)
...
async def test_verify_slack_signature_invalid(monkeypatch):
    ...
    request = DummyRequest(headers={"X-Slack-Request-Timestamp": timestamp, "X-Slack-Signature": "sig"}, body=b"data")
...
async def test_verify_slack_signature_missing_headers():
    request = DummyRequest(headers={}, body=b"")
    assert await slack_routes.verify_slack_signature(request, "secret") is False


@pytest.mark.asyncio
async def test_verify_slack_signature_no_secret():
    request = DummyRequest(headers={"X-Slack-Request-Timestamp": str(int(time.time())), "X-Slack-Signature": "sig"}, body=b"")
...
async def test_slack_install_success(monkeypatch):
    ...
    response = await slack_routes.slack_install("user-1", "https://redirect", slack_store=store)
...
async def test_slack_install_not_configured(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        await slack_routes.slack_install("user", slack_store=store)
...
async def test_slack_oauth_callback_error(monkeypatch):
    store = DummySlackStore()
    response = await slack_routes.slack_oauth_callback(state="s", error="access_denied", slack_store=store)
...
async def test_slack_oauth_callback_invalid_state(monkeypatch):
    store = DummySlackStore()
    with pytest.raises(HTTPException) as exc:
        await slack_routes.slack_oauth_callback(state="missing", code="code", slack_store=store)
...
async def test_slack_oauth_callback_missing_code(monkeypatch):
    store = DummySlackStore()
    with pytest.raises(HTTPException) as exc:
        await slack_routes.slack_oauth_callback(state="s", slack_store=store)
    assert exc.value.status_code == 400
...
async def test_slack_oauth_callback_sdk_not_installed(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        await slack_routes.slack_oauth_callback(state="state", code="code", slack_store=store)
    assert exc.value.status_code == 501
...
async def test_slack_oauth_callback_success(monkeypatch):
    store = DummySlackStore()
    store.oauth_states["state"] = SimpleNamespace(user_id="user", redirect_url="/return")
    slack_routes.FORGE_config.SLACK_CLIENT_ID = "client"
    slack_routes.FORGE_config.SLACK_CLIENT_SECRET = SimpleNamespace(get_secret_value=lambda: "secret")
    ...
    dummy_module = SimpleNamespace(WebClient=DummyWebClient)
    monkeypatch.setitem(sys.modules, "slack_sdk", dummy_module)

    response = await slack_routes.slack_oauth_callback(state="state", code="code", slack_store=store)
    assert response.status_code == 200
    assert "Slack installed successfully" in response.body.decode()
...
async def test_slack_oauth_callback_exchange_failure(monkeypatch):
    ...
    sys.modules["slack_sdk"].WebClient = DummyWebClient
...
async def test_slack_events_url_verification(monkeypatch):
    ...
    store = DummySlackStore()
    slack_routes.FORGE_config.SLACK_SIGNING_SECRET = SimpleNamespace(get_secret_value=lambda: None)
    response = await slack_routes.slack_events(request, slack_store=store)
    assert json.loads(response.body)["challenge"] == "abc"


@pytest.mark.asyncio
async def test_slack_events_invalid_signature(monkeypatch):
    request = DummyRequest(json_data={"event": {}}, headers={})

    slack_routes.FORGE_config.SLACK_SIGNING_SECRET = SimpleNamespace(get_secret_value=lambda: "secret")
    ...
    with pytest.raises(HTTPException) as exc:
        await slack_routes.slack_events(request, slack_store=DummySlackStore())
...
async def test_slack_events_thread_message(monkeypatch):
    ...
    request = DummyRequest(json_data=body, headers={})
...
async def test_handle_app_mention_missing_fields():
    await slack_routes.handle_app_mention({}, DummySlackStore())


@pytest.mark.asyncio
async def test_handle_app_mention_workspace_missing():
    event = {"team": "team", "channel": "channel", "user": "user", "ts": "ts", "text": "hello"}
    await slack_routes.handle_app_mention(event, DummySlackStore())


@pytest.mark.asyncio
async def test_handle_app_mention_user_not_linked(monkeypatch):
    store = DummySlackStore()
    workspace = SlackWorkspace(id="team", team_id="team", team_name="Team", bot_token="token", bot_user_id="bot", installed_by_user_id="user")
    store.save_workspace(workspace)
    event = {"team": "team", "channel": "channel", "user": "user", "ts": "ts", "text": "<@bot> help"}
    await slack_routes.handle_app_mention(event, store)
...
async def test_handle_app_mention_existing_conversation(monkeypatch):
    ...
    store.save_user_link(SlackUserLink(slack_user_id="user", slack_workspace_id="team", FORGE_user_id="forge", user_token=None))

    async def fake_continue(link, text, workspace, store):
        fake_continue.called = True

    fake_continue.called = False
    monkeypatch.setattr(slack_routes, "continue_conversation", fake_continue)

    event = {"team": "team", "channel": "channel", "user": "user", "ts": "ts", "text": "hello"}
    await slack_routes.handle_app_mention(event, store)
    assert fake_continue.called is True
...
async def test_handle_app_mention_starts_conversation(monkeypatch):
    ...
    async def fake_start(*args, **kwargs):
        fake_start.called = True

    fake_start.called = False
    monkeypatch.setattr(slack_routes, "start_new_conversation", fake_start)

    event = {"team": "team", "channel": "channel", "user": "user", "ts": "ts", "text": "<@bot> repo: x"}
    await slack_routes.handle_app_mention(event, store)
    assert fake_start.called is True
...
async def test_start_new_conversation_success(monkeypatch):
    ...
    workspace = SlackWorkspace(id="team", team_id="team", team_name="Team", bot_token="token", bot_user_id="bot", installed_by_user_id="user")
    store.save_workspace(workspace)
    user_link = SlackUserLink(slack_user_id="user", slack_workspace_id="team", FORGE_user_id="forge", user_token=None)
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(
            get_agent_session=lambda cid: SimpleNamespace(event_stream=DummyEventStream()),
        ),
    )
    stream_module = types.ModuleType("forge.events.stream")
    stream_module.EventStreamSubscriber = SimpleNamespace(SERVER="server")
    monkeypatch.setitem(sys.modules, "forge.events.stream", stream_module)
    monkeypatch.setattr(slack_routes, "InitSessionRequest", lambda **kwargs: SimpleNamespace(**kwargs))
...
async def test_start_new_conversation_error_response(monkeypatch):
    ...
    async def fake_new_conversation(**kwargs):
        ...
        resp.body = "error text"  # type: ignore[assignment]
...
async def test_start_new_conversation_subscription_error(monkeypatch):
    ...
    shared = __import__("forge.server.shared", fromlist=["conversation_manager"])
    monkeypatch.setattr(shared, "conversation_manager", SimpleNamespace(get_agent_session=lambda cid: SimpleNamespace(event_stream=DummyEventStream())))
    stream_module = types.ModuleType("forge.events.stream")
    stream_module.EventStreamSubscriber = SimpleNamespace(SERVER="server")
    monkeypatch.setitem(sys.modules, "forge.events.stream", stream_module)
    monkeypatch.setattr(slack_routes, "InitSessionRequest", lambda **kwargs: SimpleNamespace(**kwargs))
...
async def test_continue_conversation(monkeypatch):
    ...
    await slack_routes.continue_conversation(link, "message", workspace, store)
...
async def test_continue_conversation_error(monkeypatch):
    ...
    shared = __import__("forge.server.shared", fromlist=["conversation_manager"])
    monkeypatch.setattr(shared, "conversation_manager", SimpleNamespace(send_event_to_conversation=failing_send_event))
    monkeypatch.setattr(slack_routes, "SlackClient", DummySlackClient)
...
async def test_handle_thread_message(monkeypatch):
    ...
    event = {"team": "team", "channel": "channel", "thread_ts": "thread", "text": "hello"}
    await slack_routes.handle_thread_message(event, store)


@pytest.mark.asyncio
async def test_handle_thread_message_bot_or_missing():
    await slack_routes.handle_thread_message({"bot_id": "bot"}, DummySlackStore())
    await slack_routes.handle_thread_message({"team": "team", "channel": "channel", "thread_ts": "thread"}, DummySlackStore())
    await slack_routes.handle_thread_message({"team": "team", "thread_ts": "thread"}, DummySlackStore())
...
async def test_handle_thread_message_workspace_missing():
    ...
    store.save_conversation_link(link)
    await slack_routes.handle_thread_message({"team": "team", "channel": "channel", "thread_ts": "thread"}, store)


@pytest.mark.asyncio
async def test_list_workspaces(monkeypatch):
    store = DummySlackStore()
    store.save_workspace(SlackWorkspace(id="team", team_id="team", team_name="Team", bot_token="token", bot_user_id="bot", installed_by_user_id="user"))
    response = await slack_routes.list_workspaces("user", slack_store=store)
    data = json.loads(response.body)
...
async def test_uninstall_workspace(monkeypatch):
    store = DummySlackStore()
    store.save_workspace(SlackWorkspace(id="team", team_id="team", team_name="Team", bot_token="token", bot_user_id="bot", installed_by_user_id="user"))
    response = await slack_routes.uninstall_workspace("team", "user", slack_store=store)
...
async def test_uninstall_workspace_not_found(monkeypatch):
    store = DummySlackStore()
    with pytest.raises(HTTPException) as exc:
        await slack_routes.uninstall_workspace("team", "user", slack_store=store)
    assert exc.value.status_code == 404


@pytest.mark.asyncio
async def test_cleanup_slack_listener():
    slack_routes._slack_event_listeners["cid"] = (DummySlackClient("t"), "c", "ts")
