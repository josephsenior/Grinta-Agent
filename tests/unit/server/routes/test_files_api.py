...
class DummyRuntime:
    ...
    def copy_from(self, path):
        if isinstance(self.copy_from_calls, Exception):
            raise self.copy_from_calls
    ...
    def run_action(self, action):
        ...
        if isinstance(self.run_action_result, Exception):
            raise self.run_action_result
...
async def test_list_files_no_runtime():
    result = await files_routes.list_files(path=None, conversation=make_conversation(runtime=None))
    assert result == []
...
async def test_list_files_success(monkeypatch):
    ...
    result = await files_routes.list_files(path="sub", conversation=make_conversation(runtime))
...
async def test_list_files_runtime_unavailable(monkeypatch):
    ...
    async def fake_call(func, *args, **kwargs):
        raise runtime.list_result

    monkeypatch.setattr(files_routes, "call_sync_from_async", fake_call)
    response = await files_routes.list_files(path=None, conversation=make_conversation(runtime))
...
async def test_select_file_invalid_path():
    response = await files_routes.select_file(file="../bad", conversation=make_conversation(runtime=None))
    assert response.status_code == 400


@pytest.mark.asyncio
async def test_select_file_runtime_not_ready():
    response = await files_routes.select_file(file="valid.txt", conversation=make_conversation(runtime=None))
    assert response.status_code == 503
...
async def test_select_file_success(monkeypatch):
    ...
    runtime.run_action_result = FileReadObservation(path="/workspace/file.txt", content="hello")
    ...
    response = await files_routes.select_file(file="file.txt", conversation=make_conversation(runtime))
...
async def test_select_file_agent_error(monkeypatch):
    runtime = DummyRuntime()
    runtime.run_action_result = AgentRuntimeUnavailableError("down")
...
def test_zip_current_workspace_agent_error():
    runtime = DummyRuntime()
    runtime.copy_from_calls = AgentRuntimeUnavailableError("down")
    response = files_routes.zip_current_workspace(make_conversation(runtime))
    assert response.status_code == 500
...
async def test_git_changes_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(files_routes, "call_sync_from_async", fake_call)
    monkeypatch.setitem(sys.modules, "forge.server.shared", SimpleNamespace(conversation_manager=SimpleNamespace(attach_to_conversation=attach, detach_from_conversation=detach)))
...
async def test_git_changes_runtime_unavailable(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "forge.server.shared", SimpleNamespace(conversation_manager=SimpleNamespace(attach_to_conversation=attach)))
    response = await files_routes.git_changes("conv")
    assert response.status_code == 500
...
async def test_git_diff_invalid_path():
    response = await files_routes.git_diff(path="", conversation_store=None, conversation=make_conversation(DummyRuntime()))
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_git_diff_sanitization_error():
    response = await files_routes.git_diff(path="../bad", conversation_store=None, conversation=make_conversation(DummyRuntime()))
    assert response.status_code == 400
...
async def test_git_diff_success(monkeypatch):
    ...
    monkeypatch.setattr(files_routes, "call_sync_from_async", fake_call)
    response = await files_routes.git_diff(path="file.txt", conversation_store=None, conversation=make_conversation(runtime))
    assert response == {"path": "file.txt", "cwd": runtime.config.workspace_mount_path_in_sandbox}
...
class FakeUpload:
    def __init__(self, filename: str, data: bytes = b"content"):
