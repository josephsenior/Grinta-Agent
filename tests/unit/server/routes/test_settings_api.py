...
def test_client():
    ...
    ), patch("openhands.server.user_auth.user_auth.UserAuth.get_instance", return_value=MockUserAuth()), patch(
...
async def test_settings_api_endpoints(test_client):
    ...
    assert response.status_code == 200
    partial_settings = {"language": "fr", "llm_model": None, "llm_api_key": None}
