...
def test_message_with_vision_enabled():
    ...
    message: Message = Message(
        role="user", content=[text_content1, image_content1, text_content2, image_content2], vision_enabled=True
    )
    ...
    expected_serialized_message = {
        ...
        "content": [
            ...
            {"type": "image_url", "image_url": {"url": "http://example.com/image2.png"}},
            {"type": "text", "text": "This is another text message"},
            {"type": "image_url", "image_url": {"url": "http://example.com/image3.png"}},
            {"type": "image_url", "image_url": {"url": "http://example.com/image4.png"}},
        ],
    }
...
def test_message_with_only_text_content_and_vision_disabled():
    ...
    serialized_message: dict = message.serialize_model()
    expected_serialized_message = {"role": "user", "content": "This is a text message\nThis is another text message"}
    assert serialized_message == expected_serialized_message
    assert message.contains_image is False
...
def test_message_with_mixed_content_and_vision_disabled():
    ...
    image_content2 = ImageContent(image_urls=["http://example.com/image3.png", "http://example.com/image4.png"])
    message: Message = Message(
        role="user", content=[text_content1, image_content1, text_content2, image_content2], vision_enabled=False
    )
    serialized_message: dict = message.serialize_model()
    expected_serialized_message = {"role": "user", "content": "This is a text message\nThis is another text message"}
    assert serialized_message == expected_serialized_message
...
def test_message_tool_call_serialization():
    ...
    tool_call = ChatCompletionMessageToolCall(
        id="call_123", type="function", function={"name": "test_function", "arguments": '{"arg1": "value1"}'}
    )
    message = Message(role="assistant", content=[TextContent(text="Test message")], tool_calls=[tool_call])
...
def test_message_tool_response_serialization():
    ...
    message = Message(
        role="tool", content=[TextContent(text="Function result")], tool_call_id="call_123", name="test_function"
    )
    serialized = message.model_dump()
