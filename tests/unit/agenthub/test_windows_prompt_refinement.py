...
def mock_llm():
    ...
    cfg = LLMConfig(model="gpt-4", api_key="test_key")
...
def test_codeact_agent_tool_descriptions_no_bash_on_windows(mock_llm, agent_config):
    ...
    for tool in tools:
        if tool["type"] == "function":
            ...
            assert (
                "bash" not in description.lower()
            ), f"Tool '{
                function_info['name']}' description contains 'bash' on Windows. Description: {description}"
            parameters = function_info.get("parameters", {})
            properties = parameters.get("properties", {})
            for param_name, param_info in properties.items():
                param_description = param_info.get("description", "")
                assert (
                    "bash" not in param_description.lower()
                ), f"Tool '{
                    function_info['name']}' parameter '{param_name}' description contains 'bash' on Windows. Parameter description: {param_description}"
