...
class FakeExecutor:
    ...
    def execute(self, step, ctx, role_profile, config=None):
        ...
        trace = StepTrace(step_id=step.id, role=step.role, duration_ms=5, total_tokens=10)
...
def test_orchestrator_self_remediation_flow():
    ...
    extra = getattr(orch, "_ctx", None).extra if getattr(orch, "_ctx", None) else {}
    lineage_key = "failure_lineage::impl"
    has_lineage = lineage_key in extra
    has_hints = any((k in extra for k in ("validation_hint::impl", "failure_hint::impl")))
    assert has_lineage or has_hints
    failed_events = [
        e for e in orch.step_events if e.get("step_id") == "impl" and e.get("status") in ("failed", "warning")
    ]
    assert failed_events, f"expected failed/warning events for step impl, got: {orch.step_events}"
