...
def test_create_file(temp_dir, runtime_cls, run_as_Forge):
    ...
    try:
        ...
        action = FileEditAction(command="view", path=new_file)
        obs = runtime.run_action(action)
        logger.info(obs, extra={"msg_type": "OBSERVATION"})
...
def test_undo_edit(temp_dir, runtime_cls, run_as_Forge):
    ...
    try:
        ...
        logger.info(obs, extra={"msg_type": "OBSERVATION"})
        assert "This is a sample file." in obs.content
        action = FileEditAction(command="undo_edit", path=test_file)
        obs = runtime.run_action(action)
        logger.info(obs, extra={"msg_type": "OBSERVATION"})
...
def test_undo_edit_no_history_error(temp_dir, runtime_cls, run_as_Forge):
    ...
    try:
        ...
        runtime.run_action(action)
        action = FileEditAction(command="undo_edit", path=empty_file)
        obs = runtime.run_action(action)
        logger.info(obs, extra={"msg_type": "OBSERVATION"})
