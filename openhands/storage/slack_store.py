...
class SlackStore:
    ...
    def __init__(self, config: OpenHandsConfig) -> None:
        ...
        """
        self.config = config
        self.storage_path = Path(config.workspace_base or ".") / ".openhands" / "slack"
    ...
    def save_user_link(self, user_link: SlackUserLink) -> None:
        """Save Slack user to OpenHands user link."""
        ...
        logger.info(
            f"Linked Slack user {user_link.slack_user_id} to OpenHands user {user_link.openhands_user_id}",
        )
    ...
    def get_user_links_by_openhands_user(
        self,
        openhands_user_id: str,
    ) -> list[SlackUserLink]:
        """Get all Slack links for an OpenHands user."""
        user_links = self._read_json_file(self.user_links_file)
        return [
            SlackUserLink(**data) for data in user_links.values() if data.get("openhands_user_id") == openhands_user_id
        ]
    ...
    def save_conversation_link(self, conv_link: SlackConversationLink) -> None:
        """Save Slack thread to OpenHands conversation link."""
    ...
    def get_conversation_links_by_conversation_id(
        ...
        """Get all Slack links for an OpenHands conversation."""
        conv_links = self._read_json_file(self.conversation_links_file)
        return [
