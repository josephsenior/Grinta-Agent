...
from openhands.events.action import AgentThinkAction, CmdRunAction, MessageAction
from openhands.events.observation import (
    AgentStateChangedObservation,
    CmdOutputObservation,
)
from openhands.integrations.slack_client import SLACK_SDK_AVAILABLE, SlackClient
...
app = APIRouter(prefix="/api/slack")

# Track active Slack → OpenHands conversation streams
...
def create_slack_event_callback(
    ...
    """Create event callback to stream OpenHands events to Slack.
    ...
    async def callback(event: Event) -> None:
        """Stream OpenHands events to Slack thread."""
        try:
...
def verify_slack_signature(request: Request, slack_signing_secret: str | None) -> bool:
    ...
    # Verify signature
    sig_basestring = f"v0:{timestamp}:{request.body.decode('utf-8')}"  # nosec B324
    my_signature = (
        "v0="
        + hmac.new(
...
async def slack_install(
    ...
    slack_store: SlackStore = Depends(get_slack_store),
) -> JSONResponse:
    """Generate Slack OAuth install URL.

    Args:
        user_id: OpenHands user ID
...
async def slack_oauth_callback(
    ...
    try:
        ...
        response = client.oauth_v2_access(
            ...
            client_secret=(config.SLACK_CLIENT_SECRET.get_secret_value() if config.SLACK_CLIENT_SECRET else None),
            code=code,
        )
        ...
        if "authed_user" in response and "id" in response["authed_user"]:
            user_link = SlackUserLink(
                ...
                openhands_user_id=oauth_state.user_id,
        ...
        return Response(
            content=f'<html><body>Slack installed successfully! <a href="{redirect_url}">Return to OpenHands</a></body></html>',
...
async def slack_events(
    ...
    # Verify signature
    signing_secret = config.SLACK_SIGNING_SECRET.get_secret_value() if config.SLACK_SIGNING_SECRET else None
...
async def handle_app_mention(event: dict[str, Any], slack_store: SlackStore) -> None:
    """Handle @openhands mentions in Slack.
    ...
    try:
        ...
        if not user_link:
            ...
            client.post_ephemeral_message(
                channel=channel_id,
                user=user_id,
                text="Please link your OpenHands account first at https://app.all-hands.dev/settings/integrations",
                thread_ts=thread_ts,
            )
            return
...
async def start_new_conversation(
    ...
    """Start a new OpenHands conversation from Slack.
    ...
    try:
        ...
        conversation_response = await new_conversation(
            data=init_request,
            user_id=user_link.openhands_user_id,
            provider_tokens={},
            user_secrets={},  # type: ignore
            auth_type=None,
        )
...
async def continue_conversation(
    ...
    """Continue an existing OpenHands conversation from Slack.
    ...
    try:
        ...
        MessageAction(content=clean_text)
        event_dict = {
            "action": "message",
...
async def handle_thread_message(event: dict[str, Any], slack_store: SlackStore) -> None:
    ...
    conv_link = slack_store.get_conversation_link(team_id, channel_id, thread_ts)
    if not conv_link:
        # Not an OpenHands conversation thread
        return
...
async def uninstall_workspace(
    ...
    Args:
        team_id: Slack team ID
        user_id: OpenHands user ID (for verification)
        slack_store: Slack store
