...
async def get_conversation_link(service: GitService, conversation_id: str, body: str) -> str:
    """Appends a followup link, in the PR body, to the OpenHands conversation that opened the PR."""
    if get_server_config().app_mode != AppMode.SAAS:
        return body
...
async def save_pr_metadata(user_id: str | None, conversation_id: str, tool_result: str) -> None:
    """Extract and save PR number from tool result to conversation metadata.
    
    Parses GitHub pull request or GitLab merge request numbers from tool output.
    
    Args:
        user_id: User identifier
        conversation_id: Conversation identifier
        tool_result: Tool output string containing PR/MR URL
    """
