...
from openhands.storage.data_models.code_snippet import (
    CodeSnippet,
...
def _filter_by_query(snippets: list[CodeSnippet], query: str) -> list[CodeSnippet]:
...
def _build_snippet_collection(snippets: list[CodeSnippet]) -> SnippetCollection:
    ...
    return SnippetCollection(
        name="OpenHands Snippet Collection",
...
async def track_snippet_usage(snippet_id: str) -> CodeSnippet:
    """Track usage of a snippet (increment usage count).
    ...
    try:
        ...
        logger.exception(f"Error tracking snippet usage: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
