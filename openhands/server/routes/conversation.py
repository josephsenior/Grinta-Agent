...
from openhands.core.pydantic_compat import model_to_dict
...
async def get_vscode_url(
    ...
    try:
        ...
        if conversation:
            ...
            logger.debug("Runtime VSCode URL: %s", runtime.vscode_url)
            await conversation_manager.detach_from_conversation(conversation)
...
async def get_hosts(
    ...
    try:
        ...
        if conversation:
            runtime: Runtime = conversation.runtime
            logger.debug("Runtime type: %s", type(runtime))
            logger.debug("Runtime hosts: %s", runtime.web_hosts)
            await conversation_manager.detach_from_conversation(conversation)
