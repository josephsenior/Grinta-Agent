...
from openhands.server.session.session_manager import SessionManager
...
def get_prompt_optimizer(session: Session = Depends(SessionManager.get_session)) -> Any:
    ...
    if hasattr(session, 'orchestrator') and hasattr(session.orchestrator, 'prompt_optimizer'):
    ...
    try:
        ...
        for prompt_id in all_prompt_ids:
            ...
            if metrics:
                ...
                total_savings += best_metrics.avg_token_cost * 0.1  # Rough savings estimate
    ...
    try:
        ...
        for prompt_id in all_prompt_ids:
            ...
            if variants:
                metrics = tracker.get_all_metrics(prompt_id)
                if metrics:
                    best_variant = max(metrics.items(), key=lambda x: x[1].composite_score)
                    best_variant_id = best_variant[0]
                    best_score = best_variant[1].composite_score
            
            status_list.append(PromptOptimizationStatus(
                prompt_id=prompt_id,
...
@router.get("/prompts/{prompt_id}/metrics", response_model=Dict[str, PromptMetricsResponse])
