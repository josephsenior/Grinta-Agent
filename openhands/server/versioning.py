    ...
    if is_version_deprecated(version):
        ...
        if deprecation_info:
            ...
            if "migration_guide" in deprecation_info:
                response.headers["Link"] = f'<{deprecation_info["migration_guide"]}>; rel="successor-version"'
...
async def version_middleware(request: Request, call_next: Callable) -> Response:
    ...
    if request.url.path.startswith("/api/") and not version:
        # Allow non-versioned paths (health, socket.io, etc.)
        if not any(request.url.path.startswith(p) for p in ["/api/health", "/api/docs", "/api/openapi"]):
            return JSONResponse(
                status_code=400,
                content={
    ...
    if version:
        ...
        if is_version_deprecated(version):
            import logging
            logger = logging.getLogger("openhands.versioning")
            logger.warning(
...
def create_versioned_router(prefix: str, version: APIVersion = CURRENT_VERSION, **kwargs) -> APIRouter:
...
def api_route(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        func.__api_sunset_date__ = sunset_date
        func.__api_migration_guide__ = migration_guide
        return func
    return decorator


# Utility function for gradual migration
def supports_version(min_version: APIVersion, max_version: Optional[APIVersion] = None) -> bool:
