...
async def _replay_event_stream(
    ...
    logger.info(f"DEBUG: Replayed {event_count} events, agent_state_changed: {agent_state_changed is not None}")

    # Always send the current agent state when a client connects
    # This ensures the frontend gets the correct state even if it missed the transition events
    agent_state_sent = False
    
    if agent_state_changed:
        logger.info(f"DEBUG: Found agent state in event stream: {agent_state_changed.agent_state}")
        await sio.emit("oh_event", event_to_dict(agent_state_changed), to=connection_id)
        agent_state_sent = True
    else:
        # If no AgentStateChangedObservation was found in the event stream,
        # get the current agent state from the conversation manager
        try:
            ...
            if agent_loop_info_list and len(agent_loop_info_list) > 0:
                ...
                logger.warning(f"DEBUG: No agent_loop_info found for conversation {conversation_id}")
        except Exception as e:
            logger.warning(f"Failed to send current agent state to connection {connection_id}: {e}")
