...
class Session:
    ...
    def _extract_conversation_data(
        self,
        settings: Settings,
    ) -> tuple[dict | None, str | None, str | None, dict | None, str | None]:
    ...
    async def _start_agent_session(
        ...
        git_provider_tokens: dict | None,
        custom_secrets: dict | None,
    ...
    async def _run_metasop_orchestration(self, content: str) -> None:
        ...
        from openhands.metasop.clean_router import run_metasop_for_conversation
    ...
    async def _wait_for_client_connection(self) -> None:
        ...
        while (
            ...
            and not bool(self.sio.manager.rooms.get("/", {}).get(ROOM_KEY.format(sid=self.sid)))
        ):
            self.logger.warning(
