...
class GitHubReposMixin(GitHubMixinBase):
    ...
    def _parse_repository(self, repo: dict, link_header: str | None = None) -> Repository:
        ...
        return Repository(
            ...
            owner_type=(
                OwnerType.ORGANIZATION if repo.get("owner", {}).get("type") == "Organization" else OwnerType.USER
            ),
    ...
    def _build_public_search_params(self, query: str, params: dict) -> tuple[dict, bool]:
    ...
    async def _search_user_repositories(self, url: str, query: str, params: dict, user) -> list[dict]:
    ...
    async def _search_fuzzy_matched_orgs(self, url: str, query: str, params: dict, user_orgs: list) -> list[dict]:
    ...
    async def search_repositories(
        ...
        if public:
            ...
            org_repos = await self._search_organization_repositories(url, query, params, user_orgs)
            all_repos.extend(org_repos)

            # Search fuzzy matched organizations
            fuzzy_repos = await self._search_fuzzy_matched_orgs(url, query, params, user_orgs)
