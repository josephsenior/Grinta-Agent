...
"""
    ...
    def curate(self, insights: List[ACEInsight], current_playbook: ContextPlaybook,
               task_context: str, task_type: str = 'general', 
               role: str = None, expected_outcome: str = None) -> ACECurationResult:
        """
        ...
        try:
            ...
            # Get curation prompt
            prompt_template = self.curation_prompts.get(task_type, self.curation_prompts['general'])
            ...
            if task_type == 'appworld':
                prompt = prompt_template.format(
                    ...
                    final_generated_code=insights[0].reasoning if insights else "No generated code",
                ...
                prompt = prompt_template.format(
                    ...
                    current_playbook=playbook_content,
                    generated_attempt=insights[0].reasoning if insights else "No generated attempt",
            ...
            # Parse JSON response
            try:
                curation_data = json.loads(response)
            except json.JSONDecodeError:
                # Try to extract JSON from response
                json_start = response.find('{')
                json_end = response.rfind('}') + 1
                if json_start >= 0 and json_end > json_start:
                    curation_data = json.loads(response[json_start:json_end])
                else:
            ...
            delta_updates = self._convert_to_delta_updates(curation_data.get('operations', []))
            
            # Apply redundancy checking
            redundancy_removed = self._check_and_remove_redundancy(delta_updates, current_playbook)
            ...
            self.curation_metrics['total_tokens'] += estimated_tokens
            
            logger.debug(f"Curation completed with {len(delta_updates)} updates in {processing_time:.2f}s")
    ...
    def _convert_to_delta_updates(self, operations: List[Dict[str, Any]]) -> List[ACEDeltaUpdate]:
    ...
    def _check_and_remove_redundancy(self, delta_updates: List[ACEDeltaUpdate], 
                                   current_playbook: ContextPlaybook) -> int:
            ...
            if update.type == 'ADD' and update.content:
                ...
                for bullet_id, bullet in current_playbook.bullets.items():
                    if bullet.section == update.section:
                        ...
                        similarity = self._compute_similarity(update.content, bullet.content)
                        if similarity > 0.8:  # High similarity threshold
                            is_redundant = True
                            redundancy_removed += 1
                            logger.debug(f"Removed redundant update: {update.content[:50]}...")
                            break
    ...
    def curate_batch(self, insights_batch: List[List[ACEInsight]], 
                    current_playbook: ContextPlaybook,
                    task_contexts: List[str],
                    task_types: List[str] = None) -> List[ACECurationResult]:
        """Curate multiple batches of insights efficiently"""
        if task_types is None:
            task_types = ['general'] * len(insights_batch)
        
        results = []
        for i, (insights, context, task_type) in enumerate(zip(insights_batch, task_contexts, task_types)):
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.curation_metrics,
            ...
            'avg_updates_per_curation': self.curation_metrics['delta_updates_generated'] / total,
            'avg_redundancy_removed': self.curation_metrics['redundancy_removed'] / total
        }
