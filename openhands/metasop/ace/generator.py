...
class ACEGenerator:
    """
    Generates reasoning trajectories using context playbook
...
"""
    ...
    def generate(self, query: str, task_type: str = 'general', 
                role: str = None, max_retries: int = 3) -> ACEGenerationResult:
        """
        ...
        try:
            ...
            trajectory_content = self._generate_with_retries(prompt, max_retries)
            ...
            estimated_tokens = len(prompt.split()) + len(trajectory_content.split())
            self.generation_metrics['total_tokens'] += estimated_tokens
    ...
    def generate_with_feedback(self, query: str, previous_result: ACEGenerationResult, 
                             task_type: str = 'general', role: str = None) -> ACEGenerationResult:
        """
        ...
        start_time = time.time()
        try:
            trajectory_content = self._generate_with_retries(feedback_prompt, max_retries=3)
            ...
            processing_time = time.time() - start_time
            estimated_tokens = len(feedback_prompt.split()) + len(trajectory_content.split())
    ...
    def _generate_with_retries(self, prompt: str, max_retries: int) -> str:
        """Generate response with retry logic"""
    ...
    def _create_feedback_prompt(self, query: str, helpful_bullets: List, 
                               harmful_bullets: List, task_type: str, role: str = None) -> str:
