...
def corrective_hint(failure_type: str, meta: dict[str, Any]) -> str:
    mapping = {
    ...
    base = mapping.get(failure_type, "Address the underlying issue and retry.")
    if meta.get("error"):
        base += f" Error snippet: {meta['error'][:160]}"
    return base
