...
def _get_orchestrator_class():
    ...
    Returns:
        MetaSOPOrchestrator: The orchestrator class for MetaSOP operations.
    """
...
async def _setup_step_event_processing(
    ...
    """Set up step event processing for real-time updates.

    Args:
        conversation_id: The conversation ID for emitting events.
        orchestrator: The orchestrator to set up callbacks for.

    Returns:
        asyncio.Task: The event processor task.
    """
    step_event_queue = asyncio.Queue()
...
def _is_metasop_enabled() -> bool:
    ...
    return metasop_cfg and metasop_cfg.get("enabled", False)
...
def _calculate_metrics(report: dict) -> tuple[int, int, int]:
    """Calculate total tokens, duration, and retries from report."""
    total_tokens = 0
    total_duration = 0
    retries_sum = 0
