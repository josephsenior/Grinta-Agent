...
def _diff_sequence(label: str, observed: list[str]) -> list[str]:
    diffs: list[str] = []
    if not observed:
        diffs.append(f"{label}: empty sequence")
        return diffs
    if len(set(observed)) != len(observed):
        dupes = [x for x in observed if observed.count(x) > 1]
        diffs.append(f"{label}: duplicate entries detected: {sorted(set(dupes))}")
    return diffs
...
def _validate_step_hash_uniqueness(steps: list[dict[str, Any]]) -> list[str]:
    ...
    if len(hashes) != len(set(hashes)):
        ...
        diffs.append(f"duplicate step_hash values: {sorted(set(dupes))}")
