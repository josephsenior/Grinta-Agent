...
class MetricsRegistry:
    ...
    def _update_token_metrics(self, ev: dict[str, Any]) -> None:
        ...
        if isinstance(tokens, int):
            ...
            if model := (ev.get("model") or ev.get("model_name")):
                st.model_token_totals[model] = st.model_token_totals.get(model, 0) + tokens
    ...
    def _update_latency_metrics(self, ev: dict[str, Any], role: str | None) -> None:
        ...
        if (
            isinstance(dur, int)
            and dur >= 0
            and (ev.get("status") in {"executed", "executed_shaped", "failed", "timeout"})
        ):
            st = self._state
            ...
            st.step_duration_buckets[bucket] = st.step_duration_buckets.get(bucket, 0) + 1
    ...
    def _get_bucket(self, duration_ms: int) -> str:
        ...
        return next((f"le_{bucket}" for bucket in buckets if duration_ms <= bucket), "le_inf")
    ...
    def record_event(self, ev: dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            st.status_counts[status] = st.status_counts.get(status, 0) + 1
            self._update_status_metrics(status)
            self._update_token_metrics(ev)
            role = ev.get("role")
            self._update_latency_metrics(ev, role)
            self._update_cache_metrics(ev)
            self._update_retry_metrics(ev)
            if ev.get("reason") == "cache_hit" or ev.get("cache_hit"):
                st.context_reuse_total += 1
            else:
                st.context_unique += 1
            if ev.get("reason") == "diff_reuse" or ev.get("diff_fingerprint_reuse"):
                st.diff_reuse_total += 1
            else:
                st.diff_unique += 1
            if ev.get("reason") == "micro_iter_no_change":
                st.micro_iter_no_change_stops += 1
            elif ev.get("reason") == "micro_iter_loop_converged":
                st.micro_iter_loops_converged += 1
            elif ev.get("reason") == "micro_iter_loop_exhausted":
                st.micro_iter_loops_exhausted += 1
            st.micro_iter_events += 1 if ev.get("micro_iter") else 0
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            def _percentiles(buckets: dict[str, int]) -> dict[str, float] | None:
                ...
                ordered_bounds = sorted(
                    [
                        int(k.split("_")[1])
                        for k in buckets
                        if k.startswith("le_") and k not in {"le_inf"} and k.split("_")[1].isdigit()
                    ],
                )
                total = 0
                cumulative_map = {}
                ...
                if total == 0:
                    return None
                targets = {"p50": 0.5 * total, "p90": 0.9 * total, "p95": 0.95 * total, "p99": 0.99 * total}
                out: dict[str, float] = {}
            ...
            return {
                ...
                "avg_tokens_per_executed_step": st.total_tokens / st.steps_executed if st.steps_executed else None,
                ...
                "step_duration_histogram_by_role": {
                    role: {
                        ...
                        **({"percentiles": _percentiles(buckets)} if _percentiles(buckets) else {}),
                    }
...
class _MetricsHandler(BaseHTTPRequestHandler):
    ...
    def _build_basic_metrics(self, snap: dict) -> list[str]:
        ...
        body_lines = [
            ...
            f"metasop_total_events {
                snap['total_events']}",
        ]
        for status, count in snap["status_counts"].items():
            body_lines.extend(
                ("# TYPE metasop_status_count counter", f'metasop_status_count{{status="{status}"}} {count}'),
            )
        return body_lines
        ...
        if snap["avg_tokens_per_executed_step"] is not None:
            body_lines.extend(
                (
                    ...
                    f"metasop_avg_tokens_per_executed_step {
                        snap['avg_tokens_per_executed_step']}",
                ),
            )
        body_lines.extend(("# TYPE metasop_total_tokens counter", f"metasop_total_tokens {snap['total_tokens']}"))
        for model, tok in snap["model_token_totals"].items():
            body_lines.extend(
                ("# TYPE metasop_model_total_tokens counter", f'metasop_model_total_tokens{{model="{model}"}} {tok}'),
            )
        return body_lines
    ...
    def _build_histogram_percentiles(self, hist: dict) -> list[str]:
        ...
        return [
            f"metasop_step_duration_ms_{label} {pct[label]}" for label in ["p50", "p90", "p95", "p99"] if label in pct
        ]
    ...
    def _build_single_role_histogram(self, role: str, rbuckets: dict, meta: dict) -> list[str]:
        ...
        for b in ordered_bounds:
            ...
            lines.append(f'metasop_step_duration_ms_role_bucket{{role="{role}",le="{b}"}} {cumulative}')
    ...
    def _build_additional_metrics(self, snap: dict) -> list[str]:
        ...
        body_lines = [
            ...
            f"metasop_context_unique {
                snap.get(
                    'context_unique', 0)}",
            *(
                "# TYPE metasop_context_reuse_total counter",
                f"metasop_context_reuse_total {
                    snap.get(
                        'context_reuse_total', 0)}",
            ),
            *(
                "# TYPE metasop_diff_unique gauge",
                f"metasop_diff_unique {
                    snap.get(
                        'diff_unique', 0)}",
            ),
            *(
                "# TYPE metasop_diff_reuse_total counter",
                f"metasop_diff_reuse_total {
                    snap.get(
                        'diff_reuse_total', 0)}",
            ),
            *(
                "# TYPE metasop_micro_iter_events counter",
                f"metasop_micro_iter_events {
                    snap.get(
                        'micro_iter_events', 0)}",
            ),
            "# TYPE metasop_micro_iter_no_change_stops counter",
            f"metasop_micro_iter_no_change_stops {
                snap.get(
                    'micro_iter_no_change_stops', 0)}",
            "# TYPE metasop_micro_iter_loops_converged counter",
            f"metasop_micro_iter_loops_converged {
                snap.get(
                    'micro_iter_loops_converged', 0)}",
        ]
        body_lines.append("# TYPE metasop_micro_iter_loops_exhausted counter")
        body_lines.append(f"metasop_micro_iter_loops_exhausted {snap.get('micro_iter_loops_exhausted', 0)}")
