...
"Event emission utilities for MetaSOP.\n\nProvides a typed step event schema plus an emitter that validates and persists\nevents in-memory and (optionally) streams them to JSONL + logger.\n"
import contextlib
import json
import logging
import time
...
class EventEmitter:
    ...
    def __init__(self, config, sop_name: str) -> None:
        self._logger = logging.getLogger("openhands.metasop")
        ...
        try:
            ...
            if isinstance(metasop_cfg, dict) and metasop_cfg.get("log_events_jsonl"):
                base_dir = Path.home() / ".openhands" / "runs"
