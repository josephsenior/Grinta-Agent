...
class MultiObjectiveOptimizer:
    """
    Advanced multi-objective optimization engine that balances multiple
    ...
    def _calculate_innovation_score(
        ...
        creative_indicators = [
            ...
            'breakthrough', 'cutting-edge', 'advanced', 'sophisticated'
        ]
    ...
    def _apply_adaptation(
        ...
        for obj_weight in self.strategy.objectives:
            ...
            recent_scores = [h.get('objective_scores', {}).get(obj_name, 0) 
        ...
        return (1 - self.strategy.adaptation_rate) * base_score + \
               self.strategy.adaptation_rate * adapted_score
    ...
    def update_pareto_front(
        ...
        is_dominated = any(
            self._dominates(scores, objective_scores)
            for _, scores in self.pareto_front
        )
    ...
    def get_pareto_optimal_variants(self) -> List[Tuple[PromptVariant, Dict[str, float]]]:
        """Get all Pareto optimal variants."""
        return self.pareto_front.copy()

    def select_variant_for_exploration(
        self, 
        available_variants: List[PromptVariant],
        metrics: Dict[str, PromptMetrics]
    ) -> Optional[PromptVariant]:
        """
        Select variant for exploration (trying new approaches).
        ...
        for variant in available_variants:
            ...
            confidence_bound = math.sqrt(
                2 * math.log(sum(m.sample_count for m in metrics.values())) / 
