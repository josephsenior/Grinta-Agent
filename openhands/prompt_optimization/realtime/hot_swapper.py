...
from openhands.prompt_optimization.models import PromptVariant
...
class HotSwapper:
    ...
    async def hot_swap(
        ...
        if not await self._can_perform_swap(operation):
            return SwapResult(
                ...
                success=False,
                execution_time=0.0,
                error_message="Cannot perform swap - constraints not met"
            )
    ...
    async def _can_perform_swap(self, operation: SwapOperation) -> bool:
        ...
        from_variant = self.registry.get_variant(operation.prompt_id, operation.from_variant_id)
        to_variant = self.registry.get_variant(operation.prompt_id, operation.to_variant_id)
    ...
    async def _atomic_swap(self, operation: SwapOperation) -> SwapResult:
        ...
        try:
            ...
            # Perform atomic swap
            self.registry.set_active_variant(operation.prompt_id, operation.to_variant_id)
    ...
    async def _blue_green_swap(self, operation: SwapOperation) -> SwapResult:
        ...
        try:
            ...
            if not await self._perform_health_check(operation.prompt_id, operation.to_variant_id):
    ...
    async def _perform_health_check(self, prompt_id: str, variant_id: str) -> bool:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                health_checker(prompt_id, variant_id),
    ...
    async def _attempt_rollback(self, operation: SwapOperation) -> None:
        ...
        try:
            ...
            self.registry.set_active_variant(
                ...
                rollback_data['from_variant_id']
            )
            
            # Verify rollback
            current_variant = self.registry.get_active_variant(rollback_data['prompt_id'])
            if current_variant.id != rollback_data['from_variant_id']:
                logger.error(f"Rollback verification failed for {operation.operation_id}")
            else:
    ...
    def add_health_checker(self, prompt_id: str, checker: Callable) -> None:
    ...
    def get_swap_history(self, prompt_id: str = None) -> List[SwapResult]:
