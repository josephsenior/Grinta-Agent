...
class PromptOptimizer:
    ...
    def select_variant(self, prompt_id: str, category: PromptCategory) -> Optional[PromptVariant]:
    ...
    def _start_testing_best_variant(self, prompt_id: str):
        ...
        if available_variants:
            ...
            available_variants.sort(key=lambda v: v.composite_score, reverse=True)
            best_variant = available_variants[0]
            self.registry.add_testing_variant(prompt_id, best_variant.id)
    
    def _select_testing_variant(self, testing_variants: List[PromptVariant]) -> PromptVariant:
        """Select a variant from testing variants using weighted selection."""
        if not testing_variants:
            return testing_variants[0] if testing_variants else None
        
        # Weight selection by composite score
        scores = [v.composite_score for v in testing_variants]
    ...
    def record_execution(self, variant_id: str, prompt_id: str, category: PromptCategory,
        ...
        self.registry.update_variant_metrics(variant_id, success, execution_time, token_cost)
    ...
    def _should_switch_to_variant(self, current_variant: PromptVariant, 
                                 candidate_variant: PromptVariant) -> bool:
        """Check if we should switch to a candidate variant."""
        # Need minimum samples for both variants
        if (current_variant.total_executions < self.config.min_samples_for_switch or
            candidate_variant.total_executions < self.config.min_samples_for_switch):
            return False
    ...
    def _switch_to_variant(self, prompt_id: str, new_variant: PromptVariant):
        ...
        self._optimization_state[prompt_id].update({
            ...
            'switch_count': self._optimization_state[prompt_id].get('switch_count', 0) + 1
        })
    
    def add_variant(self, prompt_id: str, content: str, category: PromptCategory,
                   parent_id: Optional[str] = None, metadata: Optional[Dict] = None) -> str:
    ...
    def get_candidates_for_evolution(self, prompt_id: str) -> List[PromptVariant]:
        ...
        candidates.sort(key=lambda v: v.composite_score)
        
        return candidates
    
    def cleanup_old_variants(self, prompt_id: str, keep_count: int = None):
