...
class ToolOptimizer:
    ...
    def _create_optimized_tool(self, original_tool: ChatCompletionToolParam, 
                              variant: PromptVariant) -> ChatCompletionToolParam:
        ...
        # Create new tool with optimized description
        ...
        if 'parameters' in optimized_parts:
            ...
            if 'properties' in optimized_parts['parameters']:
                for param_name, param_desc in optimized_parts['parameters']['properties'].items():
                    if param_name in new_function.parameters.get('properties', {}):
                        new_function.parameters['properties'][param_name]['description'] = param_desc
        
        return ChatCompletionToolParam(
            type=original_tool.type,
        ...
        for tool_name, prompt_id in self.tool_prompt_ids.items():
            ...
            if prompt_metrics:
                summary[tool_name] = {
                    ...
                    'best_variant': prompt_metrics[0][0] if prompt_metrics else None,
                    'best_score': prompt_metrics[0][1].composite_score if prompt_metrics else 0.0
                }
