...
class RollbackManager:
    ...
    def __init__(
        ...
        Args:
            ...
            checkpoints_dir: Directory to store checkpoints (default: workspace_path/.openhands/checkpoints)
            max_checkpoints: Maximum number of checkpoints to keep
            auto_cleanup: Whether to automatically clean up old checkpoints
        """
        self.workspace_path = Path(workspace_path)
        self.checkpoints_dir = (
            Path(checkpoints_dir) if checkpoints_dir else self.workspace_path / ".openhands" / "checkpoints"
        )
    ...
    def _save_checkpoints(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save checkpoints manifest: {e}")
    ...
    def _create_git_snapshot(self) -> str | None:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "commit", "-m", "[OpenHands Checkpoint] Auto-snapshot", "--allow-empty"],
                check=False,
                cwd=self.workspace_path,
                capture_output=True,
                text=True,
            ...
            logger.warning(f"Failed to create git snapshot: {e}")
        ...
        try:
            ...
            for file_path in self.workspace_path.rglob("*"):
                if file_path.is_file():
                    # Skip .openhands directory and .git
                    if ".openhands" in file_path.parts or ".git" in file_path.parts:
                        continue
    ...
    def create_checkpoint(
        ...
        logger.info(f"Creating checkpoint: {description} (ID: {checkpoint_id})")
    ...
    def _try_git_rollback(self, checkpoint: Checkpoint) -> bool:
        ...
        if result.returncode == 0:
            logger.info(f"Git rollback successful to {checkpoint.git_commit_sha}")
            return True
    ...
    def _try_file_based_rollback(self, checkpoint_id: str) -> bool:
        ...
        if not snapshot_dir.exists():
            logger.error(f"Checkpoint snapshot directory not found: {snapshot_dir}")
            return False
    ...
    def _clear_workspace(self) -> None:
        """Clear workspace directory (except .openhands and .git)."""
        for item in self.workspace_path.iterdir():
            if item.name not in [".openhands", ".git"]:
                if item.is_dir():
                    shutil.rmtree(item)
                else:
                    item.unlink()
