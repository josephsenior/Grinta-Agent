...
class IssueResolver:
    ...
    def _initialize_basic_properties(
        ...
        # Update OpenHands configuration
    ...
    def update_sandbox_config(
        ...
        """Update OpenHands configuration with sandbox settings.

        Args:
            openhands_config: Configuration to update
            base_container_image: Base container image (if any)
            runtime_container_image: Runtime container image (if any)
            is_experimental: Whether in experimental mode

        Raises:
            ValueError: If both image types provided
        """
        if runtime_container_image and base_container_image:
            msg = "Cannot provide both runtime and base container images."
            raise ValueError(msg)

        runtime_container_image = cls._resolve_runtime_image(
        ...
        cls._apply_sandbox_config(openhands_config, sandbox_config)
    ...
    def _resolve_runtime_image(cls, runtime_img: str | None, base_img: str | None, is_experimental: bool) -> str | None:
        ...
        Args:
            runtime_img: Runtime image if specified
            base_img: Base image if specified
            is_experimental: Whether in experimental mode

        Returns:
            Resolved runtime image
        """
        if runtime_img or base_img or is_experimental:
            return runtime_img
        return f"ghcr.io/all-hands-ai/runtime:{openhands.__version__}-nikolaik"
    ...
    def _create_sandbox_config(cls, base_img: str | None, runtime_img: str | None) -> SandboxConfig:
        ...
        Args:
            base_img: Base container image
            runtime_img: Runtime container image

        Returns:
            Configured SandboxConfig
        """
        sandbox_config = SandboxConfig(
            base_container_image=str(base_img) if base_img else None,
            runtime_container_image=str(runtime_img) if runtime_img else None,
    ...
    @classmethod
    def _apply_sandbox_config(cls, openhands_config: OpenHandsConfig, sandbox_config: SandboxConfig) -> None:
        """Apply sandbox configuration to OpenHands config.
    ...
    def initialize_runtime(self, runtime: Runtime) -> None:
        ...
        if not isinstance(obs, CmdOutputObservation) or obs.exit_code != 0:
            msg = f"Failed to change directory to /workspace.\n{obs}"
    ...
    async def _run_agent(self, instruction: str, images_urls, conversation_instructions, runtime):
        ...
        try:
            ...
            error_msg = f"Agent failed with error: {e!s}"
    ...
    def _load_repo_instructions(self, repo_dir: str) -> None:
        ...
        if self.repo_instruction is None:
            ...
            if os.path.exists(openhands_instructions_path):
                with open(openhands_instructions_path, encoding="utf-8") as f:
