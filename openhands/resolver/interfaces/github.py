...
class GithubIssueHandler(IssueHandlerInterface):
    ...
    def __init__(
        self, owner: str, repo: str, token: str, username: str | None = None, base_domain: str = "github.com",
    ) -> None:
    ...
    def _filter_requested_issues(self, issue_numbers: list[int]) -> list[dict]:
        ...
        filtered = [issue for issue in all_issues if issue["number"] in issue_numbers and "pull_request" not in issue]
    ...
    def _convert_issues_to_details(self, all_issues: list[dict], comment_id: int | None) -> list[Issue]:
...
class GithubPRHandler(GithubIssueHandler):
    ...
    def download_pr_metadata(
        ...
        closing_issues_bodies, closing_issue_numbers = self._process_closing_issues(pr_data)
    ...
    def _extract_pr_data(self, response_json: dict) -> dict:
        ...
        return response_json.get("data", {}).get("repository", {}).get("pullRequest", {})
    ...
    def _process_review_threads(self, pr_data: dict, comment_id: int | None) -> tuple[list[ReviewThread], list[str]]:
        ...
        for thread in raw_review_threads:
            ...
            if not node.get("isResolved", True):
                ...
                message, files = self._process_thread_comments(my_review_threads, comment_id)
    ...
    def _process_thread_comments(self, my_review_threads: list, comment_id: int | None) -> tuple[str, list[str]]:
