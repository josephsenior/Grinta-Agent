...
def make_commit(
    ...
    if not status_result.stdout.strip():
        ...
        msg = "ERROR: Openhands failed to make code changes."
        raise RuntimeError(msg)
...
def _generate_comment_message(
    ...
    try:
        ...
        return (
            f"A new OpenHands update is available, but failed to parse or summarize the changes:\n{additional_message}"
        )
...
def _create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--git-user-name", type=str, default="openhands", help="Git user name for commits")
    parser.add_argument(
        "--git-user-email", type=str, default="openhands@all-hands.dev", help="Git user email for commits",
    )
    return parser
