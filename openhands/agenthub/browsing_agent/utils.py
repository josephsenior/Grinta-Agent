...
def _compress_chunks(
    ...
    for item, count in counter.items():
        if count > 1 and item not in skip_list and (len(item) > 10):
            def_dict[f"{identifier}-{item_id}"] = item
            item_id += 1
    compressed_text = "\n".join(text_list)
...
def compress_string(text: str) -> str:
    ...
    def_lines.extend((f"{key}:\n{value}" for key, value in def_dict.items()))
...
def _process_extracted_value(
    ...
    """Process an extracted HTML tag value."""
    if len(val) > 1:
        if not merge_multiple:
            retry_messages.append(f"Found multiple instances of the key {key}. You should have only one of them.")
        else:
            result_dict[key] = "\n".join(val)
    else:
...
def _validate_required_keys(
    ...
    for key in all_keys:
        if key not in content_dict and key not in optional_keys:
...
def parse_html_tags(
    ...
    for key in all_keys:
        if key in content_dict:
            _process_extracted_value(key, content_dict[key], merge_multiple, result_dict, retry_messages)

    _validate_required_keys(all_keys, optional_keys, content_dict, retry_messages)

    valid = not retry_messages
    retry_message = "\n".join(retry_messages)
