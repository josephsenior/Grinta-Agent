...
class FileCache:
    ...
    def __init__(
        ...
        if use_distributed and DISTRIBUTED_CACHE_AVAILABLE:
            try:
                ...
                if self.distributed_cache.enabled:
                    logger.info(f"📦 File cache using REDIS distributed backend (perfect for 1000+ users!)")
                else:
                    self.distributed_cache = None
        ...
        logger.info(f"📦 File cache initialized (mode={cache_mode}, max_size={max_cache_size}, ttl={ttl_seconds}s)")
