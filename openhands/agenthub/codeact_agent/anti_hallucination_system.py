...
class AntiHallucinationSystem:
    ...
    def should_enforce_tools(
        ...
        question_only_patterns = [
            r"^\s*why\s+", r"^\s*how does\s+", r"^\s*what is\s+", 
            r"^\s*explain\s+why\s+", r"^\s*tell me why\s+"
        ]
        ...
        action_patterns = [
            ...
            r"\bset\s+up\b", r"\bconfigure\b", r"\bdeploy\b", r"\brefactor\b",
            r"\brename\b", r"\bmove\b", r"\bcopy\b", r"\btest\b", r"\bcheck\b"
        ]
    ...
    def inject_verification_commands(
        ...
        """
        from openhands.events.action import FileEditAction, CmdRunAction
        ...
        for action in actions:
            ...
            # If this is a file operation, inject verification
            if isinstance(action, FileEditAction):
                file_path = action.path
                
                # Track this operation
                self.pending_file_operations.append(FileOperationContext(
                    operation_type="edit",
                ...
                # Inject verification command
                verification_cmd = CmdRunAction(
    ...
    def validate_response(
        ...
        if file_op_claims:
            ...
            has_file_edit = any(isinstance(a, FileEditAction) for a in actions)
    ...
    def cleanup_old_operations(self, current_turn: int, max_age: int = 3) -> None:
        ...
        self.pending_file_operations = [
            op for op in self.pending_file_operations
