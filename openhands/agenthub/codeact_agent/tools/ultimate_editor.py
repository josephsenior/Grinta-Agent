"""
Ultimate Editor - Unified Interface for Structure-Aware Editing
    ...
    def replace_code_range(
        ...
        try:
            ...
            if self.config.auto_indent:
                ...
                # Detect indentation of the line we're replacing
                if start_line <= len(lines):
                    base_indent = self.whitespace.get_line_indent(
                        lines[start_line - 1],
                        indent_config
                    )
                    new_code = self.whitespace.auto_indent_block(
                        new_code,
            ...
            if self.config.validate_syntax:
                ...
                if language:
                    ...
                    if not validation[0]:
                        return EditResult(
                            success=False,
                            message=f"Syntax error after edit: {validation[1]}",
                            syntax_valid=False,
            ...
            if self.config.clean_whitespace:
                language = self.universal.detect_language(file_path)
                cleaned = self.whitespace.clean_whitespace(new_content, language=language)
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(cleaned)
