...
class AtomicRefactor:
    ...
    def commit(
        ...
        try:
            ...
            for i, edit in enumerate(transaction.edits):
                try:
                    if edit.operation == "modify" or edit.operation == "create":
            ...
            logger.info(f"✅ Transaction {transaction.transaction_id} committed successfully ({len(applied_edits)} files)")
    ...
    def _rollback_edits(
        ...
        for edit in reversed(edits):  # Reverse order for proper rollback
            try:
