...
from openhands.events.action.files import FileEditAction, FileWriteAction
...
class HallucinationDetector:
            ...
            for match in matches:
                ...
                if not any(tool in tools_called for tool in file_tools):
                    ...
                    if not any(isinstance(a, FileEditAction) for a in actions):
                        logger.warning(f"Hallucination detected: Claimed '{claim}' without tool call")
                        return {
                            "type": "file_creation",
                            "claim": claim,
                            "confidence": 0.9,
    ...
    def _calculate_severity(self, hallucinations: list[dict]) -> str:
        ...
        has_file_hallucination = any(
            h["type"] in ("file_creation", "file_edit") 
