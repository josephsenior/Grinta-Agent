...
def _create_action_from_tool_call(tool_call, arguments: dict) -> Action:
    ...
    msg = f"Tool {
        tool_call.function.name} is not registered. (arguments: {arguments}). Please check the tool name and retry with an existing tool."
    raise FunctionCallNotExistsError(
        msg,
    )


def _process_tool_calls(assistant_msg, response: ModelResponse, thought: str) -> list[Action]:
