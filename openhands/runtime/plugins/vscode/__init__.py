...
class VSCodePlugin(Plugin):
    ...
    def _check_user_support(self, username: str) -> bool:
        ...
        if username not in ["root", "openhands"]:
            ...
            self.vscode_connection_token = None
            logger.warning(
                "VSCodePlugin is only supported for root or openhands user. It is not yet supported for other users (i.e., when running LocalRuntime).",
            )
            return False
        return True
    ...
    def _build_vscode_command(self, username: str, workspace_path: str, base_path_flag: str) -> str:
        ...
        return f"su - {username} -s /bin/bash << 'EOF'\nsudo chown -R {username}:{username} /openhands/.openvscode-server\ncd {workspace_path}\nexec /openhands/.openvscode-server/bin/openvscode-server --host 0.0.0.0 --connection-token {
