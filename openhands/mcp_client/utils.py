...
from openhands.mcp_client.cache import get_cached, set_cache
from openhands.mcp_client.client import MCPClient
from openhands.mcp_client.error_collector import mcp_error_collector
from openhands.mcp_client.wrappers import WRAPPER_TOOL_REGISTRY, wrapper_tool_params
...
async def _execute_wrapper_tool(action: MCPAction, mcp_clients: list[MCPClient]) -> MCPObservation:
    ...
    try:
        ...
        async def _call_underlying(tool_name: str, args: dict):
            ...
            return await _call_mcp_raw(mcp_clients, inner_action)
...
async def call_tool_mcp(mcp_clients: list[MCPClient], action: MCPAction) -> Observation:
    ...
    if action.name in WRAPPER_TOOL_REGISTRY:
        return await _execute_wrapper_tool(action, mcp_clients)

    # Handle direct tools
    matching_client = _find_matching_mcp_client(mcp_clients, action.name)
