...
class ShellConfigManager:
    ...
    ALIAS_PATTERNS = {
        "bash": ["^\\s*alias\\s+openhands\\s*=", "^\\s*alias\\s+oh\\s*="],
        "zsh": ["^\\s*alias\\s+openhands\\s*=", "^\\s*alias\\s+oh\\s*="],
        "fish": ["^\\s*alias\\s+openhands\\s*=", "^\\s*alias\\s+oh\\s*="],
        "powershell": ["^\\s*function\\s+openhands\\s*\\{", "^\\s*function\\s+oh\\s*\\{"],
    }

    def __init__(self, command: str = "uvx --python 3.12 --from openhands-ai openhands") -> None:
