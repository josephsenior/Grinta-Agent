...
from openhands.cli.tui import UserCancelledError, cli_confirm, kb_cancel
...
def _get_general_settings_labels(config: OpenHandsConfig) -> list[tuple[str, str]]:
...
def _get_current_values_for_modification_basic(config: OpenHandsConfig) -> tuple[str, str, str]:
    llm_config = config.get_llm_config()
    current_provider = ""
    current_model = ""
...
async def _select_model(
    ...
    default_model = _get_default_model_for_provider(provider, provider_models)
...
async def _get_api_key(session: PromptSession, config: OpenHandsConfig, provider: str, current_api_key: str) -> str:
...
async def modify_llm_settings_basic(config: OpenHandsConfig, settings_store: FileSettingsStore) -> None:
    ...
    try:
        ...
        if provider != current_provider:
            current_model = ""
            current_api_key = ""
        model = await _select_model(session, config, provider, organized_models, current_model)
...
async def _get_agent_input(session: PromptSession, config: OpenHandsConfig) -> str:
...
def _update_agent_condenser_config(config: OpenHandsConfig, llm_config, enable_memory_condensation: bool) -> None:
...
def _display_search_api_info(config: OpenHandsConfig) -> None:
    """Display information about search API configuration (DEPRECATED)."""
    print_formatted_text(HTML("\n<yellow>⚠️  DEPRECATED: Tavily Search API is deprecated.</yellow>"))
    print_formatted_text(HTML("<green>✅ Use DuckDuckGo MCP instead (FREE, unlimited, no API keys!).</green>"))
    print_formatted_text(HTML("<grey>Configure in config.toml: [mcp] stdio_servers with duckduckgo-mcp</grey>"))
    print_formatted_text("")


async def _get_new_search_api_key(session: PromptSession, config: OpenHandsConfig) -> str | None:
    """Prompt user for new search API key (DEPRECATED - returns None)."""
    # Tavily is deprecated, no longer accepting API key input
    print_formatted_text(HTML("<grey>Returning to settings menu...</grey>"))
    return None


async def _save_search_api_key(config: OpenHandsConfig, settings_store: FileSettingsStore, search_api_key: str) -> None:
...
async def modify_search_api_settings(config: OpenHandsConfig, settings_store: FileSettingsStore) -> None:
    """Modify search API settings."""
    session = PromptSession(key_bindings=kb_cancel(), style=get_cli_style())

    try:
        _display_search_api_info(config)
        search_api_key = await _get_new_search_api_key(session, config)
    ...
    await _save_search_api_key(config, settings_store, search_api_key)
