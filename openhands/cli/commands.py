...
from openhands.metasop.capability_audit import audit_capabilities
from openhands.metasop.registry import SOPS_DIR, load_role_profiles
from openhands.metasop.replay import ReplayError, replay_manifest
...
def restart_cli() -> None:
    ...
    try:
        ...
        print_formatted_text("Please restart OpenHands manually for changes to take effect.")
        ...
        try:
            with patch_stdout():
                response = await prompt_session.prompt_async(
                    HTML("<gold>Would you like to restart OpenHands now? (y/n): </gold>"),
                )
...
async def handle_commands(
    ...
    """Handle CLI commands with reduced complexity."""
    close_repl = False
    ...
    if res is not None:
        r_close_repl, r_reload_microagents, r_new_session_requested, _r_exit_reason = res
        if r_close_repl:
            close_repl = True
            exit_reason = ExitReason.INTENTIONAL
        reload_microagents = r_reload_microagents or reload_microagents
        new_session_requested = r_new_session_requested or new_session_requested
    elif command.startswith("/replay"):
        await _handle_replay_command(command)
        close_repl = False
    elif command.startswith("/capaudit"):
        await _handle_capaudit_command(command)
        close_repl = False
    else:
        close_repl = True
...
def _handle_simple_command(command, event_stream, usage_metrics, sid, config, current_dir, settings_store, agent_state):
    """Handle simple CLI commands with reduced complexity."""
    if command == "/help":
        return _handle_help_command()
    if command == "/status":
        return _handle_status_command(usage_metrics, sid)
    if command == "/new":
        return _handle_new_command(config, event_stream, usage_metrics, sid)
    if command == "/exit":
        return _handle_exit_command(config, event_stream, usage_metrics, sid)
    return None
...
async def handle_settings_command(config: OpenHandsConfig, settings_store: FileSettingsStore) -> None:
...
async def _collect_sse_server_inputs(config: OpenHandsConfig) -> tuple[str | None, str | None]:
...
async def _validate_and_create_sse_server(
    config: OpenHandsConfig, url: str, api_key: str | None,
...
async def _validate_stdio_server_name(config: OpenHandsConfig, name: str) -> bool:
...
async def _validate_stdio_server_inputs(
    config: OpenHandsConfig, name: str, command: str, args_input: str, env_input: str,
...
async def _collect_shttp_server_inputs(config: OpenHandsConfig) -> tuple[str | None, str | None]:
...
def _confirm_server_removal(config: OpenHandsConfig, server_type: str, identifier: str) -> bool:
