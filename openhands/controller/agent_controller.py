...
class AgentController:
    ...
    def _initialize_autonomy_and_validators(self, agent: Agent) -> None:
        ...
        Args:
            agent: Agent instance
        """
        from openhands.controller.autonomy import AutonomyController

        self.autonomy_controller = AutonomyController(agent.config)
        self._retry_count = 0
    ...
    def _initialize_safety_validator(self, agent: Agent) -> None:
        ...
        if hasattr(agent.config, "safety") and agent.config.safety.enable_mandatory_validation:
            from openhands.controller.safety_validator import SafetyValidator
    ...
    def _initialize_task_validator(self, agent: Agent) -> None:
        ...
        if hasattr(agent.config, "enable_circuit_breaker") and agent.config.enable_circuit_breaker:
            from openhands.controller.circuit_breaker import (
                CircuitBreaker,
    ...
    def _should_step_for_message_action(self, event: MessageAction) -> bool:
        ...
        if event.source == EventSource.USER:
            return True
        # CRITICAL FIX: Never step after agent's own messages
        # The agent has already responded - wait for user input
        return False

    def _should_step_for_observation(self, event: Observation) -> bool:
        """Determine if agent should step for observation events."""
        if isinstance(event, NullObservation):
            # ROBUST FIX: Check if this NullObservation is from a RecallAction (memory lookup)
            # If so, it's just informational (no results found) - don't step
            if event.cause is not None and event.cause > 0:
                # Check if the cause is a RecallAction by looking in history
                from openhands.events.action.agent import RecallAction
                for hist_event in reversed(self.state.history):
                    if getattr(hist_event, 'id', None) == event.cause:
                        if isinstance(hist_event, RecallAction):
                            logger.warning(f"🚫 Skipping step for NullObservation from RecallAction (memory found nothing)")
                            return False
                        break
                # Also check: if agent is awaiting user input, don't step on NullObservation
                # This prevents stepping after agent has already responded
                if self.get_agent_state() == AgentState.AWAITING_USER_INPUT:
                    logger.warning(f"🚫 Skipping step for NullObservation - agent already awaiting user input")
                    return False
            return event.cause is not None and event.cause > 0
        # CRITICAL FIX: RecallObservation should NOT trigger a step
        # It's just providing context from memory, not requiring a new response
        return not isinstance(event, (AgentStateChangedObservation, NullObservation, RecallObservation))
    ...
    async def _handle_runnable_action(self, action: Action) -> None:
        ...
        if self.safety_validator:
            from openhands.controller.safety_validator import ExecutionContext
            from openhands.events.observation import ErrorObservation
    ...
    def _get_initial_task(self):
        ...
        from openhands.validation.task_validator import Task
