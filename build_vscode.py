...
def _handle_node_version_check(vsix_path):
    ...
    logger.warning(
        "--- Warning: Node.js version < 18 detected or Node.js not found ---"
    )
...
def _install_npm_dependencies():
    ...
    if not npm_exe:
        logger.warning(
            "npm not found in PATH; skipping npm install for VS Code extension"
        )
        return
    subprocess.run(
        [npm_exe, "install"], cwd=VSCODE_EXTENSION_DIR, check=True, shell=False
    )
