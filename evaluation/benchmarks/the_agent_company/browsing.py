...
def parse_content_to_elements(content: str) -> dict[str, str]:
    ...
    for line in content.split("\n"):
        ...
        if not line:
            continue
        if anchor_match := re.match("\\[(\\d+)\\](.*)", line):
            if current_anchor and description_lines:
                elements[current_anchor] = " ".join(description_lines)
            current_anchor = anchor_match[1]
            description_lines = [anchor_match[2].strip()]
        elif current_anchor:
...
def resolve_action(action: BrowserAction, content: str) -> BrowserAction:
    ...
    if isinstance(action, (InputAction, ClickAction)) and (not action.selector.is_anchor):
        if anchor := find_matching_anchor(content, action.selector.value):
            ...
            logger.error("NO MATCH FOUND FOR SELECTOR, %s", action.selector)
            return None
    return action
...
def pre_login(runtime: Runtime, services: list[str], save_screenshots=True, screenshots_dir="screenshots"):
    ...
    plane_login_actions = [
        ...
        NoopAction(1000),
        InputAction("textbox 'Email', clickable, focused", "agent@company.com"),
        NoopAction(1000),
