...
import pandas as pd
from report_utils import check_coverage, check_mutation, count_methods, get_lines_of_code
...
from evaluation.benchmarks.testgeneval.metrics import bleu, edit_sim, exact_match, rouge_l
...
from evaluation.benchmarks.testgeneval.test_spec import TestGenEvalInstance, TestSpec, make_test_spec
...
def run_tests(
    runtime, instance, test_script, log_file="/tmp/test_output.log"
...
def process_instance(
    instance: pd.Series, metadata: EvalMetadata, reset_logger: bool = True, log_dir: str | None = None
) -> EvalOutput:
...
def _initialize_logging(reset_logger: bool, log_dir: str | None, instance_id: str) -> None:
...
def _execute_tests(runtime, instance: pd.Series, test_suite: str, test_spec: TestSpec) -> dict[str, Any]:
...
def _update_test_results(instance: pd.Series, test_results: dict[str, Any]) -> None:
    ...
    instance["test_result"]["report"].update({
        ...
        "coverage": test_results["coverage"] if test_results["coverage_success"] else 0,
        "test_stats": test_results["test_stats"],
    })


def _run_mutation_testing_if_applicable(runtime, instance: pd.Series, test_spec: TestSpec, test_results: dict[str, Any]) -> None:
    """Run mutation testing if conditions are met."""
    if not args.skip_mutation and test_results["coverage_success"] and test_results["test_stats"]["any_pass"] and (test_results["coverage"] > 0):
        _execute_mutation_testing(runtime, instance, test_spec, test_results["test_time"])


def _execute_mutation_testing(runtime, instance: pd.Series, test_spec: TestSpec, test_time: float) -> None:
    """Execute mutation testing."""
    mutation_timeout = max(10, 1.5 * test_time)
    mutation_toml = MUTATION_TEMPLATE.format(
        test_cmd=test_spec.test_cmd, source_fp=test_spec.code_file, timeout=mutation_timeout
    )
    ...
    mutation_code, mutation_output = run_mutation_testing(runtime, instance, "/tmp/mutation.sh")  # nosec B108
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mutation_buffer", type=int, default=MUTATION_BUFFER, help="Mutation buffer")
