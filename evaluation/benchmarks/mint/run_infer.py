...
AGENT_CLS_TO_FAKE_USER_RESPONSE_FN = {"CodeActAgent": codeact_user_response_mint}
...
def process_instance(instance: Any, metadata: EvalMetadata, reset_logger: bool = True):
    ...
    if "task_state" in state.extra_data:
        ...
        logger.info("Task state: %s", str(task_state.to_dict()))
    ...
    return EvalOutput(
        ...
        test_result={"success": task_state.success if task_state else False},
    )
...
if __name__ == "__main__":
    ...
    for subset in subsets:
        ...
        logger.info("Loaded MINT - %s subset", subset)
        _df = _cur_dataset.to_pandas().rename(columns={"id": "instance_id"})
        _df["instance_id"] = _df["instance_id"].apply(lambda x: f"{subset}/{x}")
        _df["in_context_example"] = in_context_example
        dataset_dfs.append(_df)
        logger.info("Loaded %s instances for subset: %s", len(_df), subset)
    dataset_df = pd.concat(dataset_dfs)
    logger.info("Loaded %s instances for subset: %s", len(dataset_df), subsets)
    llm_config = None
    if args.llm_config:
