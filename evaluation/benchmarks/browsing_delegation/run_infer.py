...
def process_instance(instance: pd.Series, metadata: EvalMetadata, reset_logger: bool = True) -> EvalOutput:
    ...
    instruction = f"""You can delegate browsing tasks to a browser agent. For example, for query 'Who is the president of the United States?', you can delegate the task to a browser agent via <execute_browse> Who is the president of the United States? </execute_browse>.
Now, solve the following query: "{instance.instruction}"
NOTE: You should copy the "query" as is into the <execute_browse> tag. DO NOT change ANYTHING in the query."""
