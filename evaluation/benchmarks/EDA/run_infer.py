...
def codeact_user_response_eda(state: State) -> str:
    ...
    game.curr_turn += 1
    logger.info("Model guess: %s", model_guess)
    logger.info("Answer response: %s", msg)
    return "/exit" if "bingo!" in msg.lower() else msg


AGENT_CLS_TO_FAKE_USER_RESPONSE_FN = {"CodeActAgent": codeact_user_response_eda}
...
def get_config(metadata: EvalMetadata) -> OpenHandsConfig:
    config = get_openhands_config_for_eval(metadata=metadata, runtime="docker")
...
def process_instance(instance: pd.Series, metadata: EvalMetadata, reset_logger: bool = True) -> EvalOutput:
    ...
    logger.info("Final message: %s | Ground truth: %s", final_message, instance["text"])
