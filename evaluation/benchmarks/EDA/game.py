...
class Q20Game:
    ...
    def __init__(
        ...
        self.first_user_utterance = "Your task is to ask a series of questions to deduce the entity that I'm thinking of with as few queries as possible. Only ask questions that can be answered by 'yes', 'no' or 'maybe'. Do not ask for hint. Make your question brief with no linebreaker. Now start asking a question."
    ...
    def preprocess_response(self, response):
        response = re.sub("the entity you are thinking of", "it", response)
        response = re.sub("the entity you're thinking of", "it", response)
        response = re.sub(" you're thinking of", "", response)
        response = re.sub(" you are thinking of", "", response)
        return response
    ...
    def answerer(self, question):
        ...
        if any(
            (
                re.search(f"(?:^|\\W){i.strip().lower()}(?:$|\\W)", question.lower())
