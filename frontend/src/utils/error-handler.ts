...
import { displayErrorToast, displayDetailedErrorToast } from "./custom-toast-handlers";
...
export function trackError({ message, source, metadata = {}, rawError }: ErrorDetails) {
  const error = rawError instanceof Error ? rawError : new Error(message);
  
  // Extract user-friendly error for better tracking
  const userFriendlyError = rawError ? extractUserFriendlyError(rawError) : null;
 ...
 */
export function showDetailedError(error: unknown, onAction?: (action: any) => void) {
  const userFriendlyError = extractUserFriendlyError(error) || formatClientError(error);
