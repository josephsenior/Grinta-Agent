...
export function useAutoNavigateToApp() {
  ...
  useEffect(() => {
    const handleServerReady = (event: CustomEvent<ServerReadyEvent>) => {
      ...
      console.warn(`[Auto-Navigate] 🔍 RECEIVED SERVER-READY EVENT:`, event.detail);
      
      // Only navigate if health check passed or is unknown (some servers take time to initialize)
      if (health_status === 'unhealthy') {
        console.warn(`[Auto-Navigate] Server at ${url} failed health check, skipping navigation`);
        return;
      }

      console.log(`[Auto-Navigate] Server ready at ${url}, navigating to browser tab`);
