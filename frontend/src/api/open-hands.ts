...
class OpenHands {
  ...
  static async getModels(): Promise<string[]> {
    const { data } = await openHands.get<string[]>(`${this.getBase()}/options/models`);
    return data;
  }
  ...
  static async getAgents(): Promise<string[]> {
    const { data } = await openHands.get<string[]>(`${this.getBase()}/options/agents`);
    return data;
  }
  ...
  static async authenticate(
    ...
    await openHands.post<AuthenticateResponse>(`${this.getBase()}/authenticate`);
    return true;
  }
  ...
  static async sendRawEvent(
    ...
    const { data } = await openHands.post<{ success: boolean; note?: string }>(
      url,
      text,
      {
        headers: {
  ...
  static async getSettings(): Promise<ApiSettings> {
    const { data } = await openHands.get<ApiSettings>(`${this.getBase()}/settings`);
    return data;
  }
  ...
  static async saveSettings(
    ...
    const data = await openHands.post(`${this.getBase()}/settings`, settings);
  ...
  static async getGitUser(): Promise<GitUser> {
    const response = await openHands.get<GitUser>(`${this.getBase()}/user/info`);
  ...
  static async getFile(conversationId: string, path: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (errorMsg.includes('directory') || errorMsg.includes('Directory')) {
          throw new Error('Cannot read directory as file');
        }
        
        if (errorMsg.includes('binary') || errorMsg.includes('Binary')) {
