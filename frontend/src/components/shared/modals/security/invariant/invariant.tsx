...
function SecurityInvariant() {
  ...
  const { refetch: exportTraces } = useGetTraces({
    onSuccess: (traces) => {
      ...
      const filename = `Forge-trace-${getFormattedDateTime()}.json`;
    ...
    logs: (
      <>
        ...
        <div
          ...
          ref={logsRef}
        >
          {logs.map((log: SecurityAnalyzerLog) => (
            <div
              key={String(log.id)}
    ...
    <div className="flex flex-1 w-full h-full">
      <div className="w-60 bg-base-secondary border-r border-r-neutral-600 p-4 flex-shrink-0">
        ...
        <ul className="space-y-2">
          <div
            className={`cursor-pointer p-2 rounded-sm ${activeSection === "logs" && "bg-background-tertiary"}`}
            onClick={() => setActiveSection("logs")}
          >
            {t(I18nKey.INVARIANT$LOG_LABEL)}
          </div>
          <div
            className={`cursor-pointer p-2 rounded-sm ${activeSection === "policy" && "bg-background-tertiary"}`}
            onClick={() => setActiveSection("policy")}
          >
            {t(I18nKey.INVARIANT$POLICY_LABEL)}
          </div>
          <div
            className={`cursor-pointer p-2 rounded-sm ${activeSection === "settings" && "bg-background-tertiary"}`}
