...
function useRetryCountdown(error: UserFriendlyErrorData) {
  const { retry_delay: retryDelay, can_retry: canRetry } = error;
  const [retryCountdown, setRetryCountdown] = React.useState<number | null>(
    null,
  );

  React.useEffect(() => {
    if (!retryDelay || !canRetry) {
...
function RetryActionContent({
  ...
  return (
    <span>
      <RefreshCw className="h-4 w-4 mr-1.5" />
      {t("ERROR$RETRY_IN", "Retry in {{countdown}}s", {
  ...
  return (
    <div className="pt-2 border-t border-border-subtle">
      ...
      >
        ...
        {t("ERROR$LEARN_MORE_DOCS", "Learn more in our documentation")}
  ...
  return (
    <div className="pt-3 border-t border-border-subtle">
      ...
      >
        ...
        <span>
          {t("ERROR$TECHNICAL_DETAILS", "Technical details {{state}}", {
            state: isExpanded
              ? t("ERROR$TECHNICAL_DETAILS_HIDE", "(hide)")
              : t("ERROR$TECHNICAL_DETAILS_SHOW", "(show)"),
          })}
        </span>
      </button>
      ...
      {isExpanded && (
        <div className="mt-3 p-3 rounded-lg bg-black/40 border border-border-subtle">
          <pre className="text-xs text-text-tertiary font-mono overflow-x-auto whitespace-pre-wrap">
            {technicalDetails}
          </pre>
          {timestamp && (
            <p className="text-xs text-text-muted mt-2">
              {t("ERROR$TIMESTAMP", "Timestamp: {{timestamp}}", {
