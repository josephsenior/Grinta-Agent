...
function AcceptanceCriteriaCard({
  ...
  const handleToggle = () => {
    ...
    onInteraction?.("criteria_toggle", { criteria, isCompleted: newCompleted });
  };
  ...
  return (
    <motion.div
      ...
      className={`bg-purple-500/5 border border-purple-500/20 rounded p-3 hover:bg-purple-500/10 transition-colors cursor-pointer ${
        isCompleted ? "opacity-60" : ""
      }`}
      onClick={() => onInteraction?.("criteria_click", criteria)}
    >
  ...
  return (
    <div className="space-y-3" key="user-stories">
      <h4 className="text-xs font-medium text-purple-300 uppercase tracking-wide flex items-center gap-2">
        ...
        {t(I18nKey.ORCHESTRATION$USER_STORIES, { count: userStories.length })}
      </h4>
...
const renderAcceptanceCriteriaSection = ({
  ...
  return (
    <div className="space-y-3" key="acceptance-criteria">
      <h4 className="text-xs font-medium text-purple-300 uppercase tracking-wide flex items-center gap-2">
        <CheckCircle className="w-4 h-4" />
        {t(I18nKey.ORCHESTRATION$ACCEPTANCE_CRITERIA, {
  ...
  return (
    <div className="space-y-3" key="success-metrics">
      <h4 className="text-xs font-medium text-purple-300 uppercase tracking-wide flex items-center gap-2">
        ...
        {t(I18nKey.ORCHESTRATION$SUCCESS_METRICS)}
      </h4>
          ...
          <div className="flex items-start justify-between">
            ...
            {metric.target && (
              <span className="text-xs text-purple-400 ml-2">
                {t(I18nKey.ORCHESTRATION$TARGET, { target: metric.target })}
              </span>
            )}
          </div>
          {metric.description && (
    ...
    <div className={`metasop-viz-pm ${className}`}>
      <div className="metasop-viz-header bg-purple-500/10 border-purple-500/20">
        ...
        <h3 className="text-sm font-semibold text-purple-300">
          {t(I18nKey.ORCHESTRATION$PRODUCT_MANAGER)}
        </h3>
        <span className="text-xs text-purple-400/60">
          {t(I18nKey.ORCHESTRATION$REQUIREMENTS_USER_STORIES)}
        </span>
      </div>
