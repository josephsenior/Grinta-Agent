...
// Type for potential future props (e.g., initialUrl, onNavigate, config)
            ...
            <span>
              {(() => {
                if (
                  ...
                  return "❌ Navigation failed - server may not be ready. The agent should wait for the server to start before navigating.";
                }
                if (controller.isLocalhostApp(currentUrl)) {
                  return "Agent app may not be running or blocked. Click 'Open in new tab' to check.";
                }
                return "This site may block iframe embedding. Click 'Open in new tab' if it doesn't load.";
              })()}
            </span>
          </div>
          <button
