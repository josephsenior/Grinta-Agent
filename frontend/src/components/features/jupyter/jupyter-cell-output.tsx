...
export function JupyterCellOutput({ lines }: JupyterCellOutputProps) {
  ...
  return (
    <div className="rounded-lg bg-background-tertiary dark:bg-background-secondary p-2 text-xs">
      <div className="mb-1 text-foreground-secondary">
        {t(I18nKey.JUPYTER$OUTPUT_LABEL)}
      </div>
      <pre
        className="scrollbar-custom scrollbar-thumb-gray-500 hover:scrollbar-thumb-gray-400 dark:scrollbar-thumb-white/10 dark:hover:scrollbar-thumb-white/20 overflow-auto px-5 max-h-[60vh] bg-background-tertiary"
      ...
      >
        ...
        {lines.map((line, index) => {
          if (line.type === "image") {
            ...
            return (
              <div
                key={`image-${index}-${String(line.url ?? line.content).slice(0, 30)}`}
              >
                <Markdown
                  components={{
                    p: paragraph,
          ...
          return (
            <div key={`line-${index}-${String(line.content).slice(0, 30)}`}>
