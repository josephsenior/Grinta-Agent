...
export function MicroagentManagementUpsertMicroagentModal({
  ...
  const { selectedRepository, selectedMicroagentItem } = useSelector(
  ...
  const modalTitle = useMemo(() => {
    ...
    if (selectedRepository) {
      return t(I18nKey.MICROAGENT_MANAGEMENT$ADD_A_MICROAGENT_TO, {
        defaultValue: `${t(I18nKey.MICROAGENT_MANAGEMENT$ADD_A_MICROAGENT_TO)} ${(selectedRepository as GitRepository).full_name}`,
      });
    }
