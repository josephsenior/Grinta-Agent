...
export const parseMessageFromEvent = (
  ...
  // Defensive: coerce null/undefined or literal "NULL" to empty string.
  const raw = isUserMessage(event) ? event.args.content : event.message;
  const m =
    raw == null || String(raw).toUpperCase() === "NULL" ? "" : String(raw);
