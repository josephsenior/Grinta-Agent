import React, { useEffect, useRef, useState } from "react";
import { Copy, Check, Terminal as TerminalIcon } from "lucide-react";
import { useSelector } from "react-redux";
import { RootState } from "#/store";
import { selectIsStreaming, selectStreamingEnabled, selectStreamContent } from "#/store/streaming-slice";

interface StreamingTerminalProps {
  // Allow either `eventId` (newer) or `streamId` (legacy tests/consumers)
  eventId?: string;
  streamId?: string;
  content?: string;
  exitCode?: number;
  command?: string;
  onComplete?: () => void;
  className?: string;
}

/**
 * StreamingTerminal - Displays terminal output with real-time streaming effect
 * 
 * Features:
 * - Character-by-character or chunk-based streaming
 * - Auto-scroll to bottom
 * - Copy button
 * - Syntax highlighting for commands
 * - Scrollable container (no truncation!)
 * - Blinking cursor while streaming
 */
export function StreamingTerminal({
  eventId,
  streamId,
  content,
  exitCode,
  command,
  onComplete,
}: StreamingTerminalProps) {
  // Tests and some callers pass `streamId` — prefer `eventId` when provided,
  // fallback to `streamId` for backward compatibility.
  const resolvedEventId = eventId || streamId || "";
  const [displayedContent, setDisplayedContent] = useState("");
  const [isCopied, setIsCopied] = useState(false);
  const [isExpanded, setIsExpanded] = useState(false);
  const terminalRef = useRef<HTMLDivElement>(null);
  const contentRef = useRef<HTMLDivElement>(null);
  
  const resolvedContentFromStore = useSelector((state: RootState) =>
    resolvedEventId ? selectStreamContent(state, resolvedEventId) : ""
  );

  const resolvedContent = content ?? resolvedContentFromStore;

  // Check if this stream is actively streaming
  const isStreaming = useSelector((state: RootState) =>
    resolvedEventId ? selectIsStreaming(state, resolvedEventId) : false
  );

  const streamingEnabled = useSelector((state: RootState) =>
    selectStreamingEnabled(state)
  );

  // Streaming effect
  useEffect(() => {
    // If streaming is disabled, show content immediately
    if (!streamingEnabled || !isStreaming) {
      setDisplayedContent(resolvedContent);
      return;
    }

    // Handle empty content
    if (!resolvedContent || resolvedContent.length === 0) {
      setDisplayedContent("");
      return;
    }

    // Streaming animation
    let currentIndex = 0;
    const chunkSize = 10; // characters per update for smooth streaming

    const interval = setInterval(() => {
      currentIndex += chunkSize;

      if (currentIndex >= resolvedContent.length) {
        setDisplayedContent(resolvedContent);
        clearInterval(interval);
        onComplete?.();
      } else {
        setDisplayedContent(resolvedContent.slice(0, currentIndex));
      }

      // Auto-scroll to bottom
      if (contentRef.current) {
        contentRef.current.scrollTop = contentRef.current.scrollHeight;
      }
    }, 16); // ~60fps for smooth animation

    return () => clearInterval(interval);
  }, [resolvedContent, isStreaming, streamingEnabled, onComplete]);
  
  // Handle copy to clipboard
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(displayedContent || "");
      setIsCopied(true);
      setTimeout(() => setIsCopied(false), 2000);
    } catch (error) {
      // Silently fail
      console.error("Failed to copy:", error);
    }
  };
  
  // Determine exit code styling
  const exitCodeColor = 
    exitCode === undefined ? "text-foreground-secondary" :
    exitCode === 0 ? "text-green-500" :
    "text-red-500";
  
  // Determine if output is long (should be collapsible)
  const lineCount = displayedContent.split("\n").length;
  const isLongOutput = lineCount > 8;
  
  // Calculate max height (bolt.new style: compact)
  const maxHeight = isExpanded ? "32rem" : "12rem"; // 192px collapsed, 512px expanded
  
  return (
    <div
      ref={terminalRef}
      className="streaming-terminal rounded-lg overflow-hidden border border-border/40 bg-[#0a0a0a] my-2 shadow-lg w-full"
    >
      {/* Terminal Header - Compact bolt.new style */}
      <div className="terminal-header flex items-center justify-between px-3 py-1.5 bg-[#1a1a1a] border-b border-border/30">
        <div className="flex items-center gap-2">
          <TerminalIcon className="w-3.5 h-3.5 text-green-500" />
          {command && (
            <>
              <span className="text-green-500 font-mono text-xs">$</span>
              <code className="text-xs text-foreground-secondary font-mono truncate max-w-[300px]">
                {command}
              </code>
            </>
          )}
          {exitCode !== undefined && (
            <span className={`text-xs font-mono ${exitCodeColor} ml-2`}>
              {exitCode === 0 ? "✓" : "✗"} {exitCode}
            </span>
          )}
          {lineCount > 0 && (
            <span className="text-[10px] text-foreground-secondary/50 ml-2">
              {lineCount} {lineCount === 1 ? "line" : "lines"}
            </span>
          )}
        </div>
        
        <div className="flex items-center gap-1">
          {/* Expand/Collapse for long outputs */}
          {isLongOutput && (
            <button
              type="button"
              onClick={() => setIsExpanded(!isExpanded)}
              className="px-2 py-0.5 text-[10px] text-foreground-secondary hover:text-foreground rounded transition-colors"
              title={isExpanded ? "Collapse" : "Expand"}
            >
              {isExpanded ? "Collapse" : "Expand"}
            </button>
          )}
          
          {/* Copy Button - More compact */}
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1 px-2 py-0.5 text-[10px] rounded transition-colors hover:bg-background-tertiary/50"
            title="Copy output"
          >
            {isCopied ? (
              <>
                <Check className="w-3 h-3 text-green-500" />
                <span className="text-green-500">Copied</span>
              </>
            ) : (
              <>
                <Copy className="w-3 h-3 text-foreground-secondary" />
                <span className="text-foreground-secondary">Copy</span>
              </>
            )}
          </button>
        </div>
      </div>
      
      {/* Terminal Content - Compact bolt.new style */}
      <div
        ref={contentRef}
        className="terminal-content px-3 py-2 overflow-y-auto font-mono text-xs leading-relaxed"
        style={{ 
          maxHeight,
          scrollbarGutter: "stable",
          transition: "max-height 0.2s ease-in-out"
        }}
      >
        <pre className="whitespace-pre-wrap break-words text-foreground-secondary/90">
          {displayedContent}
          {isStreaming && (
            <span className="inline-block w-1.5 h-3 bg-green-500 ml-1 animate-pulse" />
          )}
        </pre>
      </div>
    </div>
  );
}

