...
const backgrounds = [
  { name: "base", hex: "#0D0D0F" },
  { name: "base-secondary", hex: "#222328" },
];
...
results.sort((a, b) =>
  a.flag === b.flag ? b.count - a.count : a.flag ? -1 : 1,
);

console.log("Found", results.length, "unique hex colors.\n");
console.log(
  "Colors flagged (contrast <4.5 on any background) are listed first.\n",
);
for (const r of results) {
  const flag = r.flag ? "⚠️" : "   ";
  console.log(
    `${flag} ${r.hex} — uses: ${r.count} — contrast: base=${r.contrast.base}, base-secondary=${r.contrast["base-secondary"]} \n    sample files: ${r.files.join(", ")}`,
  );
}

// Write to JSON for further inspection
fs.writeFileSync(
  path.join(root, "contrast-scan-report.json"),
