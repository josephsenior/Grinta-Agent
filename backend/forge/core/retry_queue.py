    ...
    async def mark_failure(self, task: RetryTask, backoff_seconds: float) -> RetryTask | None:
        async with self._lock:
            ...
            if task.attempts >= task.max_attempts:
                ...
                self._tasks.pop(task.id, None)
                logger.warning(
                    "Retry task %s exceeded max attempts (%s)", task.id, task.max_attempts
                )
                return None
...
def get_retry_queue() -> RetryQueue | None:
    ...
    is_pytest = (
        ...
        or os.getenv("PYTEST_RUNNING", "").lower() in ("1", "true", "yes")
    )
    ...
    if backend_name == "redis" and REDIS_AVAILABLE:
        ...
        try:
            ...
            logger.warning("Failed to initialize Redis retry backend: %s. Falling back to in-memory.", exc)
