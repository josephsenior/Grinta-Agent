...
class TaskComplexityAnalyzer:
    ...
    ACTION_WORDS = [
        "create", "build", "implement", "develop", "add", "update", "fix",
        "modify", "edit", "delete", "remove", "refactor", "test", "verify",
        "validate", "check", "deploy", "configure", "set up", "install",
        "integrate", "connect", "write", "read", "run", "execute",
    ]
    ...
    def __init__(self, config) -> None:
        ...
        self._threshold = getattr(
            config, "planning_complexity_threshold", 3
        )
    ...
    def should_plan(self, user_message: str, state: "State") -> bool:
        ...
        should_plan = complexity >= threshold
        if should_plan:
            logger.info(
                f"📋 Task complexity {complexity:.1f} >= {threshold} - triggering automatic planning"
            )
        else:
            logger.debug(
                f"Task complexity {complexity:.1f} < {threshold} - skipping planning"
            )
    ...
    def estimate_iterations(self, complexity: float, state: "State") -> int:
        ...
        logger.debug(
            f"🎯 Estimated iterations: {estimated} (complexity: {complexity:.1f}, "
            f"range: {min_iter}-{max_iter})"
        )
