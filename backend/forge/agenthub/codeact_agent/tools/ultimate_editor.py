...
class UltimateEditor:
    ...
    def __init__(self, config: Optional[EditorConfig] = None):
        ...
        logger.info(
            f"   - Tree-sitter: {len(self.universal.get_supported_languages())} languages"
        )
    ...
    def edit_function(
        ...
        if self.config.auto_indent:
            try:
                ...
                indent_config = self.whitespace.detect_indent(
                ...
                new_body = self.whitespace.auto_indent_block(
                    new_body, base_indent=1, config=indent_config, language=language
                )
        ...
        if not result.success and "not found" in result.message.lower():
            ...
            try:
                ...
                suggestion = self.errors.symbol_not_found(
                    function_name, available_symbols
                )
                result.message += f"\n\n{suggestion.message}"
            except:
                pass
