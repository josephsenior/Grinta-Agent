...
from forge.cli.commands import check_folder_security_agreement, handle_commands
...
from forge.cli.utils import update_usage_metrics
from forge.cli.vscode_extension import attempt_vscode_extension_install
from forge.core.config import ForgeConfig, setup_config_from_args
...
from forge.io import read_task
...
class RuntimeComponents:
    ...
    memory: Any = None
    sid: str = ""
    is_loaded: asyncio.Event | None = None
    ...
    if initial_state is not None:
        ...
        if initial_state.last_error:
            ...
            if "ERROR_LLM_AUTHENTICATION" in error_message:
                ...
                if llm_config.model.startswith(("Openhands/", "openhands/", "Forge/")):
...
async def run_session(
    ...
    """Run an Forge session with the given configuration and parameters.
...
def run_alias_setup_flow(config: ForgeConfig) -> None:
    ...
    Prompts the user to set up aliases for 'forge' and 'oh' commands.
...
def _load_config_and_setup_vscode(args) -> ForgeConfig:
    ...
    config: ForgeConfig = setup_config_from_args(args)
...
async def _run_sessions(
    ...
    raw_skip_banner = getattr(args, "banner_shown", False)
    skip_banner_flag = raw_skip_banner if isinstance(raw_skip_banner, bool) else False
