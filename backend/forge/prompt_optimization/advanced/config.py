...
class AdvancedOptimizationConfig:
    ...
    default_objectives: list[OptimizationObjective] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        if not self.default_objectives:
    ...
    def get_hierarchical_strategy(
        self,
    ) -> dict[OptimizationLevel, LevelConfiguration]:
    ...
    def _validate_mode_consistency(self, errors: list[str]) -> None:
        ...
        if self.mode == AdvancedOptimizationMode.DISABLED:
            if any(
                [
                    self.multi_objective_enabled,
                    self.context_aware_enabled,
    ...
    def _validate_thresholds(self, errors: list[str]) -> None:
        ...
        if self.min_performance_threshold >= self.high_performance_threshold:
            errors.append(
                "Min performance threshold must be less than high performance threshold"
            )
    ...
    def from_dict(cls, data: dict[str, Any]) -> "AdvancedOptimizationConfig":
        ...
        config = cls(
            ...
            default_objectives=default_objectives_list,
