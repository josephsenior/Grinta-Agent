...
from forge.prompt_optimization.realtime.streaming_engine import StreamEventType
...
async def main():
    ...
    optimization_config = OptimizationConfig(storage_path="./realtime_optimization_data")
    storage = PromptStorage(
        config=optimization_config, registry=registry, tracker=tracker
    )

    # Create base optimizer
    base_optimizer = PromptOptimizer(
        registry=registry, tracker=tracker, config=optimization_config
    )
    ...
    rt_system = RealTimeOptimizationSystem(
        strategy_manager=strategy_manager, base_optimizer=base_optimizer, config=config
    )
    ...
    for prompt_data in test_prompts:
        ...
        for i in range(2, 5):
            ...
            registry.register_variant(alt_variant)
        ...
        for i in range(20):
            ...
            for prompt_data in test_prompts:
                ...
                tracker.record_execution(
                    ...
                    prompt_id=prompt_data["id"],
                    category=prompt_data["category"],
                    success=metrics.success_rate >= 0.75,
                    execution_time=metrics.avg_execution_time,
                    token_cost=metrics.avg_token_cost,
                    metadata={"sample_count": metrics.sample_count},
                )
            ...
            if i % 5 == 0:
                ...
                print(
                    f"  📊 Iteration {i}: {status['stats']['optimizations_performed']} optimizations, "
    ...
    print(
        f"  🎯 Total Optimizations: {dashboard_data['performance_summary']['optimizations_performed']}"
    )
    print(
        f"  🔀 Total Switches: {dashboard_data['performance_summary']['variants_switched']}"
    )
    print(
        f"  🚨 Total Alerts: {dashboard_data['performance_summary']['alerts_generated']}"
    )
    print(
        f"  📈 Events Processed: {dashboard_data['performance_summary']['events_processed']}"
    )
