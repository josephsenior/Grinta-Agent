...
class RealTimeMonitor:
    ...
    def __init__(
        ...
        # Metric storage
        self.metric_history: dict[str, deque[MetricSnapshot]] = defaultdict(
            lambda: deque(maxlen=max_history_size)
        )
        self.current_metrics: dict[str, dict[str, float]] = {}
        self.performance_trends: dict[str, dict[MetricType, PerformanceTrend]] = (
            defaultdict(dict)
        )
        ...
        self.alert_handlers: list[Callable[[MonitoringAlert], Any]] = []

        # Monitoring callbacks
        self.metric_callbacks: dict[str, list[Callable[[str, dict[str, float]], Any]]] = defaultdict(list)
        self.alert_callbacks: list[Callable[[MonitoringAlert], Any]] = []
    ...
    def start(
        self,
    ) -> None:  # pragma: no cover - requires continuous background monitoring
    ...
    async def _collect_current_metrics(
        self,
    ) -> None:  # pragma: no cover - depends on live metric sources
        ...
        for prompt_id in active_prompts:
            ...
            if metrics:
                # Store metrics
                variant_id = str(metrics.get("variant_id", "unknown"))
                snapshot_metrics = {
                    key: float(value)
                ...
                self.current_metrics[prompt_id] = {
                    key: float(value)
                    for key, value in metrics.items()
                    if isinstance(value, (int, float))
                }
    ...
    async def _simulate_metric_collection(
        self, prompt_id: str
    ...
    async def _analyze_metric_trend(
        self, prompt_id: str, metric_type: MetricType
        ...
        # Sort by timestamp
        metric_data.sort(key=lambda item: item["timestamp"])

        # Get recent data for trend analysis
        recent_data = metric_data[-self.trend_window_size :]
        values = [float(d["value"]) for d in recent_data]

        if len(values) < 5:
            return None

        # Calculate trend
        trend_direction, trend_strength, change_percentage = self._calculate_trend(
            values
        )
    ...
    def _calculate_trend_confidence(self, values: Sequence[float]) -> float:
    ...
    async def _check_metric_alert(
        self, prompt_id: str, metric_name: str, value: float
    ) -> None:
    ...
    async def _trigger_metric_callbacks(
        self, prompt_id: str, metrics: dict[str, float]
    ) -> None:
        """Trigger metric callbacks."""
        ...
        with self.monitoring_lock:
            ...
            if total_snapshots > 0:
                ...
                self.monitoring_stats["avg_processing_time"] = (
                    current_avg * (total_snapshots - 1) + processing_time
                ) / total_snapshots
    ...
    def add_metric_callback(
        self, prompt_id: str, callback: Callable[[str, dict[str, float]], Any]
    ) -> None:
    ...
    def add_alert_callback(
        self, callback: Callable[[MonitoringAlert], Any]
    ) -> None:
    ...
    def get_dashboard_data(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_alerts": [
                asdict(alert) for alert in self.get_alert_history(limit=10)
            ],
        }
