...
class WebSocketOptimizationServer:  # pragma: no cover - requires live websocket environment
    ...
    def __init__(
        ...
        self.message_handlers: dict[
            str, Callable[[WebSocketServerProtocol, dict[str, Any]], Awaitable[None]]
        ] = {
    ...
    async def start(
        self,
    ) -> None:  # pragma: no cover - requires active websocket server loop
        ...
        try:
            handler = cast(Any, self._handle_client)
            self.server = await websockets.serve(  # type: ignore[arg-type]
                handler,
                self.host,
                self.port,
    ...
    async def _handle_client(
    ...
    ) -> None:  # pragma: no cover - requires live websocket client
        ...
        try:
            ...
            async for message in websocket:
                ...
                await self._handle_message(websocket, message_text)
    ...
    async def _handle_trigger_optimization(
        ...
        try:
            ...
            await self._send_error(
                websocket, f"Failed to trigger optimization: {str(e)}"
            )
    ...
    async def _heartbeat_loop(
        self,
    ) -> None:  # pragma: no cover - long-running background task
        ...
        while self.is_running:
            try:
                heartbeat_message = {
                    ...
                    "timestamp": datetime.now().isoformat(),
                    "server_stats": asdict(self.server_stats),
                }
    ...
    def get_server_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "stats": asdict(self.server_stats),
        }
