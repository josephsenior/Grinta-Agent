...
from forge.runtime.action_execution_server import ActionExecutor
...
class LocalRuntimeInProcess(ActionExecutionClient):
    ...
    def __init__(
        ...
        # Initialize ToolRegistry for cross-platform detection
    ...
    async def connect(self) -> None:
        ...
        for plugin_req in self._plugin_requirements:
            if plugin_req.name in ALL_PLUGINS:
                ...
                logger.warning(f"Plugin {plugin_req.name} not found, skipping")
        ...
        self._executor = ActionExecutor(
            ...
            browsergym_eval_env=self.config.sandbox.browsergym_eval_env,
    ...
    def _setup_workspace_directory(self) -> None:
        ...
        if self._temp_workspace is None:
            ...
            logger.debug(f"Created workspace: {self._temp_workspace}")
    ...
    def list_files(self, path: str, recursive: bool = False) -> list[str]:
        ...
        try:
            ...
            logger.warning(f"Invalid path for list_files: {full_path} - {e}")
            return []
        ...
        try:
            ...
            logger.warning(f"Cannot list directory {full_path}: {e}")
            return []
        ...
        if self._temp_workspace and os.path.exists(self._temp_workspace):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    try:
                        logger.warning(f"Failed to remove workspace {self._temp_workspace} after {max_retries} attempts: {e}")
                    except Exception:
...
LocalRuntime = LocalRuntimeInProcess
