# Forge Kubernetes Runtime

This directory contains the Kubernetes runtime implementation for Forge, which allows the software to run on Kubernetes clusters for scalable and isolated execution environments.

## Local Development with KIND

For local development and testing, Forge provides a convenient setup using KIND (Kubernetes IN Docker) that creates a local Kubernetes cluster.
...
To use the Kubernetes runtime, you need to configure Forge properly. The configuration is done through a TOML configuration file.
...
Forge provides extensive configuration options for Kubernetes deployments under the `[kubernetes]` section. These options allow you to customize:
...
To set up and run Forge with the Kubernetes runtime locally:
...
2. **Create KIND Cluster**: Create a local Kubernetes cluster named "local-hands" using the configuration in `backend/kind/cluster.yaml`
...
2. Automatically start the Forge application using mirrord
