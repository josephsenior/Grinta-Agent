...
class ProcessManager:
    ...
    def register_process(
        ...
        logger.info(
            f"📝 Registered long-running process: {command[:80]} (process: {process_name}, ID: {command_id})"
        )
    ...
    async def cleanup_all(
        ...
        async with self._cleanup_lock:
            ...
            logger.info(
                f"🧹 Starting cleanup of {len(self._processes)} long-running processes"
            )
            results = {}
            ...
            logger.info(
                f"✅ Cleanup completed. Success: {sum(results.values())}/{len(results)}"
            )
            return results
...
def get_process_manager_health_snapshot(active_processes: list[ManagedProcess] | None = None) -> dict[str, Any]:
