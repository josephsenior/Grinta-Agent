...
from forge.runtime.pool import (
    ...
    WarmPoolPolicy,
    WarmRuntimePool,
    call_async_disconnect,
)
from forge.runtime.telemetry import RuntimeTelemetry, runtime_telemetry
...
class RuntimeOrchestrator:
    ...
    _IDLE_RECLAIM_SPIKE_THRESHOLD = 3
    _EVICTION_SPIKE_THRESHOLD = 1
    ...
    def _maybe_record_idle_reclaim_spike(self, key: str, total: int) -> None:
        ...
        if delta >= self._IDLE_RECLAIM_SPIKE_THRESHOLD:
    ...
    def _maybe_record_eviction_spike(self, key: str, total: int) -> None:
        ...
        delta = total - previous
        if delta >= self._EVICTION_SPIKE_THRESHOLD:
