...
class MetaSOPEventEmitter:
    ...
    def emit_step_start(
        self, step_id: str, role: str, metadata: Optional[Dict[str, Any]] = None
    ):
    ...
    def _validate_artifact(
        self, artifact: Dict[str, Any], role: str, step_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            is_valid, errors, warnings, suggestions = (
                validator.validate_with_suggestions(artifact, role)
            )

            # Log validation results
            if is_valid:
                logger.info(
                    f"✅ Artifact validation passed for {role} (step {step_id})"
                )
                if warnings:
                ...
                log_func(
                    f"❌ Artifact validation failed for {role} (step {step_id}): {len(errors)} errors"
                )
                for error in errors:
                    log_func(f"  • {error}")
...
def emit_step_event(
    ...
    if status in ["executed", "success", "completed"]:
        ...
        emitter.emit_step_failed(
            step_id, role, f"Step {step_id} failed", retries=retries
        )
    else:
...
def emit_artifact_event(
    conversation_id: str, step_id: str, artifact: Dict[str, Any], emit_callback=None
):
