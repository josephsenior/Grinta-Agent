...
class LearningStorage:
    ...
    def save_performance_history(self, history: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Saved performance history to {self.performance_history_path}"
            )
        except Exception as e:
        ...
        try:
            if self.performance_history_path.exists():
                with open(self.performance_history_path, "r") as f:
                    history = json.load(f)
                logger.debug(
                    f"Loaded performance history from {self.performance_history_path}"
                )
                return history
        except Exception as e:
    ...
    def get_learning_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "causal_patterns_count": len(
                    causal_patterns.get("conflict_patterns", {})
                ),
    ...
    def _calculate_storage_size(self) -> int:
        ...
        total_size = 0
        for path in [
            self.causal_patterns_path,
            self.parallel_stats_path,
