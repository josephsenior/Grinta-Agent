...
if TYPE_CHECKING:
    from forge.storage.knowledge_base.database_store_adapter import DatabaseStoreAdapter
...
class KnowledgeBaseStore:
    ...
    def _load_from_disk(self) -> None:
        ...
        try:
            if self.collections_file.exists():
                ...
                logger.info(f"Loaded {len(self._collections)} collections from disk")
            ...
            logger.error(f"Failed to load knowledge base data from disk: {e}")
    ...
    def _save_to_disk(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save knowledge base data to disk: {e}")
    ...
    def add_document(self, document: KnowledgeBaseDocument) -> KnowledgeBaseDocument:
        ...
        with self._lock:
            ...
            self._save_to_disk()
            logger.info(f"Added document: {document.filename} (ID: {document.id})")
            return document
...
def get_knowledge_base_store() -> KnowledgeBaseStore | Any:
    ...
    if _store is None:
        ...
        if storage_type in ("database", "db"):
            ...
            from forge.storage.knowledge_base.database_knowledge_base_store import (
                DatabaseKnowledgeBaseStore,
            )
            from forge.storage.knowledge_base.database_store_adapter import (
