"""Production-grade linter implementation with multiple backend support.

Provides advanced code linting capabilities using Tree-sitter and multiple
linter backends (ruff, pylint) with proper error formatting and reporting.
"""
...
class LintResult:
    ...
    def lint_file_diff(
        self, original_file: str, updated_file: str
    ) -> list[LintError]:
...
class DefaultLinter:
    """Production-grade code linter with multiple backend support.
    ...
    def _detect_best_backend(self) -> Optional[str]:
    ...
    def lint(self, file_path: Optional[str] = None, content: Optional[str] = None) -> LintResult:
        ...
        if self.enable_cache and cache_key:
            ...
            if cached_result is not None:
                self._cache_hits += 1
                logger.debug(f"Linter cache HIT for: {file_path or 'content'}")
    ...
    def _get_cache_key(self, file_path: Optional[str], content: Optional[str]) -> Optional[str]:
    ...
    def _get_from_cache(self, cache_key: str) -> Optional[LintResult]:
    ...
    def _lint_content(self, content: str, file_path: Optional[str] = None) -> LintResult:
    ...
    def _lint_with_ruff(self, file_path: str) -> LintResult:
        ...
        try:
            ...
            if result.returncode not in (0, 1):
                logger.warning(f"Ruff linting failed: {result.stderr}")
                return LintResult(errors=[], warnings=[])
            ...
            try:
                ...
                logger.warning(f"Failed to parse ruff JSON output: {result.stdout}")
    ...
    def _lint_with_pylint(self, file_path: str) -> LintResult:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to parse pylint JSON output: {result.stdout}")

        except subprocess.TimeoutExpired:
            logger.warning("Pylint linting timed out")
        except FileNotFoundError:
