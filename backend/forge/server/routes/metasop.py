...
def _format_artifacts_for_codeact(
    ...
    if pm_artifact := artifacts.get("pm_spec"):
        prompt_parts.extend(_format_pm_artifact(pm_artifact))

    if arch_artifact := artifacts.get("arch_design"):
        prompt_parts.extend(_format_arch_artifact(arch_artifact))

    if eng_artifact := artifacts.get("engineer_impl"):
        prompt_parts.extend(_format_engineer_artifact(eng_artifact))

    if ui_artifact := artifacts.get("ui_design"):
        prompt_parts.extend(_format_ui_artifact(ui_artifact))

    # Add final instructions
    prompt_parts.extend(_format_final_instructions(repo_root))
