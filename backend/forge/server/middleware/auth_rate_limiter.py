...
class AuthRateLimiter:
    ...
    async def __call__(
        ...
        if not await self._check_rate_limit(rate_limit_key, endpoint_type):
            logger.warning(
                f"Auth rate limit exceeded for {rate_limit_key} on {endpoint_type}"
            )
    ...
    async def _check_rate_limit(
        self, key: str, endpoint_type: str
    ) -> bool:
...
class RedisAuthRateLimiter(AuthRateLimiter):
    ...
    def __init__(
        ...
        if enabled:
            logger.info(
                f"RedisAuthRateLimiter initialized with redis_url: {self.redis_url}"
            )
    ...
    async def _check_rate_limit(
        ...
        try:
            ...
            await redis_client.zremrangebyscore(
                redis_key, 0, current_time - window
            )
