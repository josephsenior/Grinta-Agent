# Forge GitHub, GitLab & Bitbucket Issue Resolver 🙌
...
This tool allows you to use open-source AI agents based on [Forge](https://github.com/all-hands-ai/Forge)
...
3. Copy `examples/Forge-resolver.yml` to your repository's `.github/workflows/` directory
...
7. Usage:
   There are two ways to trigger the Forge agent:
   ...
   - The workflow will:
     1. Attempt to resolve the issue using Forge
   ...
   b. Using `@Forge-agent` mention:
   - Create a new comment containing `@Forge-agent` in any issue
...
Need help? Feel free to [open an issue](https://github.com/all-hands-ai/Forge/issues) or email us at [contact@all-hands.dev](mailto:contact@all-hands.dev).
...
Note: Forge works best with powerful models like Anthropic's Claude or OpenAI's GPT-4. While other models are supported, they may not perform as well for complex issue resolution.
...
```bash
python -m Forge.resolver.resolve_issue --selected-repo [OWNER]/[REPO] --issue-number [NUMBER]
```

For instance, if you want to resolve issue #100 in this repo, you would run:

```bash
python -m Forge.resolver.resolve_issue --selected-repo all-hands-ai/Forge --issue-number 100
```
...
```bash
poetry run python Forge/resolver/resolve_issue.py --selected-repo all-hands-ai/Forge --issue-number 100
```
...
```bash
python -m Forge.resolver.send_pull_request --issue-number PR_NUMBER --issue-type pr
```
...
```bash
python -m Forge.resolver.visualize_resolver_output --issue-number ISSUE_NUMBER --vis-method json
```
