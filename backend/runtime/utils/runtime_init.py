...
def _run_subprocess(command: list[str], *, log: bool = False) -> subprocess.CompletedProcess[bytes]:
    ...
    if log:
        ...
        logger.debug("Command %s returned %s – stdout: %s – stderr: %s", command, result.returncode, stdout, stderr)
    return result
...
def _add_passwordless_sudo_entry() -> None:
    ...
    if sudo_result.returncode != 0:
        ...
        logger.debug("Added sudoer successfully. Output: [%s]", sudo_result.stdout.decode().strip())
...
def _prepare_working_directory(username: str, initial_cwd: str) -> None:
    ...
    for cmd in (mkdir_cmd, chown_cmd, chmod_cmd):
        ...
        if result.returncode != 0:
            logger.warning("Command %s exited with %s: %s", cmd, result.returncode, result.stderr.decode().strip())
