...
if TYPE_CHECKING:
    from backend.core.config import ForgeConfig, SandboxConfig
...
class Runtime(
    ...
    def _verify_action_if_needed(
        ...
        try:
            ...
            verify_cmd = (
                f"python3 -c \"import os; print('VERIFIED:EXISTS' if os.path.isfile('{file_path}') else 'ERROR:MISSING')\""
            )
...
Please retry the file creation."""
                ...
                size_cmd = (
                    f"python3 -c \"print(sum(1 for _ in open('{file_path}')))\""
                )
