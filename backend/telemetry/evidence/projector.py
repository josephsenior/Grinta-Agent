...
def _blank() -> dict[str, Any]:
    return {
        'schema_version': 1,
        'run': {'duration_ms': None, 'final_agent_state': None, 'additional_user_inputs': None},
        'model': {'turns': 0, 'prompt_tokens': 0, 'completion_tokens': 0, 'reasoning_tokens': 0, 'cache_read_tokens': 0, 'cache_write_tokens': 0, 'cost_usd': 0.0},
        'tools': {'total': 0, 'successful': 0, 'failed': 0, 'blocked': 0, 'by_tool': {}},
        'verification_activity': {'test_runs': 0, 'last_test_exit_code': None},
        'workspace': {'files_read': [], 'files_changed': []},
        'reliability': {'control_interventions': 0, 'retries': 0, 'recoveries': 0, 'stuck_detections': 0, 'circuit_breaker_triggers': 0, 'context_compactions': 0, 'checkpoints_created': 0, 'restores': 0},
        'completion': {'finish_declared': False, 'task_recorded_status': None, 'completion_validator': None},
        'contract': {'total_criteria': 0, 'criteria_with_linked_evidence': 0, 'criteria_without_linked_evidence': 0, 'status_counts': {}},
    }
...
def project(source: Path | Iterable[dict[str, Any]], task_state: dict[str, Any] | None = None) -> dict[str, Any]:
    summary = _blank()
    seen: set[str] = set()
    ...
    for row in _records(source):
        ...
        if not isinstance(data, dict):
            continue
        try:
            timestamps.append(datetime.fromisoformat(str(row.get('ts')).replace('Z', '+00:00')))
        except ValueError:
            pass
        ...
        if kind == 'model_turn':
            ...
            summary['completion']['task_recorded_status'] = data.get('recorded_task_status')
    ...
    summary['run']['additional_user_inputs'] = max(0, user_inputs - 1) if user_inputs else 0
    if len(timestamps) >= 2:
        summary['run']['duration_ms'] = int((max(timestamps) - min(timestamps)).total_seconds() * 1000)
...
def _project_model(summary: dict[str, Any], data: dict[str, Any]) -> None:
    model = summary['model']
    model['turns'] += 1
    for key in ('prompt_tokens', 'completion_tokens', 'reasoning_tokens', 'cache_read_tokens', 'cache_write_tokens'):
        model[key] += int(data.get(key, 0) or 0)
    model['cost_usd'] += float(data.get('cost_usd', 0) or 0)


def _project_tool(summary: dict[str, Any], data: dict[str, Any], tools: Counter[str], files_read: set[str], files_changed: set[str]) -> None:
    ...
    files_read.update(value for value in data.get('read_paths', []) if isinstance(value, str))
    files_changed.update(value for value in data.get('changed_paths', []) if isinstance(value, str))
...
def _project_contract(summary: dict[str, Any], task_state: dict[str, Any] | None) -> None:
    if not isinstance(task_state, dict):
        return
    contract = task_state.get('contract') or {}
    items = [row for group in ('requirements', 'constraints', 'success_conditions') for row in contract.get(group, []) if isinstance(row, dict)]
    linked = sum(bool(row.get('evidence')) and any(item.get('source_event_ids') for item in row.get('evidence', []) if isinstance(item, dict)) for row in items)
    ...
    summary['contract']['status_counts'] = dict(Counter(str(row.get('status', 'unknown')) for row in items))
