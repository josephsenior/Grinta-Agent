...
EXPECTED_ROUTERS = {
    ...
    "metasop_router": "forge.server.routes.metasop",
    ...
    "prompts_router": "forge.server.routes.prompts",
    "prompt_optimization_router": "forge.server.routes.prompt_optimization",
    "snippets_router": "forge.server.routes.snippets",
...
def verify_router_imports() -> Dict[str, Tuple[bool, str]]:
    ...
    for router_name, module_path in EXPECTED_ROUTERS.items():
        try:
            ...
            if not has_router:
                results[router_name] = (False, f"Module {module_path} does not export 'router' or 'app'")
            else:
...
def verify_app_registration() -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        for router_name in EXPECTED_ROUTERS.keys():
            if f"include_router({router_name}" not in source and f"include_router({router_name.replace('_router', '')}" not in source:
