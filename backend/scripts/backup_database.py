...
async def create_backup():
    ...
    try:
        ...
        if result_sql.returncode != 0:
            print(f"WARNING: SQL backup failed (custom format backup succeeded)")
        ...
        print(f"  Format: Custom (compressed)")
...
def cleanup_old_backups(days: int = BACKUP_RETENTION_DAYS):
    ...
    old_backups = [
        b for b in backups
        if datetime.fromtimestamp(b.stat().st_mtime) < cutoff_date
    ]
...
async def main():
    ...
    parser.add_argument("--days", type=int, default=BACKUP_RETENTION_DAYS, help="Days to keep backups (default: 30)")
