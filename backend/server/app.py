...
from backend.server.shared import conversation_manager, server_config, get_conversation_manager
...
def _validate_config() -> None:
    ...
    # 2. Database URL sanity — if DB storage is enabled, asyncpg should be importable
    if os.getenv("KB_STORAGE_TYPE", "file").lower() in ("database", "db"):
        try:
            import asyncpg  # noqa: F401
        except ImportError:
            warnings.append(
                "KB_STORAGE_TYPE=database but 'asyncpg' is not installed. "
                "Install with: pip install asyncpg"
            )
        if not os.getenv("DATABASE_URL"):
            warnings.append(
                "KB_STORAGE_TYPE=database but DATABASE_URL is not set."
            )

    # 3. tmux (required by libtmux bash runtime)
    if not shutil.which("tmux"):
        warnings.append(
            "tmux is not installed or not on PATH. "
            "Some agent terminal features may not work."
        )
    ...
    if not Path("config.toml").exists():
        warnings.append(
            "No config.toml found. Copy config.template.toml → config.toml "
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    from backend.server.graceful_shutdown import register_shutdown_handler
    ...
    async with get_conversation_manager():
        ...
        from backend.server.graceful_shutdown import graceful_shutdown
...
from backend.server.middleware import LocalhostCORSMiddleware
...
from backend.server.middleware.request_id import RequestIDMiddleware

app.add_middleware(RequestIDMiddleware)

# 0.6. Request Tracing (add request IDs for debugging)
from backend.server.middleware.request_tracing import RequestTracingMiddleware
...
def get_effective_http_sample(route_path: str) -> float:
    ...
    try:
        ...
        logger.debug(f"Error matching custom sample rate: {e}")
    return _sample_http
...
metrics_enabled = os.getenv("REQUEST_METRICS_ENABLED", "false").lower() in ("true", "1", "yes")
if metrics_enabled:
    from backend.server.middleware import RequestMetricsMiddleware
...
if resource_quota_enabled:
    from backend.server.middleware.resource_quota import ResourceQuotaMiddleware
...
observability_enabled = os.getenv("OBSERVABILITY_ENABLED", "false").lower() in ("true", "1", "yes")
