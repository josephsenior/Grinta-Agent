...
class ResourceQuotaMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if is_excluded:
            logger.debug(f"Resource quota check skipped for excluded path: {path} (normalized: {normalized_path})")
            return await call_next(request)
    ...
    def _check_rate_limits(self, user_id: str, quota: ResourceQuota) -> Optional[JSONResponse]:
...
def check_conversation_quota(user_id: str, current_count: int) -> tuple[bool, Optional[str]]:
