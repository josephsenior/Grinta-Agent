...
from backend.server.static import SPAStaticFiles

# Import Socket.IO handlers to register them - this MUST be after sio is imported
try:
    import backend.server.listen_socket  # noqa: F401
...
if os.getenv("SERVE_FRONTEND", "true").lower() == "true":
    ...
    if os.path.isdir(build_dir):
        ...
        base_app.mount(
            "/locales",
            StaticFiles(directory=os.path.join(build_dir, "locales")),
            name="locales",
        )
        ...
        async def spa_fallback(request: Request, call_next):
            ...
            if os.path.commonpath([candidate_path, build_dir]) == os.path.normpath(
                build_dir
            ):
