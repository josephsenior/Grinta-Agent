...
from backend.server.services.event_query_service import get_contextual_events_text
...
from backend.server.utils import get_conversation as get_conversation_metadata
...
def _extract_request_data(
    ...
    Returns:
        Tuple containing in order:
            ...
            - create_playbook: Optional playbook creation parameters
...
def _determine_conversation_trigger(
    ...
    suggested task, playbook management, or remote API key). May override
...
async def new_conversation(
    ...
    try:
        ...
        # Only use provided conversation_id if ALLOW_SET_CONVERSATION_ID is enabled
...
async def search_conversations_route(
    ...
    conversation_trigger: ConversationTrigger | None = Query(
        None, 
        description="Filter by conversation trigger type"
    ),
):
    ...
    try:
        ...
        logger.error(f"Validation error in conversations endpoint: {e}")
        return error(
        ...
        logger.error(f"Error in conversations endpoint: {e}", exc_info=True)
        return error(
...
def generate_prompt_template(events: str) -> str:
    ...
    env = Environment(
        loader=FileSystemLoader(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "instruction", "prompts")), autoescape=True
    )
...
def _get_contextual_events(event_store: EventStore, event_id: int) -> str:
    ...
    # Delegate to service to keep routes thin and normalize replay failures.
    # Note: service includes `event_id` in the "after" window.
...
async def get_playbook_management_conversations(
    ...
    """Get conversations for the playbook management page with pagination support.

    This endpoint returns conversations with conversation_trigger = 'playbook_management'
    ...
    final_filtered_results = []
    for conversation in filtered_results:
        if conversation.trigger != ConversationTrigger.PLAYBOOK_MANAGEMENT:
            continue
