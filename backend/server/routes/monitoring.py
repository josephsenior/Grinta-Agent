...
async def event_stream_health():
    ...
    if int(stats.get("drops_per_minute", 0) or 0) >= int(
        thresholds["drops_per_minute_yellow"]
    ):
        warnings.append("event_stream_drops_rate_elevated")
    if int(stats.get("drops_per_minute", 0) or 0) >= int(
        thresholds["drops_per_minute_red"]
    ):
        warnings.append("event_stream_drops_rate_high")
    if int(stats.get("queue_utilization_pct_avg", 0) or 0) >= int(
        thresholds["queue_utilization_yellow"]
    ):
        warnings.append("event_stream_queue_utilization_elevated")
    if int(stats.get("queue_utilization_pct_avg", 0) or 0) >= int(
        thresholds["queue_utilization_red"]
    ):
