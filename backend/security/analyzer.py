...
from backend.events.action import Action, FileWriteAction, CmdRunAction
    ...
    def _assess_file_write(self, action: FileWriteAction) -> ActionSecurityRisk:
        ...
        for sensitive in _SENSITIVE_WRITE_PATHS:
            if sensitive.lower().replace("\\", "/") in path_lower:
                logger.warning(
                    "Security: write to sensitive path %s", action.path
                )
        ...
        if action.path.endswith(".py"):
            try:
                ...
                logger.warning(
                    "Security: syntax error in Python file %s", action.path
                )
                risk = max(risk, ActionSecurityRisk.HIGH)
            except Exception as exc:
                logger.warning(
                    "Security: could not parse %s: %s", action.path, exc
                )
                risk = max(risk, ActionSecurityRisk.MEDIUM)
