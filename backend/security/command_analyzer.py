...
_CRITICAL_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\brm\s+(-[a-zA-Z]*r[a-zA-Z]*f|--force\s+--recursive|-[a-zA-Z]*f[a-zA-Z]*r)\b", re.I), "recursive forced delete"),
    (re.compile(r"\brm\s+-[a-zA-Z]*r[a-zA-Z]*\s+/\s*$", re.I), "recursive delete on root"),
    ...
    (re.compile(r"\bsudo\s+(su|passwd|visudo|chmod\s+[0-7]*7[0-7]*\s+/)\b", re.I), "privilege escalation"),
    # Windows equivalents
    (re.compile(r"\bRemove-Item\b.*-Recurse.*-Force\b", re.I), "recursive forced delete (PowerShell)"),
...
_HIGH_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bcat\s+.*(/etc/passwd|/etc/shadow|\.ssh/|\.env|\.aws/credentials)", re.I), "credential file read"),
    (re.compile(r"\bexport\b.*(_KEY|_SECRET|_TOKEN|PASSWORD)\b", re.I), "secret export"),
    ...
    (re.compile(r"\bSet-ExecutionPolicy\s+Unrestricted\b", re.I), "execution policy bypass"),
...
_MEDIUM_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bdocker\s+run\b", re.I), "container execution"),
    ...
    (re.compile(r"\bInstall-Module\b|\bInstall-Package\b", re.I), "PowerShell package install"),
]
...
_RECOMMENDATIONS: dict[RiskCategory, list[str]] = {
    RiskCategory.CRITICAL: [
        ...
        "Consider running in a sandboxed / containerised environment.",
    ],
    RiskCategory.HIGH: [
...
__all__ = ["CommandAnalyzer", "RiskCategory"]
