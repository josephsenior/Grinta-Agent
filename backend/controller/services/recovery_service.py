...
class RecoveryService:
    ...
    async def react_to_exception(self, exc: Exception) -> None:
        ...
        controller.state.last_error = self._format_user_message(exc, error_type)
    ...
    async def _execute_recovery_actions(
        ...
        logger.info(
            f"Auto-recovery for {error_type}: attempt {self._retry_service.retry_count + 1}"
        )
        ...
        if not recovery_actions:
            logger.info(
                f"No recovery actions available for {error_type}, skipping retry to prevent infinite loop",
            )
            return
        ...
        if (
            ...
            if self._retry_service.retry_count > self._max_retries:
                logger.warning(
                    f"Reached maximum retry limit ({self._retry_service.retry_count}), stopping recovery attempt"
                )
