...
class ToolTelemetry:
    ...
    def _setup_prometheus_metrics(self) -> None:
        try:
            from prometheus_client import Counter as RuntimeCounter
            from prometheus_client import Histogram as RuntimeHistogram
        except Exception:  # pragma: no cover - dependency unavailable
            self._invocations = None
            self._latency = None
            return

        self._invocations = RuntimeCounter(
            "forge_tool_invocations_total",
            "Number of tool invocations processed by the agent controller",
            labelnames=("tool", "outcome"),
        )
        self._latency = RuntimeHistogram(
    ...
    def on_observe(self, ctx, observation) -> None:
        ...
        self._record(tool_name, outcome, duration, telemetry if isinstance(telemetry, dict) else None)
    ...
    def on_blocked(self, ctx, reason: str | None = None) -> None:
        ...
        self._record(tool_name, f"blocked:{outcome_reason}", duration, telemetry if isinstance(telemetry, dict) else None)
    ...
    def _elapsed_since(self, telemetry: Optional[Dict[str, Any]]) -> float:
        start = None
    ...
    def _record(
        self, tool: str, outcome: str, duration: float, telemetry: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def _action_to_schema(self, action: Any) -> Optional[ActionSchemaUnion]:
        ...
        try:
            ...
            if not schema_class:
                logger.debug(f"Unknown action type: {act_type}")
                return None
    ...
    def _observation_to_schema(self, observation: Any) -> Optional[ObservationSchemaUnion]:
        ...
        try:
            ...
            if not schema_class:
                logger.debug(f"Unknown observation type: {obs_type}")
                return None
    ...
    def _append_observation_fields(self, obs_dict: Dict[str, Any], observation: Any) -> None:
        optional_fields = ["command", "code", "path", "error_id", "image_urls", "hidden"]
        for field in optional_fields:
