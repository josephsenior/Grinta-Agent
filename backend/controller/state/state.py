...
class State:
    ...
    def _to_json_str(self) -> str:
        ...
        doc["metrics"] = metrics.get() if hasattr(metrics, "get") else None

        pms = data.get("parent_metrics_snapshot")
        doc["parent_metrics_snapshot"] = pms.get() if hasattr(pms, "get") else None

        return json.dumps(doc, separators=(",", ":"))

    @staticmethod
    def _from_raw(raw: str) -> State:
        """Detect format and deserialize: JSON (starts with '{') or legacy pickle/base64."""
        stripped = raw.strip()
    ...
    def _from_json_str(raw: str) -> State:
        ...
        if agent_state_val is not None:
            state.agent_state = AgentState(agent_state_val)
