...
if TYPE_CHECKING:
    from backend.controller.autonomy import AutonomyController
...
from backend.events import EventSource, EventStream, EventStreamSubscriber, RecallType
...
from backend.events.observation.agent import AgentThinkObservation, RecallObservation
...
class AgentController:
    ...
    async def _handle_finish_action(self, action: PlaybookFinishAction) -> None:
        ...
        if not await self.task_validation_service.handle_finish(action):
            return
        self.state.outputs = action.outputs
        await self.set_agent_state_to(AgentState.FINISHED)
        await self.log_task_audit(status="success")
    ...
    async def log_task_audit(
        ...
        try:
            from backend.server.shared import get_conversation_store
