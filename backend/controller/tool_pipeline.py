...
from backend.core.logger import LOG_ALL_EVENTS, forge_logger as logger
...
class ToolInvocationMiddleware:
    ...
    async def observe(
        self, ctx: ToolInvocationContext, observation: Optional["Observation"]
...
class SafetyValidatorMiddleware(ToolInvocationMiddleware):
    ...
    async def verify(self, ctx: ToolInvocationContext) -> None:
        ...
        context = ExecutionContext(
            ...
            is_autonomous=bool(
                getattr(self.controller.autonomy_controller, "autonomy_level", "") == "full"
            ),
        )
...
class PlanningMiddleware(ToolInvocationMiddleware):
    ...
    async def plan(self, ctx: ToolInvocationContext) -> None:
        ...
        if should_plan:
            ...
            logger.info(
                f"📋 Planning middleware: Task complexity {complexity:.1f} - "
...
class ReflectionMiddleware(ToolInvocationMiddleware):
    ...
    async def _verify_file_action(
        self, ctx: ToolInvocationContext, agent
    ) -> None:
        ...
        if path.endswith((".py", ".js", ".ts", ".json")):
            ...
            if path.endswith(".json") and content:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Reflection: Potential JSON syntax error in {path}"
                    )
                    # Don't block, but log warning

        logger.debug(f"✅ Reflection: File action verified for {path}")

    async def _verify_command_action(
        self, ctx: ToolInvocationContext, agent
    ) -> None:
        ...
        for pattern in destructive_patterns:
            if re.search(pattern, command):
                logger.warning(
                    f"⚠️ Reflection: Potentially destructive command detected: {command}"
                )
                # Don't block, but log warning (safety validator should handle this)

        logger.debug(f"✅ Reflection: Command action verified: {command}")
