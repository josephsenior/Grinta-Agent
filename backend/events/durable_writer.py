...
class DurableEventWriter:
    ...
    def stop(self, timeout: float = 2.0) -> None:
        ...
        try:
            ...
            logger.warning("DurableEventWriter: queue.join() failed during stop; pending events may be lost", exc_info=True)
        self._stop_flag.set()
        try:
    ...
    def enqueue(self, persisted_event: PersistedEvent) -> bool:
        ...
        try:
            ...
            self._drops += 1
            logger.warning(
                "DurableEventWriter queue full after %.1fs; dropped event id=%s "
                "filename=%s (total drops: %d)",
                self._put_timeout,
