...
def _handle_delegate_to_navigator(arguments: dict) -> Action:
    """Handle delegate_to_navigator tool call."""
    from backend.events.action import MessageAction
    return MessageAction(content=f"Delegation to Navigator with inputs {arguments} is not supported anymore.")
...
def _handle_ultimate_editor_tool(arguments: dict) -> Action:
    ...
    # Initialize editor
    try:
        from backend.engines.orchestrator.tools.ultimate_editor import UltimateEditor
...
def _create_tool_dispatch_map() -> dict[str, ToolHandler]:
    ...
    return {
        ...
        "delegate_to_navigator": _handle_delegate_to_navigator,
