...
class KnowledgeBaseManager:
    ...
    def _add_chunk_to_vector_store(
        self, vector_store: EnhancedVectorStore, chunk: DocumentChunk, document: KnowledgeBaseDocument, 
    ...
    def _add_chunks_to_vector_store(
        self, collection_id: str, chunks: list[DocumentChunk], 
        document: KnowledgeBaseDocument, filename: str
    ) -> int:
        ...
        try:
            ...
            if chunks_added < len(chunks):
                logger.warning(
                    f"Only {chunks_added}/{len(chunks)} chunks added to vector store. "
                    f"Failed chunks: {failed_chunks}"
                )

            if chunks_added == 0:
                logger.error(
                    f"Failed to add any chunks to vector store for document {document.id}. "
                    f"Document exists but is not searchable."
                )
            else:
                logger.info(
                    f"Added document '{filename}' to collection {collection_id} "
                    f"({chunks_added}/{len(chunks)} chunks)"
                )
        except Exception as e:
    ...
    def delete_collection(self, collection_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted {deleted_count} vector chunks for collection {collection_id}"
            )
        except Exception as e:
            logger.error(f"Failed to delete vectors for collection {collection_id}: {e}")
    ...
    def add_document(
        ...
        if chunks_added < len(chunks):
            logger.warning(
                f"Document '{filename}' stored but vector chunks may be incomplete. "
                f"Consider re-uploading the document."
            )
    ...
    def delete_document(self, document_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted {deleted_count} vector chunks for document {document_id}"
            )
        except Exception as e:
