...
# Global factory helper to maintain backward compatibility with current ad-hoc instantiation
...
class DatabaseKnowledgeBaseStore:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.critical(f"Failed to initialize Knowledge Base database schema: {e}")
            raise
    ...
    async def add_document(self, document: KnowledgeBaseDocument) -> KnowledgeBaseDocument:
        ...
        logger.info(f"Added document: {document.filename} ({document.id})")
        return document
    ...
    def _row_to_document(self, row: asyncpg.Record) -> KnowledgeBaseDocument:
        return KnowledgeBaseDocument(
            ...
            uploaded_at=row["uploaded_at"].replace(tzinfo=timezone.utc) if row["uploaded_at"].tzinfo is None else row["uploaded_at"],
        )
