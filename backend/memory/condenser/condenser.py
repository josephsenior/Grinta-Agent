...
class Condenser(ABC):
    ...
    def write_metadata(self, state: State) -> None:
        ...
        if len(meta_list) > MAX_CONDENSER_META_BATCHES:
            state.extra_data[CONDENSER_METADATA_KEY] = meta_list[-MAX_CONDENSER_META_BATCHES:]
...
class RollingCondenser(Condenser, ABC):
    ...
    def condense(self, view: View) -> View | Condensation:
        """Return condensed view if needed, otherwise original view."""
        return self.get_condensation(view) if self.should_condense(view) else view
