...
from backend.events.event import Event, RecallType, EventSource
...
class ConversationMemory:
    ...
    def _handle_agent_finish_action(self, action: PlaybookFinishAction) -> list[Message]:
        ...
        Args:
            action: The PlaybookFinishAction containing the agent's final thought
        ...
        Example:
            >>> finish_action = PlaybookFinishAction(thought="Task complete", source="agent")
    ...
    def _merge_tool_metadata_thought(self, action: PlaybookFinishAction) -> None:
    ...
    def _process_action(
        ...
        Args:
            action: The action to convert. Can be one of:
                ...
                - PlaybookFinishAction: For ending the interaction
        ...
        if self._is_instance_of(action, PlaybookFinishAction):
            return self._handle_agent_finish_action(cast(PlaybookFinishAction, action))
    ...
    def _process_workspace_context_recall(
        ...
        filtered_agents = self._filter_playbooks(obs)
    ...
    def _filter_playbooks(self, obs: RecallObservation) -> list[PlaybookKnowledge]:
        """Filter playbooks based on disabled list."""
        if not obs.playbook_knowledge:
            return []
        return [
            agent
            for agent in obs.playbook_knowledge
            if agent.name not in self.agent_config.disabled_playbooks
        ]
    ...
    def _build_message_content(
        ...
        if filtered_agents:
            formatted_playbook_text = self.prompt_manager.build_playbook_info(
    ...
    def _process_knowledge_recall(
        ...
        filtered_agents = self._filter_agents_in_playbook_obs(
        ...
        if filtered_agents:
            formatted_parts.append(
                self.prompt_manager.build_playbook_info(
                    triggered_agents=filtered_agents
                )
            )
    ...
    def _filter_agents_in_playbook_obs(
        self,
        obs: RecallObservation,
        current_index: int,
        events: list[Event],
        ...
        Returns:
            list[PlaybookKnowledge]: The filtered list of playbook knowledge
