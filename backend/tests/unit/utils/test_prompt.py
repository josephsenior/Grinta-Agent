"""Tests for backend.utils.prompt — dataclasses, PromptManager, and prompt builder."""
...
class TestOrchestratorPromptManager:
    ...
    def test_mcp_tools_and_server_hints_are_rendered(self, tmp_path):
        ...
        mock_config.enable_checkpoints = False
        mock_config.enable_permissions = False
        # Mark the catalogue as non-empty so the per-tool description
        # below ("route calls through ``call_mcp_tool(tool_name=...)``")
        # is the one the renderer picks.
        mock_config.mcp_capability_status = {
            'state': 'healthy',
        ...
        # The system prompt must NOT list concrete MCP tool names —
        # those are reserved for the per-turn addendum so the provider
...
# ── PromptManager workspace context (extended) ─────────────────────────


class TestPromptManagerWorkspaceContext:
    @pytest.fixture
    ...
    def test_build_workspace_context_includes_project_path_guidance(self, pm):
        from backend.utils.prompt import RuntimeInfo
    ...
    def test_build_workspace_context_notes_bare_workspace(self, pm):
        from backend.utils.prompt import RuntimeInfo
    ...
    def test_build_workspace_context_omits_bare_note_with_repo_instructions(self, pm):
        from backend.utils.prompt import RuntimeInfo
...
# ── OrchestratorPromptManager internals (extended) ─────────────────────


class TestOrchestratorPromptManagerExtended:
    @pytest.fixture
    ...
    def test_set_prompt_tier_debug(self, prompt_dir):
        from backend.utils.prompt import OrchestratorPromptManager
...
def test_python_shell_command_prefers_python3_in_bash_mode():
    from backend.utils.terminal.terminal_contract import get_python_shell_command
