...
def test_get_user_settings_store():
    ...
    with patch(
        'backend.persistence.auth.get_app_settings_root',
        return_value='/tmp/test_settings',
    ):
        with patch('backend.persistence.auth.get_file_store') as mock_get_file_store:
            ...
            mock_get_file_store.assert_called_once_with(
                'local', local_data_root='/tmp/test_settings'
            )
...
def test_get_user_secret_store():
    ...
    with patch(
        ...
        with patch('backend.persistence.auth.get_file_store') as mock_get_file_store:
            ...
            mock_get_file_store.assert_called_once_with(
                'local', local_data_root='/tmp/test_secrets'
            )
...
async def test_get_user_settings():
    ...
    with patch(
        'backend.persistence.auth.get_user_settings_store', return_value=mock_store
    ):
...
async def test_get_user_secrets():
    ...
    with patch(
        'backend.persistence.auth.get_user_secret_store', return_value=mock_store
    ):
