...
class TestSqliteEventStoreExtendedCoverage:
    ...
    def test_destructive_delete_single_event_enabled(
        self, store: SQLiteEventStore
    ) -> None:
        ...
        with patch.dict(
            os.environ, {'GRINTA_SQLITE_LEDGER_ALLOW_DESTRUCTIVE_DELETE': 'true'}
        ):
            store.delete_event(0)
    ...
    def test_destructive_delete_event_exception_rolls_back(
        self, store: SQLiteEventStore
    ) -> None:
    ...
    def test_destructive_delete_from_exception_rolls_back(
        self, store: SQLiteEventStore
    ) -> None:
    ...
    def test_read_event_payload_id_mismatch(self, store: SQLiteEventStore) -> None:
        ...
        conn.execute(
            'UPDATE events SET payload = ? WHERE id = 0', (json.dumps(payload_data),)
        )
        conn.commit()
        conn.close()
    ...
    def test_check_application_integrity_order_violation(
        self, store: SQLiteEventStore
    ) -> None:
    ...
    def test_check_application_integrity_id_mismatch(
        self, store: SQLiteEventStore
    ) -> None:
    ...
    def test_check_application_integrity_checksum_mismatch(
        self, store: SQLiteEventStore
    ) -> None:
        ...
        conn.execute(
            ...
            (
                ...
                'test',
                json.dumps(
                    {'id': 0, 'action': 'test', '_grinta_checksum': 'wrong_checksum'}
                ),
                time.time(),
            ),
        )
    ...
    def test_check_application_integrity_exception(
        self, store: SQLiteEventStore
    ) -> None:
