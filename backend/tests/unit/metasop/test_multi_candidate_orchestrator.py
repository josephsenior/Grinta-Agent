...
def make_template_with_engineer_step():
...
class DummyExecutor(BaseStepExecutor):
    ...
    def execute(
        ...
        artifact = Artifact(
            step_id=step.id, role=step.role, content={"candidates": self.candidates}
        )
        return StepResult(ok=True, artifact=artifact)


def test_orchestrator_selects_best_candidate(tmp_path):
    ...
    orch.template = type("T", (), {"steps": make_template_with_engineer_step()})
    candidates = [
    ...
    orch.settings.enable_micro_iterations = (
        False  # Disable micro-iterations to avoid validation issues
    )
    ...
    art = done["eng_step"]
    content_text = (
        json.dumps(art.content) if isinstance(art.content, dict) else str(art.content)
    )
