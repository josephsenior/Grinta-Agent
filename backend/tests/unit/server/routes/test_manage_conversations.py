...
class DummyConversationManager:
    ...
    async def get_agent_loop_info(self, filter_to_sids=None, **kwargs):  # noqa: ANN001
        return [
            info
            for info in self.agent_loop_info
            if info.conversation_id in (filter_to_sids or set())
        ]
...
def test_filter_conversations_by_age_filters_old_entries():
    ...
    conversations: list[ConversationMetadata] = [
        ...
        cast(ConversationMetadata, SimpleNamespace()),
    ]
...
@pytest.mark.asyncio
async def test_build_conversation_result_set_maps_connections(
    ...
    patch_conversation_manager.agent_loop_info = [
        SimpleNamespace(conversation_id="c1", status=ConversationStatus.RUNNING)
    ]

    async def fake_get_conversation_info(
        conversation, num_connections, agent_loop_info
    ):
        return SimpleNamespace(
            conversation_id=conversation.conversation_id,
            num_connections=num_connections,
            status=agent_loop_info.status,
        )
...
def test_determine_conversation_trigger_priority():
    trigger, repo, provider = mc._determine_conversation_trigger(
        None,
        CreateMicroagent(repo="r", git_provider=ProviderType.GITHUB),
        None,
    )
    ...
    trigger, repo, provider = mc._determine_conversation_trigger(
        suggested_task, None, mc.AuthType.BEARER
    )
...
def test_validate_remote_api_request_handles_missing_message():
    ...
    assert mc._validate_remote_api_request(ConversationTrigger.GUI, "") is None
...
async def test_verify_repository_access_skips_none(monkeypatch):
    ...
    tokens = MappingProxyType(
        {ProviderType.GITHUB: ProviderToken(token=SecretStr("x"))}
    )
    await mc._verify_repository_access("owner/repo", ProviderType.GITHUB, tokens)
    assert called["repository"] == "owner/repo"
    await mc._verify_repository_access(None, ProviderType.GITHUB, tokens)
...
def test_normalize_provider_tokens_handles_strings():
    normalized = mc._normalize_provider_tokens(
        cast(
            MappingProxyType[ProviderType, ProviderToken] | None,
            {"github": ProviderToken(token=SecretStr("1"))},
        )
    )
...
async def test_search_conversations_filters(monkeypatch, patch_conversation_manager):
    ...
    store = SimpleNamespace(
        search=_async_wrap(
            lambda page_id, limit: SimpleNamespace(
                results=conversations, next_page_id=None
            )
        )
    )
    monkeypatch.setattr(
        mc,
        "_resolve_conversation_store",
        _async_wrap(lambda store_override, user_id=None: store),
    )
    monkeypatch.setattr(mc.config, "conversation_max_age_seconds", 3600)
    result = await mc.search_conversations(
        limit=25,
        selected_repository="repo",
...
@pytest.mark.asyncio
async def test_get_conversation_details_returns_info(
    monkeypatch, patch_conversation_manager
):
    now = datetime.now(timezone.utc)
    metadata = ConversationMetadata(
...
async def test_delete_conversation_entry_handles_missing(monkeypatch):
    store = SimpleNamespace(
        get_metadata=_async_wrap(
            lambda cid: (_ for _ in ()).throw(FileNotFoundError())
        ),
    )
    monkeypatch.setattr(
        mc,
...
@pytest.mark.asyncio
async def test_delete_conversation_entry_success(
    monkeypatch, patch_conversation_manager
):
    store = SimpleNamespace(
        get_metadata=_async_wrap(
            lambda cid: ConversationMetadata(
                conversation_id=cid,
                title="title",
                selected_repository="repo",
                user_id="u",
...
def test_get_contextual_events_merges_before_and_after(monkeypatch):
    ...
    store = cast(EventStore, DummyEventStore())
...
def test_generate_prompt_parses_xml(monkeypatch):
    ...
    prompt = mc.generate_prompt(LLMConfig(), "template", "cid")
    assert prompt == "trim me"
    monkeypatch.setattr(
        mc.ConversationManagerImpl,
        "request_llm_completion",
        staticmethod(lambda *args, **kwargs: "no tags"),
    )
    with pytest.raises(ValueError):
        mc.generate_prompt(LLMConfig(), "template", "cid")
...
async def test_update_conversation_error(monkeypatch):
    ...
    store = SimpleNamespace(
        get_metadata=_async_wrap(lambda cid: metadata),
        save_metadata=_async_wrap(
            lambda meta: (_ for _ in ()).throw(RuntimeError("fail"))
        ),
    )
...
async def test_get_microagent_management_conversations_filters(monkeypatch):
    ...
    monkeypatch.setattr(
        mc, "_resolve_conversation_store", _async_wrap(lambda store_override: store)
    )
    monkeypatch.setattr(mc.config, "conversation_max_age_seconds", 3600)
    ...
    await mc.get_microagent_management_conversations(
        "repo", provider_tokens=provider_tokens
    )
...
async def test_handle_regular_conversation_returns_status(monkeypatch):
    ...
    monkeypatch.setattr(
        mc, "create_new_conversation", _async_wrap(lambda **kwargs: agent_loop_info)
    )
...
async def test_new_conversation_success(monkeypatch, fake_request):
    ...
    monkeypatch.setattr(
        ...
        lambda suggested_task, create_microagent, auth_type: (
    ...
    monkeypatch.setattr(
        ...
        lambda repo, provider, override_repo, override_provider, task, msg: (
            repo,
            provider,
            msg,
        ),
    ...
    monkeypatch.setattr(
        mc,
        "_prepare_conversation_params",
        lambda uid, tokens, secrets: ("user", MappingProxyType({}), UserSecrets()),
    )

    verify_called: dict[str, Any] = {}
    ...
    expected_response = mc.ConversationResponse(
        status="ok",
        conversation_id="cid",
    ...
    monkeypatch.setattr(
        ...
        _async_wrap(lambda **kwargs: expected_response),
    )
...
async def test_new_conversation_handles_known_error(monkeypatch, fake_request):
    ...
    monkeypatch.setattr(
        mc, "_apply_conversation_overrides", lambda *args, **kwargs: (None, None, "msg")
    )
    monkeypatch.setattr(
        mc,
        "_prepare_conversation_params",
    ...
    monkeypatch.setattr(
        ...
        "_handle_regular_conversation",
        _async_wrap(
            lambda **kwargs: (_ for _ in ()).throw(mc.MissingSettingsError("missing"))
        ),
    )
...
async def test_start_conversation_success(monkeypatch, patch_conversation_manager):
    ...
    monkeypatch.setattr(
        ...
        _async_wrap(lambda *args, **kwargs: SimpleNamespace()),
    )

    response = await mc.start_conversation(
        mc.ProvidersSetModel(providers_set=[ProviderType.GITHUB]),
        conversation_id="cid",
        user_id="user",
    )
...
async def test_start_conversation_not_found(monkeypatch):
    store = SimpleNamespace(
        get_metadata=_async_wrap(
            lambda cid: (_ for _ in ()).throw(RuntimeError("missing"))
        )
    )
    monkeypatch.setattr(
        mc,
    ...
    response = await mc.start_conversation(
        mc.ProvidersSetModel(), conversation_id="cid", user_id="user"
    )
    assert isinstance(response, JSONResponse)
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
async def test_stop_conversation_running(monkeypatch, patch_conversation_manager):
    patch_conversation_manager.agent_loop_info = [
        SimpleNamespace(conversation_id="cid", status=ConversationStatus.RUNNING)
    ]
    closed: dict[str, str] = {}
...
def test_normalize_provider_tokens_converts(monkeypatch):
    ...
    tokens = {
        "github": ProviderToken(token=SecretStr("value")),
        "invalid": ProviderToken(token=SecretStr("value")),
    }
    normalized = mc._normalize_provider_tokens(
        cast(MappingProxyType[ProviderType, ProviderToken] | None, tokens)
    )
    assert ProviderType.GITHUB in normalized
...
async def test_get_conversation_route(monkeypatch, fake_request):
    ...
    monkeypatch.setattr(
        ...
        _async_wrap(lambda cid, conversation_store, user_id: "details"),
    )
...
async def test_delete_conversation_route(monkeypatch, fake_request):
    ...
    monkeypatch.setattr(
        ...
        _async_wrap(lambda cid, user_id, conversation_store: True),
    )
...
async def test_get_prompt_success(monkeypatch):
    monkeypatch.setattr(
        mc, "EventStore", lambda **kwargs: cast(EventStore, SimpleNamespace())
    )
    monkeypatch.setattr(
        mc, "_get_contextual_events", lambda event_store, event_id: "events"
    )

    class DummySettingsStore:
        async def load(self):
            return SimpleNamespace(
                llm_model="model", llm_api_key="key", llm_base_url="url"
            )

    monkeypatch.setattr(mc, "generate_prompt_template", lambda events: "template")
    monkeypatch.setattr(
        mc, "generate_prompt", lambda config, template, conversation_id: "prompt"
    )
    metadata = ConversationMetadata(
        conversation_id="cid",
        title="title",
    ...
    assert response.status_code == status.HTTP_200_OK
    assert json.loads(bytes(response.body))["prompt"] == "prompt"
...
async def test_get_conversation_info_error(monkeypatch):
    ...
    result = await mc._get_conversation_info(
        conversation, num_connections=0, agent_loop_info=None
    )
    assert result is None
