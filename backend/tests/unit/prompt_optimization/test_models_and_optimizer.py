...
def test_registry_registers_testing_and_exports() -> None:
    ...
    active_variant = registry.get_active_variant(first.prompt_id)
    assert active_variant is not None
    assert active_variant.id == first_id
    assert registry.set_active_variant(first.prompt_id, second_id) is True
    active_variant = registry.get_active_variant(first.prompt_id)
    assert active_variant is not None
    assert active_variant.id == second_id
    ...
    cloned_variant = clone.get_variant(first_id)
    assert cloned_variant is not None
    assert cloned_variant.content == first.content
    active_variant = clone.get_active_variant(first.prompt_id)
    assert active_variant is not None
    assert active_variant.id == second_id
    ...
    first_variant = registry.get_variant(first_id)
    assert first_variant is not None
    first_variant.composite_score = 0.3  # type: ignore[attr-defined]
    second_variant = registry.get_variant(second_id)
    assert second_variant is not None
    second_variant.composite_score = 0.8  # type: ignore[attr-defined]
...
def test_optimizer_selection_switching_and_status(
    ...
    active_variant = registry.get_variant(active_id)
    assert active_variant is not None
    active_variant.composite_score = 0.5  # type: ignore[attr-defined]
    challenger_variant = registry.get_variant(challenger_id)
    assert challenger_variant is not None
    challenger_variant.composite_score = 0.9  # type: ignore[attr-defined]
...
def test_optimizer_management_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for index, variant_id in enumerate(variant_ids):
        ...
        variant.composite_score = 0.2 + index * 0.2  # type: ignore[attr-defined]
...
def test_optimizer_start_testing_best_variant() -> None:
    ...
    active_variant = registry.get_variant(variant_active)
    assert active_variant is not None
    active_variant.composite_score = 0.1  # type: ignore[attr-defined]
    candidate_variant = registry.get_variant(variant_candidate)
    assert candidate_variant is not None
    candidate_variant.composite_score = 0.9  # type: ignore[attr-defined]
...
def test_optimizer_should_not_switch_without_samples() -> None:
    ...
    assert active_variant is not None
    assert active_variant.id == active_id
...
def test_optimizer_cleanup_old_variants_respects_keep_count() -> None:
    ...
    for idx, variant_id in enumerate(ids):
        variant = registry.get_variant(variant_id)
        assert variant is not None
        variant.composite_score = idx  # type: ignore[attr-defined]
