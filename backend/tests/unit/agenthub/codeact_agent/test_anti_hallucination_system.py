...
def test_should_enforce_tools_detects_action_commands():
    system = AntiHallucinationSystem()
    choice = system.should_enforce_tools("Please create a new file", _make_state())
    assert choice == "required"
    assert system.stats["strict_mode_activations"] == 1


def test_should_enforce_tools_allows_question():
    system = AntiHallucinationSystem()
    choice = system.should_enforce_tools(
        "Why is the sky blue?", _make_state(), strict_mode=False
    )
    assert choice == "auto"
    assert system.stats["strict_mode_activations"] == 0


def test_should_enforce_tools_pending_operations_force_required():
    system = AntiHallucinationSystem()
    system.pending_file_operations.append(FileOperationContext("edit", ["foo.py"]))
    choice = system.should_enforce_tools(
        "What's next?", _make_state(), strict_mode=False
    )
    assert choice == "required"


def test_should_enforce_tools_default_strict():
    system = AntiHallucinationSystem()
    choice = system.should_enforce_tools("", _make_state())
    assert choice == "required"


def test_should_enforce_tools_auto_when_non_strict():
    system = AntiHallucinationSystem()
    choice = system.should_enforce_tools("inform me", _make_state(), strict_mode=False)
    assert choice == "auto"


def test_inject_verification_commands_tracks_operations():
    system = AntiHallucinationSystem()
...
def test_validate_response_detects_hallucination():
    ...
    assert ok is False
    assert error is not None and "HALLUCINATION PREVENTED" in error
    assert system.stats["hallucinations_prevented"] == 1


def test_validate_response_accepts_when_tools_called():
    system = AntiHallucinationSystem()
...
def test_extract_file_operation_claims_variations():
    system = AntiHallucinationSystem()
    text = (
...
def test_mark_and_cleanup_operations():
    system = AntiHallucinationSystem()
...
def test_get_stats_includes_pending_counts():
    system = AntiHallucinationSystem()
    system.pending_file_operations.append(
        FileOperationContext("edit", ["file.py"], False, 0)
    )
