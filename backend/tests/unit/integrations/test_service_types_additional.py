...
class BrokenService(BaseGitService):
    async def _make_request(
        self, url: str, params: dict | None = None, method=None
    ):  # pragma: no cover - unused
        return ({}, {})

    async def _get_cursorrules_url(
        self, repository: str
    ...
    def _get_microagents_directory_params(
        self, microagents_path: str
    ) -> dict | None:  # pragma: no cover - unused
        return None

    def _is_valid_microagent_file(
        self, item: dict
    ...
    def _get_file_path_from_item(
        self, item: dict, microagents_path: str
    ) -> str:  # pragma: no cover - unused
        return ""
...
def test_parse_microagent_content_success_and_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = _make_service()
    dummy_microagent = SimpleNamespace(
        content="body", metadata=SimpleNamespace(triggers=["on_push"])
    )
    with patch(
        "forge.integrations.service_types.BaseMicroagent.load",
        return_value=dummy_microagent,
    ):
...
async def test_fetch_and_check_cursorrules(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        service, "_make_request", AsyncMock(return_value=("content", {}))
    )
    content = await service._fetch_cursorrules_content("repo")
    assert content == "content"

    monkeypatch.setattr(
        service, "_fetch_cursorrules_content", AsyncMock(return_value="data")
    )
...
@pytest.mark.asyncio
async def test_process_microagents_directory_variants(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = _make_service()
    monkeypatch.setattr(
        service, "_get_microagents_directory_url", AsyncMock(return_value="dir")
    )
    ...
    monkeypatch.setattr(
        service, "_make_request", AsyncMock(return_value=values_response)
    )
    microagents = await service._process_microagents_directory(
        "repo", ".Forge/microagents"
    )
    ...
    monkeypatch.setattr(
        service, "_make_request", AsyncMock(return_value=nodes_response)
    )
    microagents = await service._process_microagents_directory(
        "repo", ".Forge/microagents"
    )
    ...
    monkeypatch.setattr(
        service, "_make_request", AsyncMock(return_value=invalid_response)
    )
    monkeypatch.setattr(
        service,
...
@pytest.mark.asyncio
async def test_get_microagents_combines_sources(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = _make_service()
    now = datetime.utcnow()
    cursorrules_resp = MicroagentResponse(
        name="rules", path=".cursorrules", created_at=now
    )
    directory_resp = [MicroagentResponse(name="agent", path="agent.md", created_at=now)]
    monkeypatch.setattr(
        service, "_check_cursorrules_file", AsyncMock(return_value=cursorrules_resp)
    )
    monkeypatch.setattr(
        service,
        "_process_microagents_directory",
        AsyncMock(return_value=directory_resp),
    )
