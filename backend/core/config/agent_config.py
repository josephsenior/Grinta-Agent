...
from backend.core.constants import (
    ...
    DEFAULT_AGENT_LLM_EDITOR_ENABLED,
    ...
    DEFAULT_AGENT_ULTIMATE_EDITOR_ENABLED,
...
class AgentConfig(BaseModel, metaclass=CanonicalModelMetaclass):
    ...
    enable_vector_memory: bool = Field(
        default=DEFAULT_AGENT_VECTOR_MEMORY_ENABLED, description="Enable persistent vector memory store"
    )
    enable_hybrid_retrieval: bool = Field(
        default=DEFAULT_AGENT_HYBRID_RETRIEVAL_ENABLED, description="Enable hybrid retrieval for vector memory"
    )
    enable_prompt_caching: bool = Field(
        default=DEFAULT_AGENT_PROMPT_CACHING_ENABLED, description="Enable prompt caching hints for LLMs"
    )
    ...
    auto_retry_on_error: bool = Field(
        default=DEFAULT_AGENT_AUTO_RETRY_ON_ERROR, description="Automatically retry actions when recoverable errors occur"
    )
    ...
    # Editor tool toggles
    enable_editor: bool = Field(default=DEFAULT_AGENT_EDITOR_ENABLED)
    enable_llm_editor: bool = Field(default=DEFAULT_AGENT_LLM_EDITOR_ENABLED)
    enable_ultimate_editor: bool = Field(default=DEFAULT_AGENT_ULTIMATE_EDITOR_ENABLED)
    ...
    max_autonomous_iterations: int = Field(
        default=DEFAULT_AGENT_MAX_AUTONOMOUS_ITERATIONS, description="Maximum self-directed iterations when autonomy is full"
    )
