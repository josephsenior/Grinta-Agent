...
class ProviderConfig:
    ...
    def validate_base_url(self, base_url: Optional[str]) -> Optional[str]:
        ...
        if self.handles_own_routing:
            logger.debug(
                f"Provider {self.name} handles own routing - clearing base_url"
            )
            return None
        ...
        if self.requires_protocol and not any(
            ...
            logger.warning(
                f"Provider {self.name} requires base_url with protocol - clearing invalid URL: {base_url}"
            )
            return None
...
class ProviderConfigurationManager:
    ...
    def _process_forbidden_param(
        ...
        """
        logger.debug(
            f"Removing forbidden parameter '{param_name}' for provider {provider}"
        )
        warnings.append(
    ...
    def _process_unknown_param(
        ...
        if provider == "unknown":
            logger.debug(
                f"Allowing unknown parameter '{param_name}' for unknown provider"
            )
        else:
            logger.debug(
                f"Parameter '{param_name}' not specified for {provider} provider - allowing for flexibility"
            )
    ...
    def validate_and_clean_params(
        ...
        if warnings:
            logger.warning(
                f"Parameter validation warnings for {provider}: {'; '.join(warnings)}"
            )

        logger.debug(
            f"Parameter validation complete for {provider}: {len(cleaned_params)} params kept"
        )
    ...
    def validate_api_key_format(self, provider: str, api_key: Optional[str]) -> bool:
        ...
        if len(api_key) < config.api_key_min_length:
            logger.warning(
                f"API key for {provider} is shorter than expected minimum ({config.api_key_min_length})"
            )
            return False
        ...
        if prefixes:
            if not any(api_key.startswith(prefix) for prefix in prefixes):
                logger.warning(
                    f"API key for {provider} doesn't match expected prefixes: {config.api_key_prefixes}"
                )
