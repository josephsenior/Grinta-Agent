...
def get_cli_parser() -> argparse.ArgumentParser:
    ...
    for p in [serve_parser]:
        p.add_argument(
            "--mount-cwd",
            action="store_true",
            help="Mount current working directory into the agent sandbox",
        )
        p.add_argument("--gpu", action="store_true", help="Enable GPU support")

    subparsers.add_parser(
        "cli", help="[DEPRECATED] Launch the Forge interface (now redirects to GUI)"
    )
