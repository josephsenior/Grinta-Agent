...
class SmartConfigCache:
    ...
    def _get_user_settings_memory(
        ...
        logger.debug(
            f"🚀 User settings memory cache MISS for {user_id} - loaded and cached"
        )
        return merged_settings
    ...
    def get_cache_stats(self) -> dict[str, Union[bool, str, float, int]]:
        ...
        """
        stats: dict[str, Union[bool, str, float, int]] = {
        ...
        if self.redis_available:
            try:
                ...
                stats.update(
                    {
                        "redis_used_memory_mb": info.get("used_memory", 0)
                        / 1024
                        / 1024,
