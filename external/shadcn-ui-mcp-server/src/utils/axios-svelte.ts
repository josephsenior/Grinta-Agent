...
async function getAvailableComponents(): Promise<string[]> {
  try {
    ...
    const response = await githubApi.get(
      `/repos/${REPO_OWNER}/${REPO_NAME}/contents/${REGISTRY_PATH}/ui`
    );
    ...
    if (error.response) {
      ...
      if (status === 403 && message.includes('rate limit')) {
        ...
        throw new Error(
          `Components directory not found. The path ${BLOCKS}/ui may not exist in the repository.`
        );
      } else if (status === 401) {
        throw new Error(
...
async function getBlockCode(blockName: string, includeComponents: boolean = true): Promise<any> {
  ...
  try {
    ...
    if (error.response?.status === 404) {
      throw new Error(
        `Block "${blockName}" not found. Available blocks can be found in the v4 blocks directory.`
      );
    }
    throw error;
  }
