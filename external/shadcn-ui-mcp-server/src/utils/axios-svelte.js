...
async function getAvailableComponents() {
    try {
        ...
        const components = response.data.map((item) => item.name);
...
async function getComponentMetadata(componentName) {
    try {
        ...
        const metadata = registryContent.items.map((item) => {
            return {
                name: item.name,
                type: item.type,
        ...
        const component = metadata.find((item) => item.name === componentName.toLowerCase());
        if (!component) {
 ...
 */
async function buildDirectoryTree(owner = REPO_OWNER, repo = REPO_NAME, path = BLOCKS, branch = REPO_BRANCH) {
    try {
 ...
 */
async function buildDirectoryTreeWithFallback(owner = REPO_OWNER, repo = REPO_NAME, path = BLOCKS, branch = REPO_BRANCH) {
    try {
